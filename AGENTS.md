...
- 测试随完成边界执行：先按缺陷影响确定验证范围，再选择能覆盖该范围的最小匹配验证；不得把“最小验证”理解为只验证用户指出的单一入口。改动进入可验证状态就运行验证，失败先修复再提交；提交前检查工作区，避免混入无关文件、临时截图、日志、安装包或非必要生成产物
...
- 定位缺陷后，必须沿根因检查共享实现的调用方、同类实现及相关状态切换，确认是否存在同源缺陷；不能只以用户截图、报错或点名的单一入口作为排查边界。排查以相关实现和证据为依据，不默认扩展成全项目审计。
- 在已确认的完整影响范围内做最小修复。用户已授权修复时，范围内的同源缺陷应一并修复和验证，无需等待用户逐个指出；发现无关问题时单独说明，按已有授权处理，不夹带无关重构。局部修改能恢复正确契约时不要重写；禁止吞错、复制业务分支或只改参数掩盖原因。确需重构时说明为何局部修复不足及验证范围。
- 收尾时简要说明同类问题的实际排查范围，区分已修复、检查后未发现同类缺陷、尚未验证的部分；不得仅凭单一入口通过就宣称所有功能均已修好。
...
- 每次开始修改前，先确认当前分支、上游分支与工作区状态。独占工作区或当前任务的独立 worktree 干净时执行 `git pull --rebase`，确保基于远端最新提交开始工作
- 单任务独占工作区时，提交前确认全部改动都属于当前任务且均已妥善处理；提交后不得遗留本任务应提交而未提交的改动，整个工作区必须恢复干净
- 同一项目有多个任务并行时，优先为每个任务使用独立分支和 Git worktree，让每个任务拥有可独立同步、验证、提交和回滚的干净工作区
- 多个任务不得已共用同一工作区时，不要求清理其他任务的未提交改动，但开始时必须记录本任务的明确文件范围；只允许用 `git add -- <本次文件...>` 精确暂存，提交前必须检查暂存区只包含本任务改动，禁止使用 `git add .`、`git add -A` 或 `git commit -a`
...
- Git、GitHub CI 查询与 npm 等网络故障先分类：临时 TLS 握手中断/EOF、连接重置或超时，保持原配置，先按 2/5/10 秒退避重试，仍中断时延长到 30–60 秒继续尝试并定期报告进度，不得仅因达到 3 次重试就结束任务。写操作结果不确定时，先核对远端引用或已发布版本，避免重复写入。非快进按上面的同步规则处理，认证/权限拒绝、证书校验失败停止盲重试并报告。禁止关闭 TLS 校验、擅改代理或网络配置、用强推绕过失败。
