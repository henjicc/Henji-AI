...
- **用户界面文案只服务于用户任务**：只呈现用户需要理解的目标、状态、结果与可执行动作。禁止把实现机制、协议名、风险分级、内部缩写、调试信息或面向开发者的解释性内容直接写进正式界面；不要为了说明代码如何工作而增加辅助文案。确需帮助用户做决定时，改写成用户能理解的影响与后果；仅供排查的信息进入现有日志、开发模式或调试界面。
