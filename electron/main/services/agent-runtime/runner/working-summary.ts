...
function completeActiveStep(
  ...
  return {
    stepId: event.toolCallId,
    title: active?.title ?? `执行工具 ${event.toolName}`,
    status: 'completed',
    toolName: event.toolName,
  ...
  if (event.type === 'PlanUpdated') {
    ...
    next = {
      ...
      unresolvedItems: appendBounded(
        ...
        `${event.toolName} 未收敛：${event.error.code}`,
        10
      ),
    ...
    next = {
      ...next,
      artifactRefs: appendBounded(next.artifactRefs, event.artifactRef, 12),
    }
