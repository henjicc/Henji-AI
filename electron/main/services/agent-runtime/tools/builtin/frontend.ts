...
export function createFrontendBuiltinTools(invoke: FrontendToolInvoker): AgentToolDefinition[] {
  ...
  const searchModels = defineAgentTool({
    ...
    version: 1,
    title: '搜索生成模型',
    description: '按模型名称关键词、媒体类型或供应商搜索生成模型目录，默认一次返回最多 32 个紧凑候选（最多 50 个），含通用模型标识、定性描述、能力标签与默认参数价格估算。内容、题材和风格不得放入 query；未指定模型时使用空 query 与 mediaType。用户明确偏好供应商时直接传 providerId；用户要求省钱时传 sortBy=lowest_estimated_price。',
    category: 'models',
    side: 'frontend',
    risk: 'R0',
    ...
    aiInputSchema: {
      ...
      properties: {
        ...
        sortBy: { type: 'string', enum: ['registry', 'recommended', 'lowest_estimated_price'] },
  ...
  const getModelSchema = defineAgentTool({
    ...
    version: 1,
    title: '读取模型参数结构',
    description: '读取单个生成模型的通用标识、定性描述、能力标签、当前默认参数价格估算与裁剪后的参数结构。',
    category: 'models',
    side: 'frontend',
    risk: 'R0',
    permission: 'model_catalog:read',
