...
export function requireFrontendSuccess(result: ApplicationCapabilityResult): Record<string, unknown> {
  ...
  const facts = applicationTransactionFailureFactsSchema.safeParse(result.error.details?.transaction)
  if (facts.success) throw new ApplicationTransactionFactsError(`[${result.error.code}] ${result.error.message}`, facts.data)
