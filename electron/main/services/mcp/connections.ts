...
const recordSchema = z.object({ id: z.string().uuid(), name: z.string().min(1).max(80), expiresAt: z.number(), token: z.string().length(64), allowWrites: z.boolean().default(false), allowDestructive: z.boolean().default(false) }).strict()
...
export class McpConnections {
  ...
  list(): McpConnectionInfo[] { return this.load().map(({ id, name, expiresAt, allowWrites, allowDestructive }) => ({ id, name, expiresAt, allowWrites, allowDestructive })) }
  access(id: string): { allowWrites: boolean; allowDestructive: boolean } {
    this.assertActive(id)
    const record = this.load().find((item) => item.id === id)!
    return { allowWrites: record.allowWrites, allowDestructive: record.allowWrites && record.allowDestructive }
  }
  create(name: string, access: { allowWrites?: boolean; allowDestructive?: boolean } = {}): McpConnectionInfo {
    const records = this.load().filter((record) => record.expiresAt > this.now())
    if (records.length >= 32) throw new Error('连接数量已达上限，请先撤销不再使用的连接。')
    const record = recordSchema.parse({ id: randomUUID(), name: name.trim(), expiresAt: this.now() + 30 * 24 * 60 * 60 * 1000, token: randomBytes(32).toString('hex'), allowWrites: access.allowWrites ?? false, allowDestructive: access.allowWrites === true && access.allowDestructive === true })
    this.save([...records, record])
    return { id: record.id, name: record.name, expiresAt: record.expiresAt, allowWrites: record.allowWrites, allowDestructive: record.allowDestructive }
  }
