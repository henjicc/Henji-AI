...
it('真实SDK客户端读取媒体，缺少凭据和撤销连接后均不能读取', async () => {
  ...
  try {
    ...
    expect(result.structuredContent).toMatchObject({ ok: true, data: { mimeType: 'image/png', byteLength: 5, totalBytes: bytes.length, base64: bytes.subarray(0, 5).toString('base64') } })
