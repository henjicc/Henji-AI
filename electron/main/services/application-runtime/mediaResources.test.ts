...
describe('稳定业务引用的媒体读取', () => {
  ...
  it.each(['generation.result', 'asset', 'canvas.node'] as const)('读取 %s 正式关联字节、末页及 EOF', async (kind) => {
    ...
    expect(await readApplicationMediaResource({ ref: { kind, id }, offset: 5 })).toMatchObject({ base64: '', byteLength: 0, eof: true })
    await expect(readApplicationMediaResource({ ref: { kind, id }, offset: 6 })).rejects.toThrow()
  })
  ...
  it('当前任务及旧历史均只按完整主键读取保存结果，未保存任务不猜其他历史', async () => {
    ...
    await expect(readApplicationMediaResource({ ref: { kind: 'generation.result', id: 'current-task' }, offset: 8 })).rejects.toMatchObject({ code: 'MEDIA_RANGE_OUT_OF_BOUNDS' })
  })
  ...
  it.each([{ outputIndex: -1 }, { outputIndex: 0.5 }, { offset: -1 }, { offset: 1.5 }, { offset: Infinity }, { length: 0 }, { length: 262145 }, { length: 1.5 }])('拒绝非法范围 %j', async (range) => {
    await expect(readApplicationMediaResource({ ref: { kind: 'asset', id: 'x' }, ...range })).rejects.toThrow()
  })
  it('拒绝未知类型、输入路径/URL和多余参数，不把引用当路径', async () => {
    await expect(readApplicationMediaResource({ ref: { kind: 'file', id: '/secret.png' } })).rejects.toThrow()
    for (const id of ['https://example.test/a.png', path.join(state.root, 'a.png')]) await expect(readApplicationMediaResource({ ref: { kind: 'asset', id } })).rejects.toThrow()
    await expect(readApplicationMediaResource({ ref: { kind: 'asset', id: 'x' }, path: '/secret.png' } as Parameters<typeof readApplicationMediaResource>[0])).rejects.toThrow()
  })
  it('拒绝远程代理、根目录逃逸、非媒体和错误序号', async () => {
    for (const file_path of ['https://example.test/a.png', path.join(directory, 'secret.png'), path.join(state.root, 'secret.txt')]) {
      ...
      await expect(readApplicationMediaResource({ ref: { kind: 'asset', id: 'x' } })).rejects.toThrow()
    }
    await expect(readApplicationMediaResource({ ref: { kind: 'asset', id: 'x' }, outputIndex: 1 })).rejects.toThrow()
  })
  it('真实目录链接指向授权根外仍拒绝', async () => {
    ...
    await expect(readApplicationMediaResource({ ref: { kind: 'asset', id: 'x' } })).rejects.toMatchObject({ code: 'MEDIA_ACCESS_DENIED' })
  })
})
