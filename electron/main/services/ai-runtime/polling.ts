import { AiRuntimeError, cancelledError } from './errors'
...
/** 终止型错误：重试无意义，必须原样上抛 */
const TERMINAL_POLL_ERROR_CODES = new Set(['provider_task_failed', 'cancelled'])

function isTerminalPollError(error: unknown): boolean {
  return error instanceof AiRuntimeError && TERMINAL_POLL_ERROR_CODES.has(error.code)
}
