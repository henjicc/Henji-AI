...
function parseRenderRequest(input: unknown): CameraStageRenderRequestDto {
  ...
  return { requestId, nodeId, projectId, resolutionPreset }
}
...
function parseRenderResult(input: unknown): CameraStageRenderResultDto {
  ...
  if (![durationSeconds, frameCount, width, height].every(Number.isFinite)) {
    throw new Error('Expected finite camera stage render result dimensions')
  }
  return { mediaUrl, mediaPath, savedPath, durationSeconds, frameCount, width, height }
}
