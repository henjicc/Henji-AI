...
- [ ] 将所有“负面提示”与常规文本输入改为 `TextInput`（扩展支持 `type="password"` 用于 API 密钥）
  - [x] Wan 负面提示
  - [x] 通用负面提示（除特定模型）
- [ ] 抽象“复杂面板触发器”（模型选择器、分辨率设置、音色面板）为通用触发+弹层组件
...
- [ ] 抽象统一“上传器”（图片/视频/音频），支持 `accept/multiple/maxCount/onFiles` 与拖拽/粘贴
