...
- **参数配置**: `src/schemas/modelParams.ts`
- **渲染组件**: `src/components/ui/SchemaForm.tsx`

### 2. 如何新增/修改模型参数
1. 打开 `src/schemas/modelParams.ts`。
2. 定义参数数组 (例如 `export const newModelParams: ParamDef[] = [...]`)。
3. 在 `src/components/MediaGenerator.tsx` 中引入该配置。
4. 使用 `<SchemaForm schema={newModelParams} values={...} onChange={...} />` 渲染。

### 3. 支持的参数类型
- `dropdown`: 下拉选择 (支持 `options` 动态生成, `disabled` 联动)
- `toggle`: 开关
- `number`: 数字输入 (支持 `min`, `max`, `step`, `precision`)
- `text`: 文本输入

### 4. 联动逻辑
- **显示/隐藏**: 使用 `hidden: (values) => boolean`
- **禁用/启用**: 使用 `disabled: (values) => boolean`
- **动态选项**: `options` 可以是一个函数 `(values) => Option[]`

### 5. 常见问题与注意事项
- **Dropdown 显示英文问题**: `SchemaForm` 渲染 `Dropdown` 时，若未指定 `display` 属性，必须在 `SchemaForm` 内部根据当前 `value` 查找 `options` 中的 `label` 并传给 `Dropdown`，否则 `Dropdown` 组件会默认显示 `value` (通常是英文 ID)。
...
- [x] 将所有参数区块的行容器统一替换为 `ParamRow`，确保不同宽度组件的视觉间距一致
- [x] 为 `Dropdown` 等组件接入选项禁用策略，表达联动约束（分辨率受时长限制等）
- [x] 在 `src/types/` 定义参数 schema，并实现表单渲染器按 schema 自动选择组件与属性
  - [x] 定义 Schema 类型 (`src/types/schema.ts`)
  - [x] 实现渲染器组件 (`src/components/ui/SchemaForm.tsx`)
  - [x] 迁移 Wan 2.5 模型参数 (作为试点)
  - [x] 迁移 Vidu 模型参数 (模式/宽高比/风格/运动幅度/BGM)
  - [x] 迁移 Kling 模型参数 (时长/宽高比/CFG)
  - [x] 迁移 Hailuo 模型参数 (时长/分辨率/Fast模式/提示词优化)
  - [x] 迁移 PixVerse 模型参数 (宽高比/分辨率/快速模式/风格)
  - [x] 迁移 Seedance 模型参数 (版本/分辨率/宽高比/时长/相机固定)
  - [x] 迁移 Seedream 模型参数 (批量生成)
  - [x] 迁移 Minimax Speech 音频参数 (规格/音色/情绪/语种/采样率/比特率/格式/声道/高级选项)
