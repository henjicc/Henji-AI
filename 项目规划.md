# 痕迹AI - 开发规范与技术方案

## 一、技术栈选型

### 核心框架
- **React 18+** - 组件化架构
- **TypeScript 5.x** - 类型安全保障
- **Vite 5.x** - 构建工具(替代 Create React App)
- **Tailwind CSS 4.x** - 原子化 CSS 框架

### UI 组件库
**推荐方案: shadcn/ui**
- 基于 Radix UI 原语,天生无障碍访问
- 与 Tailwind CSS 深度集成
- 复制到项目而非依赖包,完全掌控代码
- TypeScript 原生支持
- 按需引入,无冗余代码

**备选方案:**
- **Mantine** - 功能全面,100+ 组件
- **Chakra UI** - 开发体验优秀,易于定制
- **NextUI** - 现代化设计,性能优异

### 状态管理
- **Zustand** (推荐) - 轻量级,API 简洁
- **Redux Toolkit** (备选) - 大型项目可选

### HTTP 请求
- **Axios** - 统一请求封装,拦截器支持
- **TanStack Query (React Query)** - 服务端状态管理,自动缓存和重试

### 工具库
- **React Hook Form** - 表单管理
- **Zod** - 运行时类型验证
- **date-fns** - 日期处理
- **lodash-es** - 工具函数(按需导入)
- **react-responsive** - 响应式媒体查询

### 国际化 (i18n)
- **react-i18next** - React 国际化库
- **i18next** - i18n 核心框架
- **i18next-browser-languagedetector** - 自动语言检测

### 图标库
- **lucide-react** - 现代化图标集,与 shadcn/ui 配套

---

## 二、响应式设计规范

### 1. 设计原则

#### Desktop-First 优先
由于主要面向桌面端使用,采用 Desktop-First 设计策略,优先保证桌面端的完整功能和最佳体验,移动端作为补充支持。

```typescript
// ✅ 推荐: Desktop-First
const Container = styled.div`
  padding: 3rem;           // 桌面端默认
  
  @media (max-width: 1024px) {
    padding: 2rem;         // 平板
  }
  
  @media (max-width: 768px) {
    padding: 1rem;         // 移动端
  }
`

// Tailwind CSS 使用方式
<div className="
  p-8              // 桌面端默认
  lg:p-6           // 小屏桌面
  md:p-4           // 平板
  sm:p-2           // 移动端
">
```

### 2. 断点系统 (Breakpoints)

使用 Tailwind CSS 默认断点系统:

```typescript
// tailwind.config.js
export default {
  theme: {
    screens: {
      'sm': '640px',   // 手机横屏/小平板
      'md': '768px',   // 平板
      'lg': '1024px',  // 小屏笔记本
      'xl': '1280px',  // 桌面 (主要目标)
      '2xl': '1536px', // 大屏桌面
    }
  }
}
```

**桌面布局**: > 1024px (主要开发目标)  
**平板布局**: 768px - 1024px (适配支持)  
**移动端布局**: < 768px (基础支持)

### 3. 响应式组件库

#### 推荐使用 react-responsive
react-responsive 是最易用的 React 媒体查询模块,提供 Hooks 和组件两种方式:

```bash
npm install react-responsive
```

**使用 Hooks (推荐)**
```typescript
import { useMediaQuery } from 'react-responsive'

const ImageGenerator: React.FC = () => {
  const isDesktop = useMediaQuery({ minWidth: 1024 })  // 主要关注桌面端
  const isTablet = useMediaQuery({ minWidth: 768, maxWidth: 1023 })
  const isMobile = useMediaQuery({ maxWidth: 767 })

  // 桌面端优先,完整功能
  if (isDesktop) {
    return <DesktopLayout />  // 完整界面,所有功能
  }
  
  // 平板和移动端简化版
  if (isTablet) {
    return <TabletLayout />   // 简化但保持可用
  }
  
  return <MobileLayout />      // 基础功能
}
```

**使用组件方式**
```typescript
import { useMediaQuery } from 'react-responsive'

// 创建复用的响应式组件
const Desktop = ({ children }) => {
  const isDesktop = useMediaQuery({ minWidth: 1024 })
  return isDesktop ? children : null
}

const TabletAndBelow = ({ children }) => {
  const isTabletAndBelow = useMediaQuery({ maxWidth: 1023 })
  return isTabletAndBelow ? children : null
}

// 使用
const App = () => (
  <>
    <Desktop>
      <DesktopSidebar />
      <DesktopToolbar />
    </Desktop>
    <TabletAndBelow>
      <SimplifiedNavbar />
    </TabletAndBelow>
  </>
)
```

### 4. 布局策略

#### 桌面端布局特点
桌面端有更多屏幕空间,可以实现复杂的多栏布局和并排显示

```typescript
// ✅ 桌面端: 侧边栏 + 主内容区 + 预览区三栏布局
<div className="flex h-screen">
  {/* 左侧边栏 - 固定宽度 */}
  <aside className="
    w-80                    // 桌面端 320px
    lg:w-64                 // 小屏桌面 256px
    md:hidden               // 平板以下隐藏,使用抽屉
    border-r border-zinc-200 dark:border-zinc-700
    overflow-y-auto
  ">
    <ConfigPanel />
  </aside>

  {/* 主内容区 - 弹性宽度 */}
  <main className="flex-1 overflow-y-auto">
    <div className="
      max-w-7xl mx-auto       // 桌面端最大宽度限制
      p-8                     // 桌面端充足间距
      md:p-6                  // 平板
      sm:p-4                  // 移动端
    ">
      <GeneratorForm />
    </div>
  </main>

  {/* 右侧预览区 - 可选 */}
  <aside className="
    w-96                    // 桌面端 384px
    xl:block                // 仅大屏桌面显示
    hidden                  // 小屏隐藏
    border-l border-zinc-200 dark:border-zinc-700
    overflow-y-auto
  ">
    <PreviewPanel />
  </aside>
</div>

// 平板/移动端: 单栏堆叠布局
<div className="flex flex-col md:hidden">
  <MobileHeader />
  <div className="flex-1 p-4">
    <SimplifiedForm />
  </div>
</div>
```

#### Grid 布局示例
```typescript
// 桌面端: 3-4 列网格
<div className="
  grid 
  grid-cols-4           // 桌面端: 4列
  2xl:grid-cols-4
  xl:grid-cols-3        // 大屏: 3列
  lg:grid-cols-3        // 笔记本: 3列
  md:grid-cols-2        // 平板: 2列
  sm:grid-cols-1        // 移动端: 1列
  gap-6                 // 桌面端较大间距
  md:gap-4              // 平板
  sm:gap-3              // 移动端
">
  {items.map(item => <Card key={item.id} {...item} />)}
</div>
```

### 5. 字体和间距

#### 桌面端优先的字体大小
```typescript
<div>
  <h1 className="
    text-4xl              // 桌面端大标题
    lg:text-3xl           // 小屏桌面
    md:text-2xl           // 平板
    sm:text-xl            // 移动端
    font-bold
  ">
    AI 媒体生成工具
  </h1>

  <p className="
    text-lg               // 桌面端正文
    md:text-base          // 平板及以下
    text-zinc-600 dark:text-zinc-400
  ">
    描述文本
  </p>
</div>
```

#### 间距系统
```typescript
// 桌面端充足的呼吸感
<div className="
  space-y-8             // 桌面端: 2rem 间距
  md:space-y-6          // 平板: 1.5rem
  sm:space-y-4          // 移动端: 1rem
">
  <Section />
  <Section />
</div>
```

### 6. 导航栏设计

#### 桌面端: 完整横向导航
```typescript
const Navbar: React.FC = () => {
  const isDesktop = useMediaQuery({ minWidth: 1024 })

  return (
    <nav className="bg-white dark:bg-[#131313] border-b border-zinc-200 dark:border-zinc-800">
      <div className="max-w-7xl mx-auto px-6 py-4">
        {isDesktop ? (
          // 桌面端: 完整横向导航
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-8">
              <Logo />
              <NavLinks />  {/* 所有导航链接平铺 */}
            </div>
            <div className="flex items-center gap-4">
              <SearchBar />
              <ThemeSwitcher />
              <LanguageSwitcher />
              <UserMenu />
            </div>
          </div>
        ) : (
          // 平板/移动端: 汉堡菜单
          <MobileNavbar />
        )}
      </div>
    </nav>
  )
}
```

### 7. 桌面端特殊优化

#### 鼠标悬停效果
桌面端可以充分利用鼠标悬停效果提升交互体验

```typescript
<button className="
  px-6 py-3
  bg-blue-600 text-white rounded-lg
  
  // 桌面端悬停效果
  hover:bg-blue-700 
  hover:shadow-lg
  hover:scale-105
  
  // 桌面端过渡动画
  transition-all duration-200
  
  // 移动端禁用悬停(避免点击后效果残留)
  md:hover:scale-100
">
  生成图片
</button>
```

#### 快捷键支持
桌面端应该提供键盘快捷键

```typescript
// hooks/useKeyboardShortcuts.ts
const useKeyboardShortcuts = () => {
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      // Ctrl/Cmd + Enter: 提交表单
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
        handleSubmit()
      }
      
      // Ctrl/Cmd + K: 打开搜索
      if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
        e.preventDefault()
        openSearch()
      }
      
      // Esc: 关闭弹窗
      if (e.key === 'Escape') {
        closeModal()
      }
    }

    window.addEventListener('keydown', handleKeyPress)
    return () => window.removeEventListener('keydown', handleKeyPress)
  }, [])
}
```

#### 右键菜单
桌面端可以实现自定义右键菜单

```typescript
const ImageCard = ({ image }) => {
  const [contextMenu, setContextMenu] = useState<{x: number, y: number} | null>(null)

  const handleContextMenu = (e: React.MouseEvent) => {
    e.preventDefault()
    setContextMenu({ x: e.clientX, y: e.clientY })
  }

  return (
    <>
      <div onContextMenu={handleContextMenu}>
        <img src={image.url} alt={image.prompt} />
      </div>
      
      {contextMenu && (
        <ContextMenu 
          position={contextMenu}
          onClose={() => setContextMenu(null)}
        >
          <MenuItem onClick={handleDownload}>下载</MenuItem>
          <MenuItem onClick={handleCopy}>复制</MenuItem>
          <MenuItem onClick={handleDelete}>删除</MenuItem>
        </ContextMenu>
      )}
    </>
  )
}
```

### 8. 性能优化

#### 桌面端可以加载更高质量资源
```typescript
const ImageDisplay = () => {
  const isDesktop = useMediaQuery({ minWidth: 1024 })

  return (
    <img 
      src={isDesktop ? image.highResUrl : image.thumbnailUrl}
      alt={image.prompt}
      className="w-full h-auto"
    />
  )
}
```

### 9. 测试规范

桌面端优先的测试策略

**必测分辨率(按优先级)**:
- 🖥️ **1920x1080** (主要目标 - 最常见桌面分辨率)
- 🖥️ **2560x1440** (2K 显示器)
- 💻 **1366x768** (小屏笔记本)
- 🖥️ **3840x2160** (4K 显示器)
- 📲 **1024x768** (平板)
- 📱 **375x667** (移动端基础支持)

**测试工具**:
- Chrome DevTools (响应式模式)
- 真实桌面浏览器测试
- 多显示器配置测试

---

## 三、多语言支持 (i18n) 规范

### 1. 技术选型

#### 推荐: react-i18next
react-i18next 是最流行的 React 国际化库,建立在 i18next 框架之上,每周下载量超过 630 万次,GitHub 上有 9.8K stars

**为什么选择 react-i18next:**
- ✅ 功能最完善,插件生态丰富
- ✅ 与 React Hooks 完美集成
- ✅ 支持命名空间,按需加载翻译
- ✅ TypeScript 支持良好
- ✅ 社区活跃,文档完善

**安装**:
```bash
npm install i18next react-i18next i18next-browser-languagedetector
```

### 2. 项目结构

```
src/
├── locales/                 # 翻译文件
│   ├── zh-CN/              # 简体中文
│   │   ├── common.json     # 通用翻译
│   │   ├── nav.json        # 导航翻译
│   │   ├── image-gen.json  # 图片生成模块
│   │   └── video-gen.json  # 视频生成模块
│   ├── en-US/              # 英文
│   │   ├── common.json
│   │   ├── nav.json
│   │   ├── image-gen.json
│   │   └── video-gen.json
│   └── index.ts            # i18n 初始化配置
│
├── components/
│   └── LanguageSwitcher.tsx # 语言切换组件
```

### 3. i18n 配置

```typescript
// src/locales/index.ts
import i18n from 'i18next'
import { initReactI18next } from 'react-i18next'
import LanguageDetector from 'i18next-browser-languagedetector'

// 导入翻译文件
import zhCN_common from './zh-CN/common.json'
import zhCN_nav from './zh-CN/nav.json'
import enUS_common from './en-US/common.json'
import enUS_nav from './en-US/nav.json'

i18n
  .use(LanguageDetector) // 自动检测用户语言
  .use(initReactI18next)
  .init({
    resources: {
      'zh-CN': {
        common: zhCN_common,
        nav: zhCN_nav,
      },
      'en-US': {
        common: enUS_common,
        nav: enUS_nav,
      }
    },
    fallbackLng: 'zh-CN',     // 默认语言: 中文
    defaultNS: 'common',      // 默认命名空间
    
    interpolation: {
      escapeValue: false,     // React 已经防 XSS
    },
    
    detection: {
      // 语言检测顺序
      order: ['localStorage', 'navigator'],
      caches: ['localStorage'],
    }
  })

export default i18n
```

### 4. 翻译文件示例

```json
// locales/zh-CN/common.json
{
  "app": {
    "title": "AI 媒体生成工具",
    "description": "使用 AI 生成图片和视频"
  },
  "buttons": {
    "generate": "生成",
    "cancel": "取消",
    "download": "下载",
    "clear": "清除"
  },
  "messages": {
    "success": "生成成功!",
    "error": "生成失败,请重试",
    "loading": "正在生成中..."
  },
  "form": {
    "prompt": "提示词",
    "promptPlaceholder": "输入你想生成的内容描述...",
    "model": "模型",
    "size": "尺寸",
    "quality": "质量"
  }
}

// locales/zh-CN/nav.json
{
  "home": "首页",
  "imageGen": "图片生成",
  "videoGen": "视频生成",
  "history": "历史记录",
  "settings": "设置"
}

// locales/en-US/common.json
{
  "app": {
    "title": "AI Media Generator",
    "description": "Generate images and videos with AI"
  },
  "buttons": {
    "generate": "Generate",
    "cancel": "Cancel",
    "download": "Download",
    "clear": "Clear"
  },
  "messages": {
    "success": "Generated successfully!",
    "error": "Generation failed, please retry",
    "loading": "Generating..."
  },
  "form": {
    "prompt": "Prompt",
    "promptPlaceholder": "Enter your content description...",
    "model": "Model",
    "size": "Size",
    "quality": "Quality"
  }
}
```

### 5. 使用方式

#### 在组件中使用 useTranslation Hook
```typescript
import { useTranslation } from 'react-i18next'

const ImageGenerator: React.FC = () => {
  const { t } = useTranslation(['common', 'nav']) // 加载多个命名空间

  return (
    <div>
      <h1>{t('app.title')}</h1>
      <p>{t('app.description')}</p>
      
      <input 
        placeholder={t('form.promptPlaceholder')}
      />
      
      <button>{t('buttons.generate')}</button>
      
      {/* 带变量插值 */}
      <p>{t('messages.success', { name: 'image.png' })}</p>
    </div>
  )
}
```

#### 语言切换组件
```typescript
import { useTranslation } from 'react-i18next'
import { Globe } from 'lucide-react'

const LanguageSwitcher: React.FC = () => {
  const { i18n } = useTranslation()
  
  const languages = [
    { code: 'zh-CN', name: '简体中文', flag: '🇨🇳' },
    { code: 'en-US', name: 'English', flag: '🇺🇸' },
  ]

  return (
    <div className="flex items-center gap-2">
      <Globe className="w-5 h-5" />
      <select 
        value={i18n.language}
        onChange={(e) => i18n.changeLanguage(e.target.value)}
        className="bg-transparent border rounded px-2 py-1"
      >
        {languages.map(lang => (
          <option key={lang.code} value={lang.code}>
            {lang.flag} {lang.name}
          </option>
        ))}
      </select>
    </div>
  )
}
```

### 6. TypeScript 类型安全

```typescript
// src/types/i18n.d.ts
import 'react-i18next'
import common from './locales/zh-CN/common.json'
import nav from './locales/zh-CN/nav.json'

declare module 'react-i18next' {
  interface CustomTypeOptions {
    defaultNS: 'common'
    resources: {
      common: typeof common
      nav: typeof nav
    }
  }
}
```

现在使用 `t()` 函数时会有自动补全和类型检查!

### 7. 最佳实践

#### 翻译键命名规范
```typescript
// ✅ 推荐: 使用命名空间和层级结构
t('common:buttons.generate')
t('nav:imageGen')
t('form.prompt')

// ❌ 避免: 扁平化命名
t('generateButton')
t('imageGenNav')
```

#### 处理复数和性别
```json
// common.json
{
  "items": "{{count}} 个项目",
  "items_zero": "没有项目",
  "items_one": "1 个项目",
  "items_other": "{{count}} 个项目"
}
```

```typescript
t('items', { count: 0 })  // "没有项目"
t('items', { count: 1 })  // "1 个项目"
t('items', { count: 5 })  // "5 个项目"
```

#### 懒加载翻译文件
```typescript
// 仅在需要时加载特定命名空间
const { t } = useTranslation()

// 动态加载
i18n.loadNamespaces(['image-gen', 'video-gen'])
```

---

## 四、主题切换规范

### 1. 技术方案

使用 React Context API 管理主题状态,结合 Tailwind CSS 的深色模式支持和 localStorage 实现主题持久化

**核心特性**:
- ✅ 支持亮色/暗色主题切换
- ✅ 默认暗色主题
- ✅ 自动检测系统偏好
- ✅ 主题状态持久化 (localStorage)
- ✅ 平滑过渡动画

### 2. Tailwind CSS 配置

```javascript
// tailwind.config.js
export default {
  darkMode: 'class', // 使用 class 策略
  theme: {
    extend: {
      colors: {
        // 亮色主题
        light: {
          bg: '#ffffff',
          'bg-secondary': '#f5f5f5',
          text: '#000000',
          'text-secondary': '#404040',
          border: '#e5e5e5',
          accent: '#0066cc',
        },
        // 暗色主题
        dark: {
          bg: '#000000',
          'bg-secondary': '#1a1a1a',
          text: '#ffffff',
          'text-secondary': '#a0a0a0',
          border: '#404040',
          accent: '#3b82f6',
        }
      }
    }
  }
}
```

### 3. 主题 Context 实现

```typescript
// src/contexts/ThemeContext.tsx
import React, { 
  createContext, 
  useContext, 
  useState, 
  useEffect,
  ReactNode 
} from 'react'

type Theme = 'light' | 'dark' | 'auto'
type ResolvedTheme = 'light' | 'dark'

interface ThemeContextType {
  theme: Theme
  resolvedTheme: ResolvedTheme
  setTheme: (theme: Theme) => void
  toggleTheme: () => void
}

const ThemeContext = createContext<ThemeContextType | undefined>(undefined)

export const ThemeProvider: React.FC<{ children: ReactNode }> = ({ 
  children 
}) => {
  // 从 localStorage 读取,默认为 'dark'
  const [theme, setThemeState] = useState<Theme>(() => {
    const stored = localStorage.getItem('theme') as Theme
    return stored || 'dark' // 默认暗色主题
  })

  // 计算实际应用的主题
  const [resolvedTheme, setResolvedTheme] = useState<ResolvedTheme>('dark')

  useEffect(() => {
    const root = window.document.documentElement
    
    // 检测系统偏好
    const systemPrefersDark = window.matchMedia(
      '(prefers-color-scheme: dark)'
    ).matches

    let applied: ResolvedTheme
    
    if (theme === 'auto') {
      applied = systemPrefersDark ? 'dark' : 'light'
    } else {
      applied = theme as ResolvedTheme
    }

    setResolvedTheme(applied)

    // 应用主题
    root.classList.remove('light', 'dark')
    root.classList.add(applied)
    
    // 保存到 localStorage
    localStorage.setItem('theme', theme)
  }, [theme])

  // 监听系统主题变化
  useEffect(() => {
    const mediaQuery = window.matchMedia('(prefers-color-scheme: dark)')
    
    const handleChange = () => {
      if (theme === 'auto') {
        const newTheme = mediaQuery.matches ? 'dark' : 'light'
        setResolvedTheme(newTheme)
      }
    }

    mediaQuery.addEventListener('change', handleChange)
    return () => mediaQuery.removeEventListener('change', handleChange)
  }, [theme])

  const setTheme = (newTheme: Theme) => {
    setThemeState(newTheme)
  }

  const toggleTheme = () => {
    setThemeState(current => {
      if (current === 'light') return 'dark'
      if (current === 'dark') return 'light'
      return 'dark' // auto 时切换到 dark
    })
  }

  return (
    <ThemeContext.Provider 
      value={{ theme, resolvedTheme, setTheme, toggleTheme }}
    >
      {children}
    </ThemeContext.Provider>
  )
}

export const useTheme = () => {
  const context = useContext(ThemeContext)
  if (!context) {
    throw new Error('useTheme must be used within ThemeProvider')
  }
  return context
}
```

### 4. 在应用中使用

```typescript
// src/main.tsx
import React from 'react'
import ReactDOM from 'react-dom/client'
import App from './App'
import { ThemeProvider } from './contexts/ThemeContext'
import './locales' // i18n 初始化
import './index.css'

ReactDOM.createRoot(document.getElementById('root')!).render(
  <React.StrictMode>
    <ThemeProvider>
      <App />
    </ThemeProvider>
  </React.StrictMode>
)
```

### 5. 主题切换组件

```typescript
// src/components/ThemeSwitcher.tsx
import { useTheme } from '@/contexts/ThemeContext'
import { Sun, Moon, Monitor } from 'lucide-react'

const ThemeSwitcher: React.FC = () => {
  const { theme, setTheme } = useTheme()

  const themes: Array<{ value: Theme; icon: React.ReactNode; label: string }> = [
    { value: 'light', icon: <Sun className="w-4 h-4" />, label: '亮色' },
    { value: 'dark', icon: <Moon className="w-4 h-4" />, label: '暗色' },
    { value: 'auto', icon: <Monitor className="w-4 h-4" />, label: '自动' },
  ]

  return (
    <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-800 rounded-lg p-1">
      {themes.map(({ value, icon, label }) => (
        <button
          key={value}
          onClick={() => setTheme(value)}
          className={`
            flex items-center gap-2 px-3 py-2 rounded-md
            transition-all duration-200
            ${theme === value 
              ? 'bg-white dark:bg-zinc-700 shadow-sm' 
              : 'hover:bg-zinc-200 dark:hover:bg-zinc-700'
            }
          `}
          title={label}
        >
          {icon}
          <span className="text-sm">{label}</span>
        </button>
      ))}
    </div>
  )
}

export default ThemeSwitcher
```

### 6. CSS 全局样式

```css
/* src/index.css */
@tailwind base;
@tailwind components;
@tailwind utilities;

@layer base {
  /* 平滑过渡 */
  * {
    @apply transition-colors duration-200;
  }

  /* 根元素样式 */
  html {
    @apply antialiased;
  }

  body {
    @apply bg-white dark:bg-black;
    @apply text-zinc-900 dark:text-zinc-100;
  }
}

/* 自定义滚动条 */
::-webkit-scrollbar {
  @apply w-2;
}

::-webkit-scrollbar-track {
  @apply bg-zinc-100 dark:bg-[#131313];
}

::-webkit-scrollbar-thumb {
  @apply bg-zinc-300 dark:bg-zinc-700 rounded-full;
  @apply hover:bg-zinc-400 dark:hover:bg-zinc-600;
}
```

### 7. 使用 Tailwind 暗色模式类

```typescript
// 在组件中使用 dark: 前缀
<div className="
  bg-white dark:bg-[#131313]
  text-zinc-900 dark:text-zinc-100
  border border-zinc-200 dark:border-zinc-700
">
  <h2 className="text-2xl font-bold text-black dark:text-white">
    标题
  </h2>
  
  <button className="
    bg-blue-500 hover:bg-blue-600 
    dark:bg-blue-600 dark:hover:bg-blue-700
    text-white px-4 py-2 rounded
  ">
    按钮
  </button>
</div>
```

### 8. 配色系统

#### 亮色主题配色
```typescript
const lightTheme = {
  background: '#ffffff',
  backgroundSecondary: '#f5f5f5',
  text: '#000000',
  textSecondary: '#404040',
  border: '#e5e5e5',
  accent: '#0066cc',
  success: '#16a34a',
  error: '#dc2626',
  warning: '#ea580c',
}
```

#### 暗色主题配色 (默认)
```typescript
const darkTheme = {
  background: '#000000',
  backgroundSecondary: '#1a1a1a',
  text: '#ffffff',
  textSecondary: '#a0a0a0',
  border: '#404040',
  accent: '#3b82f6',
  success: '#22c55e',
  error: '#ef4444',
  warning: '#f97316',
}
```

### 9. 最佳实践

#### 避免主题闪烁
在 `index.html` 中添加脚本,在页面加载前应用主题:

```html
<!-- public/index.html -->
<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <script>
    // 防止主题闪烁 (FOUC - Flash of Unstyled Content)
    (function() {
      const theme = localStorage.getItem('theme') || 'dark'
      const systemDark = window.matchMedia('(prefers-color-scheme: dark)').matches
      const applied = theme === 'auto' ? (systemDark ? 'dark' : 'light') : theme
      document.documentElement.classList.add(applied)
    })()
  </script>
</head>
<body>
  <div id="root"></div>
</body>
</html>
```

#### 图片适配暗色主题
```typescript
// 为暗色模式提供不同的图片
<img 
  src={theme === 'dark' ? '/logo-dark.svg' : '/logo-light.svg'}
  alt="Logo"
  className="w-32 h-auto"
/>

// 或使用 CSS filter
<img 
  src="/logo.svg"
  alt="Logo"
  className="dark:invert" // 暗色模式下反色
/>
```

---

## 五、项目结构规范

### 推荐的文件夹结构

```
src/
├── adapters/              # API 适配器层
│   ├── openai/
│   │   ├── OpenAIAdapter.ts
│   │   └── types.ts
│   ├── stability/
│   │   ├── StabilityAdapter.ts
│   │   └── types.ts
│   ├── base/
│   │   └── BaseAdapter.ts    # 适配器基类
│   └── index.ts
│
├── components/            # UI 组件
│   ├── ui/               # shadcn/ui 组件(自动生成)
│   │   ├── button.tsx
│   │   ├── card.tsx
│   │   └── ...
│   ├── common/           # 通用组件
│   │   ├── Header.tsx
│   │   ├── Footer.tsx
│   │   ├── Loading.tsx
│   │   ├── ThemeSwitcher.tsx    # 主题切换
│   │   └── LanguageSwitcher.tsx # 语言切换
│   └── features/         # 功能组件
│       ├── ImageGenerator/
│       │   ├── index.tsx
│       │   ├── ConfigPanel.tsx
│       │   └── ResultDisplay.tsx
│       └── VideoGenerator/
│
├── hooks/                # 自定义 Hooks
│   ├── useApiCall.ts
│   ├── useImageGeneration.ts
│   ├── useTaskPolling.ts
│   ├── useMediaQuery.ts     # 响应式媒体查询
│   └── useTheme.ts          # 主题管理
│
├── stores/               # 状态管理
│   ├── apiStore.ts
│   ├── userStore.ts
│   └── historyStore.ts
│
├── contexts/             # React Context
│   ├── ThemeContext.tsx     # 主题上下文
│   └── index.ts
│
├── locales/              # 多语言翻译文件
│   ├── zh-CN/
│   ├── en-US/
│   └── index.ts
│
├── services/             # 业务逻辑层
│   ├── api.ts           # Axios 实例配置
│   ├── storage.ts       # 本地存储
│   └── validation.ts    # 数据验证
│
├── types/                # 类型定义
│   ├── api.ts
│   ├── models.ts
│   └── index.ts
│
├── utils/                # 工具函数
│   ├── formatters.ts
│   ├── validators.ts
│   └── constants.ts
│
├── config/               # 配置文件
│   ├── providers.json   # API 厂商配置
│   └── app.config.ts
│
├── App.tsx
├── main.tsx
└── index.css
```

---

## 三、编码规范

### 1. 命名规范

#### 文件命名
- **组件文件**: PascalCase (例: `ImageGenerator.tsx`)
- **Hooks**: camelCase,以 use 开头 (例: `useApiCall.ts`)
- **工具函数**: camelCase (例: `formatDate.ts`)
- **类型文件**: camelCase (例: `api.types.ts`)
- **常量文件**: camelCase (例: `constants.ts`)

#### 变量命名
```typescript
// 组件命名 - PascalCase
const ImageGenerator: React.FC = () => {}

// 变量/函数 - camelCase
const apiKey = 'xxx'
const fetchUserData = () => {}

// 常量 - UPPER_SNAKE_CASE
const API_BASE_URL = 'https://api.example.com'
const MAX_RETRY_COUNT = 3

// 类型/接口 - PascalCase,接口加 I 前缀(可选)
interface GenerateParams {}
type ApiResponse = {}
```

### 2. TypeScript 规范

#### 严格类型检查
```typescript
// tsconfig.json
{
  "compilerOptions": {
    "strict": true,
    "noImplicitAny": true,
    "strictNullChecks": true,
    "noUnusedLocals": true,
    "noUnusedParameters": true
  }
}
```

#### 类型定义
```typescript
// ✅ 推荐: 明确类型
interface GenerateImageParams {
  prompt: string
  model: string
  size?: '512x512' | '1024x1024'
  quality?: 'standard' | 'hd'
}

// ❌ 避免: any 类型
const handleSubmit = (data: any) => {} // 不推荐

// ✅ 使用泛型
const fetchData = async <T>(url: string): Promise<T> => {
  const response = await axios.get<T>(url)
  return response.data
}
```

### 3. 组件开发规范

#### 函数组件优先
```typescript
// ✅ 推荐: 函数组件 + TypeScript
interface ButtonProps {
  variant?: 'primary' | 'secondary'
  children: React.ReactNode
  onClick?: () => void
}

const Button: React.FC<ButtonProps> = ({ 
  variant = 'primary', 
  children,
  onClick 
}) => {
  return (
    <button 
      className={`btn btn-${variant}`}
      onClick={onClick}
    >
      {children}
    </button>
  )
}

export default Button
```

#### 单一职责原则
```typescript
// ✅ 每个组件只负责一个功能
const ConfigPanel: React.FC = () => {
  // 只处理配置面板逻辑
}

const ResultDisplay: React.FC = () => {
  // 只处理结果显示逻辑
}

// ❌ 避免: 一个组件做太多事
const ImageGenerator: React.FC = () => {
  // 配置 + 生成 + 显示 + 历史记录... (太复杂)
}
```

#### Props 解构与默认值
```typescript
// ✅ 推荐
const Card: React.FC<CardProps> = ({ 
  title, 
  content, 
  isLoading = false 
}) => {
  // ...
}

// ❌ 避免
const Card: React.FC<CardProps> = (props) => {
  return <div>{props.title}</div>
}
```

### 4. Hooks 使用规范

#### 自定义 Hook 封装
```typescript
// hooks/useImageGeneration.ts
interface UseImageGenerationResult {
  generate: (params: GenerateParams) => Promise<void>
  isLoading: boolean
  error: Error | null
  result: ImageResult | null
}

export const useImageGeneration = (): UseImageGenerationResult => {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<Error | null>(null)
  const [result, setResult] = useState<ImageResult | null>(null)

  const generate = async (params: GenerateParams) => {
    setIsLoading(true)
    setError(null)
    try {
      const data = await apiService.generateImage(params)
      setResult(data)
    } catch (err) {
      setError(err as Error)
    } finally {
      setIsLoading(false)
    }
  }

  return { generate, isLoading, error, result }
}
```

#### 依赖数组规范
```typescript
// ✅ 明确列出所有依赖
useEffect(() => {
  fetchData(userId, filter)
}, [userId, filter]) // 列出所有使用的外部变量

// ❌ 避免空依赖数组(除非确实只需运行一次)
useEffect(() => {
  fetchData(userId) // userId 变化不会重新运行
}, [])
```

### 5. API 适配器模式

#### 基础适配器接口
```typescript
// adapters/base/BaseAdapter.ts
export interface MediaGeneratorAdapter {
  name: string
  generateImage(params: GenerateImageParams): Promise<ImageResult>
  generateVideo(params: GenerateVideoParams): Promise<VideoResult>
  checkStatus(taskId: string): Promise<TaskStatus>
}

// adapters/openai/OpenAIAdapter.ts
export class OpenAIAdapter implements MediaGeneratorAdapter {
  name = 'OpenAI'
  
  async generateImage(params: GenerateImageParams): Promise<ImageResult> {
    // 实现 OpenAI 特定逻辑
    const response = await axios.post('/api/openai/images', {
      prompt: params.prompt,
      model: params.model
    })
    return this.transformResponse(response.data)
  }
  
  private transformResponse(data: any): ImageResult {
    // 转换为统一格式
    return {
      id: data.id,
      url: data.url,
      createdAt: new Date(data.created * 1000)
    }
  }
}
```

#### 适配器工厂
```typescript
// adapters/index.ts
type AdapterType = 'openai' | 'stability' | 'midjourney'

export const getAdapter = (type: AdapterType): MediaGeneratorAdapter => {
  switch (type) {
    case 'openai':
      return new OpenAIAdapter()
    case 'stability':
      return new StabilityAdapter()
    case 'midjourney':
      return new MidjourneyAdapter()
    default:
      throw new Error(`Unknown adapter type: ${type}`)
  }
}
```

### 6. 样式规范

#### Tailwind CSS 使用
```typescript
// ✅ 推荐: 使用 Tailwind 工具类
<div className="flex items-center justify-between p-4 bg-white rounded-lg shadow-md">
  <h2 className="text-xl font-bold text-zinc-900">标题</h2>
</div>

// ✅ 条件类名使用 clsx
import { clsx } from 'clsx'

const Button = ({ variant, isActive }) => (
  <button className={clsx(
    'px-4 py-2 rounded-md',
    variant === 'primary' && 'bg-blue-500 text-white',
    variant === 'secondary' && 'bg-zinc-200 text-zinc-800',
    isActive && 'ring-2 ring-blue-400'
  )}>
    Click me
  </button>
)

// ❌ 避免: 内联样式(除非动态值)
<div style={{ backgroundColor: 'white' }}> // 不推荐
```

#### 黑白配色系统
```typescript
// config/theme.ts
export const theme = {
  colors: {
    primary: {
      black: '#000000',
      darkzinc: '#1a1a1a',
      zinc: '#404040',
      lightzinc: '#a0a0a0',
      white: '#ffffff',
    },
    accent: {
      blue: '#0066cc', // 可选的强调色
    },
    state: {
      error: '#dc2626',
      success: '#16a34a',
      warning: '#ea580c',
    }
  }
}

// tailwind.config.js
export default {
  theme: {
    extend: {
      colors: {
        'primary-black': '#000000',
        'primary-white': '#ffffff',
        'zinc': {
          50: '#fafafa',
          100: '#f5f5f5',
          200: '#e5e5e5',
          300: '#d4d4d4',
          400: '#a3a3a3',
          500: '#737373',
          600: '#525252',
          700: '#404040',
          800: '#262626',
          900: '#171717',
        }
      }
    }
  }
}
```

### 7. 错误处理规范

#### 统一错误处理
```typescript
// services/api.ts
import axios, { AxiosError } from 'axios'

export class ApiError extends Error {
  constructor(
    message: string,
    public statusCode: number,
    public data?: any
  ) {
    super(message)
    this.name = 'ApiError'
  }
}

// Axios 拦截器
axios.interceptors.response.use(
  response => response,
  (error: AxiosError) => {
    if (error.response) {
      throw new ApiError(
        error.response.data?.message || 'Request failed',
        error.response.status,
        error.response.data
      )
    }
    throw new Error('Network error')
  }
)

// 组件中使用
const Component = () => {
  const handleSubmit = async () => {
    try {
      await apiService.generate(params)
    } catch (error) {
      if (error instanceof ApiError) {
        toast.error(`API Error: ${error.message}`)
      } else {
        toast.error('An unexpected error occurred')
      }
    }
  }
}
```

### 8. 性能优化规范

#### React.memo 使用
```typescript
// ✅ 对昂贵的纯组件使用 memo
export const ExpensiveComponent = React.memo<Props>(({ data }) => {
  // 复杂渲染逻辑
  return <div>{/* ... */}</div>
})

// 自定义比较函数
export const ImageCard = React.memo<ImageCardProps>(
  ({ image }) => <div>{/* ... */}</div>,
  (prevProps, nextProps) => prevProps.image.id === nextProps.image.id
)
```

#### useMemo 和 useCallback
```typescript
// ✅ 缓存计算结果
const ExpensiveComponent = ({ items, filter }) => {
  const filteredItems = useMemo(() => {
    return items.filter(item => item.category === filter)
  }, [items, filter])

  return <List items={filteredItems} />
}

// ✅ 缓存回调函数
const Parent = () => {
  const handleClick = useCallback(() => {
    console.log('clicked')
  }, []) // 空依赖,函数永远不变

  return <Child onClick={handleClick} />
}
```

---

## 四、配置文件规范

### API 厂商配置
```json
// config/providers.json
{
  "providers": [
    {
      "id": "openai",
      "name": "OpenAI DALL-E",
      "type": "image",
      "endpoint": "https://api.openai.com/v1",
      "models": ["dall-e-3", "dall-e-2"],
      "requiresAuth": true,
      "rateLimit": {
        "requests": 50,
        "period": "minute"
      }
    },
    {
      "id": "stability",
      "name": "Stability AI",
      "type": "image",
      "endpoint": "https://api.stability.ai/v1",
      "models": ["stable-diffusion-xl", "stable-diffusion-v1-6"],
      "requiresAuth": true
    }
  ]
}
```

### 环境变量
```bash
# .env.example
VITE_APP_TITLE=AI Media Generator
VITE_API_TIMEOUT=30000

# API Keys (用户自行配置,不提交)
VITE_OPENAI_API_KEY=
VITE_STABILITY_API_KEY=
VITE_MIDJOURNEY_API_KEY=
```

---

## 五、代码质量工具

### ESLint 配置
```json
// .eslintrc.json
{
  "extends": [
    "eslint:recommended",
    "plugin:react/recommended",
    "plugin:react-hooks/recommended",
    "plugin:@typescript-eslint/recommended"
  ],
  "rules": {
    "react/react-in-jsx-scope": "off",
    "@typescript-eslint/no-unused-vars": "warn",
    "@typescript-eslint/no-explicit-any": "error"
  }
}
```

### Prettier 配置
```json
// .prettierrc
{
  "semi": false,
  "singleQuote": true,
  "tabWidth": 2,
  "trailingComma": "es5",
  "printWidth": 80,
  "arrowParens": "avoid"
}
```

### Husky + lint-staged
```json
// package.json
{
  "lint-staged": {
    "*.{ts,tsx}": [
      "eslint --fix",
      "prettier --write"
    ]
  }
}
```

---

## 六、Git 提交规范

### Commit Message 格式
```
<type>(<scope>): <subject>

<body>

<footer>
```

**Type 类型:**
- `feat`: 新功能
- `fix`: 修复 bug
- `docs`: 文档更新
- `style`: 代码格式调整(不影响功能)
- `refactor`: 重构
- `perf`: 性能优化
- `test`: 测试相关
- `chore`: 构建/工具配置

**示例:**
```
feat(image-gen): 添加 OpenAI DALL-E 3 支持

- 实现 OpenAIAdapter 适配器
- 添加配置面板 UI
- 支持自定义分辨率选择

Closes #123
```

---

## 七、开发流程

### 1. 分支管理
- `main` - 主分支,生产环境
- `develop` - 开发分支
- `feature/*` - 功能分支
- `bugfix/*` - 修复分支

### 2. 开发步骤
1. 从 `develop` 创建 `feature/xxx` 分支
2. 开发并本地测试
3. 提交代码,通过 ESLint 和 Prettier 检查
4. 发起 Pull Request 到 `develop`
5. Code Review 后合并
6. 定期从 `develop` 合并到 `main`

### 3. Code Review 检查清单
- [ ] 代码符合 TypeScript 规范
- [ ] 组件拆分合理,遵循单一职责
- [ ] 正确使用 Hooks,无闭包陷阱
- [ ] 错误处理完善
- [ ] 性能优化(memo/useMemo/useCallback)
- [ ] UI 符合设计规范(黑白配色)
- [ ] 无 console.log 等调试代码

---

## 八、测试规范(可选)

### 单元测试
```typescript
// __tests__/Button.test.tsx
import { render, screen, fireEvent } from '@testing-library/react'
import { Button } from '@/components/ui/button'

describe('Button', () => {
  it('renders correctly', () => {
    render(<Button>Click me</Button>)
    expect(screen.getByText('Click me')).toBeInTheDocument()
  })

  it('handles click events', () => {
    const handleClick = jest.fn()
    render(<Button onClick={handleClick}>Click me</Button>)
    fireEvent.click(screen.getByText('Click me'))
    expect(handleClick).toHaveBeenCalledTimes(1)
  })
})
```

---

## 九、部署规范

### 构建优化
```typescript
// vite.config.ts
export default defineConfig({
  build: {
    rollupOptions: {
      output: {
        manualChunks: {
          'react-vendor': ['react', 'react-dom'],
          'ui-vendor': ['@radix-ui/react-dialog', 'lucide-react'],
        }
      }
    }
  }
})
```

### 环境区分
- **开发环境**: `npm run dev`
- **预览环境**: `npm run build && npm run preview`
- **生产环境**: CI/CD 自动部署

---

## 十、文档规范

### README.md 必备内容
1. 项目简介
2. 技术栈
3. 安装步骤
4. 配置说明(API Key 等)
5. 开发指南
6. 部署说明
7. 常见问题

### 组件文档
```typescript
/**
 * ImageGenerator 组件
 * 
 * @description 图片生成主组件,支持多个 AI 厂商
 * 
 * @example
 * ```tsx
 * <ImageGenerator 
 *   provider="openai"
 *   onSuccess={(result) => console.log(result)}
 * />
 * ```
 */
export const ImageGenerator: React.FC<Props> = ({ ... }) => {
  // ...
}
```