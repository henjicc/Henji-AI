...
voice_id
string
请求的音色编号。与 timbre_weights 二选一”必填”。
...
**pronunciation_dict**: 隐藏 properties
...
int
范围[1,100]
权重，须与 voice_id 同步填写。最多支持 4 种音色混合，取值为整数，单一音色取值占比越高，合成音色越像。


**stream**: 是否流式。默认 false，即不开启流式。


**stream_options**: 隐藏 properties
...
**voice_modify**: 声音效果器设置，该参数支持的音频格式：
非流式：mp3, wav, flac
流式：mp3
...
```bash
curl --request POST \
  --url https://api.ppinfra.com/v3/minimax-speech-2.6-turbo \
