...
2. **`nodeRegistry.ts`**：加一个 `CanvasNodeDefinition`，参考 `imageEditNodeDefinition`（约第 159 行起）。关键字段取值见 [references/canvas/node-registry-fields.md](node-registry-fields.md)，别凭空猜字段含义。
...
适用于"有独有交互的生成节点"，比如分镜生成节点（格子编辑器）。完整真实案例和踩坑点见 [references/canvas/special-node-pattern.md](special-node-pattern.md)，这里只给装配公式。
...
- [ ] `nodeRegistry.ts` 的 `CanvasNodeDefinition` 字段填全，对照 [references/canvas/node-registry-fields.md](node-registry-fields.md)
