...
| 模态 | 视频 |
| 供应商 | fal.ai（聚合平台） |
| 平台模型 ID | `bytedance/seedance-2.0/mini/text-to-video`、`bytedance/seedance-2.0/mini/image-to-video`、`bytedance/seedance-2.0/mini/reference-to-video` |
| 接口形态 | 队列异步（推荐）或同步直连 |
| 文档可见性 | 公开，无需登录 |
| 价格可见性 | 公开，无需登录 |
...
| 能力 | endpoint id |
|---|---|
| 文生视频 | `bytedance/seedance-2.0/mini/text-to-video` |
| 图生视频（首帧，可选尾帧） | `bytedance/seedance-2.0/mini/image-to-video` |
| 参考生视频（图 / 视频 / 音频多模态） | `bytedance/seedance-2.0/mini/reference-to-video` |
...
| `resolution` | string | 可选 | `720p` | **仅 `480p` / `720p`** |
...
| 480p（按秒，约） | **$0.0721 / 秒** |
| 720p（按秒，约） | **$0.1547 / 秒** |
| token 价（480p / 720p 相同） | **$0.007 / 1000 tokens** |

token 数计算：`(输出视频高 × 输出视频宽 × 时长 × 24) / 1024`。

`reference-to-video` 有视频输入时：**视频输入按所选分辨率单价的 0.6 倍计费**（480p 约 $0.0433/秒、720p 约 $0.0928/秒），**输入与输出都按输出分辨率的费率计费**；token 数按 `(高 × 宽 × (输入视频时长 + 输出视频时长) × 24) / 1024`。
...
- `reference-to-video` 的 prompt 里要用 `@Image1` / `@Video1` / `@Audio1` 显式引用素材，这是 Fal 独有的约定。
...
| 信息 | 链接 | 是否需登录 |
|---|---|---|
| 文生视频 schema + 价格 | https://fal.ai/models/bytedance/seedance-2.0/mini/text-to-video/llms.txt | 否 |
| 图生视频 schema + 价格 | https://fal.ai/models/bytedance/seedance-2.0/mini/image-to-video/llms.txt | 否 |
| 参考生视频 schema + 价格 | https://fal.ai/models/bytedance/seedance-2.0/mini/reference-to-video/llms.txt | 否 |
