...
- 2026-09-24 复核结束语义、合法空帧、重复最终快照、用量、错误、截断及取消；详见 [三模型共享事件契约与回归矩阵](../Qwen-MT-Flash/Qwen-MT-Flash_百炼.md#三模型共享事件契约与回归矩阵2026-09-24)。Plus 仍按累积快照计算变化；只有 `stop` 才交付完整 item，`length` 明确报告截断。
