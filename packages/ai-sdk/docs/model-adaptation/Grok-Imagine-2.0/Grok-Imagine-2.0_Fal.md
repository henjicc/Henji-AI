...
来源：两个端点的 `llms.txt`（2026-08-30 重新核对）。
...
- 编辑输入图加价必须按三个运行场景中的最大有效图片数量估算：生成提交的 `uploadedFilePaths`、画布的 `images` 与对话面板的 `uploadedImages`；空字符串和无效项不计数，较短的旧路径数组不能压住较新的完整图片列表。
