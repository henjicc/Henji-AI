...
**公开来源曾经不一致；2026-08-30 重新核验后，模型专属页的“定价详情”和总定价中心已统一为下列当前生效价。API 文档正文仍是旧价。**

来源 A —— [APIMart 定价中心](https://apimart.ai/zh/pricing)与 [Seedream 5.0 Pro 模型专属页](https://apimart.ai/zh/model/seedream-5-0-pro)（2026-08-30 读取，图像标签页，1 Credit ≈ $0.1）：

| 规格 | 价格 |
|---|---|
| 默认 | 0.36 Credits/张 ≈ **$0.036/张** |
| 1K / 1.5K | 0.2925 Credits/张 ≈ **$0.02925/张** |
| 2K | 0.585 Credits/张 ≈ **$0.0585/张** |
| 第 2 张起的参考图片 | 0.0195 Credits/张 ≈ **$0.00195/张**（第 1 张免费） |
| 1K / 1.5K-layer（图层拆分） | 0.14625 Credits/输出图层 ≈ **$0.014625/输出图层** |
| 2K / auto-layer（图层拆分） | 0.2925 Credits/输出图层 ≈ **$0.02925/输出图层** |
...
共同规则：1.5K 与 1K 同价；`size` 为精确像素时按**实际输出面积**判档，`resolution` 不影响；**第 1 张参考图免费，第 2 张起每张 $0.00195**；任务失败自动全额退款。

**图层拆分的预扣与结算**：提交时按 **17 张**保守预扣（`1K`/`1.5K` 按 1K 档，`2K` 与 `auto` 按 2K 档），账户余额必须能覆盖 17 张预扣；完成后按底图与每个实际图层的真实像素面积**逐张判档求和**结算，多扣自动退回。SDK 本地计算器无法在结果返回前知道总图层数，因此图层模式返回“暂不可估算”（`NaN`），不能把一个图层的单位价冒充最终总价；上表单位价仅用于解释实际账单。
...
- 普通生成 / 编辑估价需要把输出价与参考图加价相加：`输出价 + max(0, 参考图数量 - 1) × $0.00195`。图层拆分在生成前返回“暂不可估算”，最终总价以后端实际输出图层数与结算为准。
- Pro 是单图模型，UI 不要出现数量选择；比例集合必须严格取平台支持列表，越界会 400 而不是回退。
- SDK 已补齐合法的 `2:1` / `1:2`，并对 `4:5` / `5:4` 等列表外比例显式报错；旧 Photoshop 插件不能继续把 COMMON10 原样用于本模型。
...
| Seedream 5.0 Pro 模型专属页（当前定价详情与参考图加价） | https://apimart.ai/zh/model/seedream-5-0-pro | 否 |
