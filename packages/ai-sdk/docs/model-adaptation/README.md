...
另有 12 个按能力按需分发的 Fal 图像工具，不进入默认 111 模型兼容目录：
...
工具均复用 Fal 队列、上传与结果解析；它们是独立的按需 SDK 入口，不是新的执行内核。Henji-AI 运行客户端显式装载三个 pack，但普通模型选择器仍固定为 111 个默认模型：六个图片实用工具通过图片能力菜单创建固定模型节点，三个多角度工具由专用多角度节点选择，消除工具保留给后续遮罩能力接入。真实质量、延迟和账单仍待获得付费授权后验证。

## 2026-09-24 已适配更新

- [HappyHorse 1.1 · KIE](HappyHorse-1.1/HappyHorse-1.1_KIE.md)：文生、图生与多图参考。
- [Seedream 5.0 Flash · 火山方舟](Seedream-5.0-Flash/Seedream-5.0-Flash_火山引擎.md)：生成、编辑与图层拆分。
- [GLM Flash / FlashX](GLM-5.3-Flash/GLM-5.3-Flash_智谱.md)：新增 FlashX 与 JSON 契约。
- [百炼 ASR](供应商/百炼.md)：新增 Qwen-Audio 3.1 三种语音识别协议预设。
- [MiMo 2.6 与 Responses](../llm-adaptation/供应商/小米MiMo.md)、[Kimi 缓存价格](../llm-adaptation/供应商/Kimi.md)。
...
5. 价格依赖本地媒体像素、时长或文件指标时，由模型在 `pricing.mediaContext` 声明需求，宿主统一读取并注入 calculator；禁止在单个页面或画布节点复制媒体计价逻辑。指标未解析完成时不显示伪造的兜底总价。
...
| Topaz 透明图放大 | 图片 | Fal | [Topaz-透明图放大_Fal.md](Topaz-透明图放大/Topaz-透明图放大_Fal.md) |
| SeedVR2 图片放大 | 图片 | Fal | [SeedVR2-图片放大_Fal.md](SeedVR2-图片放大/SeedVR2-图片放大_Fal.md) |
| Bria 创意放大 | 图片 | Fal | [Bria-创意放大_Fal.md](Bria-创意放大/Bria-创意放大_Fal.md) |
| Ideogram 图片放大 | 图片 | Fal | [Ideogram-图片放大_Fal.md](Ideogram-图片放大/Ideogram-图片放大_Fal.md) |
...
| Topaz 图片放大 | — | — | `topaz/upscale/image/{precision,creative,generative}` |
| Topaz 透明图放大 | — | — | `topaz/upscale/image/transparent` |
| SeedVR2 图片放大 | — | — | `fal-ai/seedvr/upscale/image` |
| Bria 创意放大 | — | — | `bria/upscale/creative` |
| Ideogram 图片放大 | — | — | `fal-ai/ideogram/upscale` |
