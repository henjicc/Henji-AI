...
| 模态 | 视频 |
| 供应商 | KIE.ai（聚合平台） |
| 平台模型 ID | `bytedance/seedance-2-fast` |
...
- **提交**：`POST /api/v1/jobs/createTask`，体为 `{ model: "bytedance/seedance-2-fast", callBackUrl?, input }`
- **查询**：`GET /api/v1/jobs/recordInfo?taskId=...`
- **终态**：`state` ∈ `waiting` / `queuing` / `generating` / `success` / `fail`
- **结果**：`JSON.parse(resultJson)` → `{ resultUrls: [...] }`；**开启 `return_last_frame` 时结构为 `{ resultUrls: [], firstFrameUrl: [], lastFrameUrl: [] }`**
...
`state=success` 后 `JSON.parse(resultJson)` → `{ "resultUrls": ["https://....mp4"] }`；开启 `return_last_frame` 时为 `{ "resultUrls": [], "firstFrameUrl": [], "lastFrameUrl": [] }`。
...
|---|---|---|---|---|
| 480p no video input | 11.7 /秒 | **$0.059/秒** | $0.1125 | 47.6% |
| 480p with video input | 6.8 /秒 | **$0.034/秒** | $0.0675 | 49.6% |
| 720p no video input | 24.8 /秒 | **$0.124/秒** | $0.2419 | 48.7% |
| 720p with video input | 15 /秒 | **$0.075/秒** | $0.1451 | 48.3% |
...
| 信息 | 链接 | 是否需登录 |
|---|---|---|
| Seedance 2.0 Fast 文档 | https://docs.kie.ai/cn/market/bytedance/seedance-2-fast | 否 |
