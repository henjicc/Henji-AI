...
`packages/ai-sdk/src/catalog/ppio/kling-3.0.model.ts` 的 `endpoints.selector` 用模板拼路由——按分辨率档位映射 `std`/`pro`/`4k`，按是否上传图片映射 `t2v`/`i2v`，动作控制单独返回固定路由。源码里只有两条字面量路由字符串，容易被 grep 误判为"只接了 2 条"，实际七条都能命中。

`packages/ai-sdk/tests/catalog/ppio.test.ts` 直接执行 catalog 的真实 selector，逐条断言 7 种组合的落点。
