...
async function verify() {
  ...
  fs.writeFileSync(path.join(consumerRoot, 'entry.js'), [
    ...
    ...['embedding', 'rerank'].flatMap(kind => [
      `import '@henjicc/ai-sdk/capabilities/${kind}'`,
      ...['siliconflow', 'bailian', 'ppio', 'bigmodel', ...(kind === 'embedding' ? ['volcengine'] : [])].map(provider => `import '@henjicc/ai-sdk/capabilities/${kind}/${provider}'`),
    ]),
    ...
    "import '@henjicc/ai-sdk/capabilities/speech-recognition/volcengine'",
    "import '@henjicc/ai-sdk/capabilities/speech-recognition/volcengine/realtime'",
    "import '@henjicc/ai-sdk/capabilities/speech-recognition/siliconflow'",
    "import '@henjicc/ai-sdk/capabilities/speech-recognition/groq'",
    ...
    "import '@henjicc/ai-sdk/tool-models/fal/flux-2-multiple-angles'",
    "import '@henjicc/ai-sdk/tool-packs/fal-image-edit-tools'",
    "import '@henjicc/ai-sdk/tool-packs/fal-image-utility-tools'",
    "import '@henjicc/ai-sdk/tool-packs/fal-multi-angle-tools'",
  ...
  fs.writeFileSync(path.join(consumerRoot, 'consumer.ts'), [
    ...
    "import type { EmbeddingInput, EmbeddingOutput } from '@henjicc/ai-sdk/capabilities/embedding'",
    "import type { RerankInput, RerankOutput } from '@henjicc/ai-sdk/capabilities/rerank'",
    "const typedEmbedding: CapabilityModule<EmbeddingInput, EmbeddingOutput> = createSiliconflowEmbeddingModule()",
    "const typedRerank: CapabilityModule<RerankInput, RerankOutput> = createSiliconflowRerankModule()",
    ...
    "import { createVolcengineAsrModule, volcengineSeedAsrFile } from '@henjicc/ai-sdk/capabilities/speech-recognition/volcengine'",
    "import { createVolcengineRealtimeAsrModule, volcengineSeedAsrRealtime } from '@henjicc/ai-sdk/capabilities/speech-recognition/volcengine/realtime'",
    "import { createSiliconFlowAsrModule, siliconFlowSenseVoiceSmall } from '@henjicc/ai-sdk/capabilities/speech-recognition/siliconflow'",
    "import { createGroqAsrModule, groqWhisperLargeV3Turbo } from '@henjicc/ai-sdk/capabilities/speech-recognition/groq'",
    ...
    "import { model as falFluxMultipleAnglesModel, pack as falFluxMultipleAnglesPack } from '@henjicc/ai-sdk/tool-models/fal/flux-2-multiple-angles'",
    ...
    "import { pack as falMultiAngleTools } from '@henjicc/ai-sdk/tool-packs/fal-multi-angle-tools'",
    ...
    '',
    'const modular = createModularGenerationClient({ runtime, packs: [zImagePack, falRelightingPack, falFluxMultipleAnglesPack] })',
    "modular.catalog.get('kie/z-image')",
    'modular.catalog.get(falRelightingModel.meta.id)',
    'modular.catalog.get(falFluxMultipleAnglesModel.meta.id)',
    ...
    'const multiAngleDiscovery = createModelCapabilityDiscovery({ generationPacks: [falMultiAngleTools] })',
    "void multiAngleDiscovery.search({ providerIds: 'fal', outputModalities: 'image' })",
    '',
    ...
    "  execute: async () => ({ output: 'ok', reasoningOutput: '', usage: null, finishReason: 'stop', truncated: false }),",
    '}',
    ...
    "const setup: LlmProviderSetup = normalizeLlmProviderSetup({ kind: 'custom', apiKeyManagementUrl: 'https://keys.example.com/manage' })",
    "const builtIn = createProviderFromPreset(findLlmProviderPreset('ppio')!, { lifecycle: 'builtin' })",
    "if (setup.kind !== 'custom' || builtIn.setup?.kind !== 'preset' || resolveLlmProviderApiKeyUrl(builtIn) === null) throw new Error('provider setup contract unavailable')",
    ...
    'void createVolcengineAsrModule(volcengineSeedAsrFile)',
    'void createVolcengineRealtimeAsrModule(volcengineSeedAsrRealtime)',
    'void createSiliconFlowAsrModule(siliconFlowSenseVoiceSmall)',
    'void createGroqAsrModule(groqWhisperLargeV3Turbo)',
  ...
  fs.writeFileSync(path.join(consumerRoot, 'runtime-probe.mjs'), [
    ...
    "import { model as falFluxMultipleAnglesModel } from '@henjicc/ai-sdk/tool-models/fal/flux-2-multiple-angles'",
    "import { pack as falImageUtilityTools } from '@henjicc/ai-sdk/tool-packs/fal-image-utility-tools'",
    "import { pack as falMultiAngleTools } from '@henjicc/ai-sdk/tool-packs/fal-multi-angle-tools'",
    ...
    "import { volcengineFileAsrPresets, createVolcengineAsrModule } from '@henjicc/ai-sdk/capabilities/speech-recognition/volcengine'",
    "import { volcengineRealtimeAsrPresets, createVolcengineRealtimeAsrModule } from '@henjicc/ai-sdk/capabilities/speech-recognition/volcengine/realtime'",
    "import { siliconFlowAsrPresets, createSiliconFlowAsrModule } from '@henjicc/ai-sdk/capabilities/speech-recognition/siliconflow'",
    "import { groqAsrPresets, createGroqAsrModule } from '@henjicc/ai-sdk/capabilities/speech-recognition/groq'",
    "import { BAILIAN_QWEN_MT_PRESETS, createBailianQwenMtTranslationModule } from '@henjicc/ai-sdk/capabilities/translation/bailian'",
    "import { createProviderFromPreset, findLlmProviderPreset, normalizeLlmProviderSetup, resolveLlmProviderApiKeyUrl } from '@henjicc/ai-sdk/llm'",
    ...
    '  ...volcengineFileAsrPresets.map(createVolcengineAsrModule),',
    '  ...volcengineRealtimeAsrPresets.map(createVolcengineRealtimeAsrModule),',
    '  ...siliconFlowAsrPresets.map(createSiliconFlowAsrModule),',
    '  ...groqAsrPresets.map(createGroqAsrModule),',
    ...
    'const multiAngleDiscovery = createModelCapabilityDiscovery({ generationPacks: [falMultiAngleTools] })',
    "const multiAngleIds = multiAngleDiscovery.search({ providerIds: 'fal', outputModalities: 'image' }).map((item) => item.id)",
    'if (bailianNonRealtimeAsrPresets.length + volcengineFileAsrPresets.length + siliconFlowAsrPresets.length + groqAsrPresets.length !== 10 || bailianRealtimeAsrPresets.length + volcengineRealtimeAsrPresets.length !== 5 || Object.keys(BAILIAN_QWEN_MT_PRESETS).length !== 3) {',
    "  throw new Error('内置能力数量与发布契约不一致')",
    '}',
    'if (ids.length !== 19 || new Set(ids).size !== ids.length) {',
    ...
    '}',
    'if (multiAngleIds.length !== 3 || new Set(multiAngleIds).size !== 3 || !multiAngleIds.includes(falFluxMultipleAnglesModel.meta.id)) {',
    "  throw new Error(`Fal 多角度工具发布发现不完整：${JSON.stringify(multiAngleIds)}`)",
    '}',
    "const moduleClient = createLlmModuleClient({ runtime: { transport: { fetch: async () => { throw new Error('network forbidden') } }, credentials: { get: async () => undefined }, media: { read: async () => { throw new Error('media forbidden') } } }, modules: [createGroqLlmModule(), { descriptor: { id: 'fixture.external.llm', source: { kind: 'external', namespace: 'com.example.node-probe' }, providerId: 'fixture', modelId: 'fixture-model', capabilities: { text: true, image: false, video: false, audio: false, streaming: false, toolCall: false, parallelTools: false, jsonOutput: false, structuredOutputMode: 'none', reasoning: false, sampling: false, contextWindow: null, maxOutputTokens: null, usage: false }, executionModes: ['request-response'] }, execute: async () => ({ output: 'ok', reasoningOutput: '', usage: null, finishReason: 'stop', truncated: false }) }] })",
    "let groqConflict = false",
    "try { moduleClient.register({ descriptor: { ...createGroqLlmModule().descriptor, id: 'plugin.shadow.groq', source: { kind: 'plugin', namespace: 'com.example.shadow' } }, execute: async () => ({ output: '', reasoningOutput: '', usage: null, finishReason: null, truncated: false }) }) } catch (error) { groqConflict = String(error).includes('com.example.shadow') && String(error).includes('@henjicc/ai-sdk') }",
    ...
    "console.log(JSON.stringify({ package: '@henjicc/ai-sdk', nodeRuntime: true, asr: 10, realtime: 5, translation: 3, falImageUtilities: utilityIds.length, falMultiAngleTools: multiAngleIds.length, groq: 1, externalLlm: 1, discovery: ids.length }))",
  ].join('\n'))
  ...
  try {
    ...
    const entries = [
      ...
      ...['embedding', 'rerank'].flatMap(kind => [
        `@henjicc/ai-sdk/capabilities/${kind}`,
        ...['siliconflow', 'bailian', 'ppio', 'bigmodel', ...(kind === 'embedding' ? ['volcengine'] : [])].map(provider => `@henjicc/ai-sdk/capabilities/${kind}/${provider}`),
      ]),
      ...
      '@henjicc/ai-sdk/tool-models/fal/flux-2-multiple-angles',
      '@henjicc/ai-sdk/tool-packs/fal-image-edit-tools',
      '@henjicc/ai-sdk/tool-packs/fal-image-utility-tools',
      '@henjicc/ai-sdk/tool-packs/fal-multi-angle-tools',
    ]
    const resolved = {}
