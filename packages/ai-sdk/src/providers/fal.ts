...
async function pollByStatusUrl(input: ProviderContinuePollingInput, statusUrl: string): Promise<JsonValue> {
  return await pollUntilResult(input, async () => {
    const response = await input.runtime.transport.fetch(statusUrl, {
...
function enrichRequestId(value: JsonValue, requestId: string): JsonValue {
  return isJsonObject(value) ? { request_id: normalizeRequestId(requestId), ...value } : value
}

function normalizeRequestId(taskId: string): string {
  if (!taskId.startsWith('http://') && !taskId.startsWith('https://')) return taskId
  try {
    const matched = new URL(taskId).pathname.match(/\/requests\/([^/]+)/)
    return matched?.[1] ? decodeURIComponent(matched[1]) : taskId
  } catch {
    return taskId
  }
}
