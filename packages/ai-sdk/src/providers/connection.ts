...
import { buildApiMartEndpoints, markApiMartEndpointReachable } from './endpoints/apimart'
...
// 详见 packages/ai-sdk/docs/model-adaptation/供应商/Grsai.md §5.2。宁可保持"已保存未校验"，也不要拿一个不确定的契约去误判用户 Key 状态。
...
export async function testProviderConnection(
  ...
  const { transport, credentials, logger } = resolveRuntimeContext(runtimeContext)
  const startedAt = Date.now()
  const apiKey = await credentials.get('generation', providerId)
