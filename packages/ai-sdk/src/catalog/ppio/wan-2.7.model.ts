...
export const wan27Model = defineModel({
  ...
  params: [
    ...
    },
    {
      ...
      options: Object.keys(WAN27_SIZE_MAP).map((value) => ({ value })),
      apiField: 'size'
    },
    {
      ...
      options: [
        { value: 'auto' },
        ...Object.keys(WAN27_EDIT_RATIO_MAP).map((value) => ({ value }))
      ],
      apiField: 'ratio'
    },
    {
  ...
  request: {
    builder: (params) => {
      ...
      const clampDuration = (value: JsonValue, min: number, max: number, fallback: number): number => {
      ...
      const readText = (value: JsonValue, maxLength: number): string => {
      ...
      const requestData: JsonObject = { watermark: false }
