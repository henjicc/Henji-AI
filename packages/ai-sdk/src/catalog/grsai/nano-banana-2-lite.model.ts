/**
 * Grsai Nano Banana 2 Lite 图片生成模型（运行时契约）
 *
 * 与 Nano Banana 2 是两个独立产品模型（canonicalModelId 不同），不是同一模型下的渠道/分辨率选项，
 * 详见 packages/ai-sdk/docs/model-adaptation/Nano-Banana-2-Lite/Nano-Banana-2-Lite_Grsai.md。
 *
 * dashboard 上价格/规格与本模型完全相同的 `nano-banana-fast` 是初代「香蕉」世代的遗留命名，
 * 本项目已明确排除、不适配，不要把它当成本模型的备用渠道加回来
 * （见 packages/ai-sdk/docs/model-adaptation/供应商/Grsai.md 第 7 节）。
 */
...
export const grsaiNanoBanana2LiteModel = defineModel({
  ...
  request: {
    builder: (params) => {
      ...
      const body: JsonObject = { model: 'nano-banana-2-lite', prompt, aspectRatio: ratio }
