import type { ModelStepEvent, ModelStepInput, ModelStepResult } from '../modelStep'
import {
  normalizeProviderError,
  shouldRetry,
  type ProviderModelStepError,
} from '../../runtime'

/** 任务 4.2 从应用侧 LLM 执行层迁入 SDK，逻辑不变。 */
...
export async function executeModelStepWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt += 1) {
    ...
    try {
      ...
      if (!shouldRetry(structured, 'request') || attempt > maxRetries) throw structured
