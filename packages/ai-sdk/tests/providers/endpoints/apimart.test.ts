...
import type { Transport } from '../../../src/runtime'
...
describe('APIMart 端点排序', () => {
  ...
  it('启动预热探测：主域名可达时保持默认顺序', async () => {
    ...
    const transport: Transport = { fetch: fetchMock }

    await warmApiMartEndpointPreference(transport)
  ...
  it('启动预热探测：全部域名都不通时保持默认顺序，静默失败', async () => {
    ...
    await expect(warmApiMartEndpointPreference(transport)).resolves.toBeUndefined()
