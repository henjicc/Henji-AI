...
describe('KIE Seedance 2.0 音频默认值', () => {
  it.each(modelFiles)(
    '%s 默认生成音频，仅在用户明确关闭时禁用',
    (fileName) => {
      const source = readFileSync(resolve(__dirname, '../../src/catalog/kie', fileName), 'utf8')
      ...
      expect(audioParameter?.[1]).toBe('true')
      expect(builderFallback?.[1]).toMatch(/!==\s*false\s*$/)
    }
  )
})
