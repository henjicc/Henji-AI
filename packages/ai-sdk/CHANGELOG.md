...
- 完善聊天与翻译 SSE 的事件边界和完成状态校验：保留合法空中间帧、用量与状态通知，提前结束、服务端错误和无效终态不再伪装成功；保留翻译增量/累积协议差异。
- 修复百炼实时 ASR 字段校验、Qwen 中间文字拼接和状态通知处理；保留 0.6.1 空中间帧兼容，诊断包含实际模型、协议及失败阶段。
- 收口共享实时会话的取消、超时、后台故障通知和资源释放；SDK 会话提供可选 result promise，旧自定义会话仍兼容。握手清理失败保留首个故障，火山服务错误默认脱敏。
- 阻止聊天及火山实时识别在取消后继续派发同一帧的后续结果；明确智谱推理网络错误与上下文截断，扩展通知不冒充完成证据。
- 增加脱敏事件序列回归并执行修复撤销验证，更新聊天示例的合法结束帧。未改变供应商配额、网络稳定性或模型识别质量。
...
- 包含原待发布 0.4.2 网络重试修复；外部消费者从公共 npm 精确安装 0.5.0。
...
- 发布前门禁固定校验 npm 公共 registry、仓库 scope 映射与 `access=public`，并提供显式指定公共源的 `npm run publish:sdk`，避免用户级配置把版本误发回私有源；正式发布仍只由 npm 包维护者手动执行。
