...
SDK `0.1.3` 私有发布在 GitHub Packages。先在**消费项目**的 `.npmrc` 配置：
...
```bash
npm install @henjicc/ai-sdk@0.1.3
```
...
发布产物是保留 175 个模块边界的 ESM + `.d.ts`，没有 CJS `require` 入口。CommonJS 项目请用
