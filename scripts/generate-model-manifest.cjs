...
const CUSTOM_BUILDER_OVERRIDES = {
  'ppio-seedream-4.0': `(params) => { const maxImages = params.maxImages || params.max_images || 1; const prompt = typeof params.prompt === 'string' ? params.prompt : ''; const finalPrompt = maxImages > 1 ? ('生成' + maxImages + '张图片。' + prompt) : prompt; const requestData = { prompt: finalPrompt, watermark: false }; if (params.resolution) { const resolution = params.resolution; if (resolution.width && resolution.height) { requestData.size = resolution.width + 'x' + resolution.height; } else { const quality = resolution.quality === '4K' ? '4K' : '2K'; const target = quality === '4K' ? 16777216 : 4194304; const ratioText = (resolution.aspectRatio === 'smart' || resolution.aspectRatio === 'auto' || !resolution.aspectRatio) ? '1:1' : String(resolution.aspectRatio); const pair = ratioText.split(':').map(Number); const ratio = pair[0] > 0 && pair[1] > 0 ? pair[0] / pair[1] : 1; let h = Math.sqrt(target / ratio); let w = h * ratio; let width = Math.max(15, Math.round(w)); let height = Math.max(15, Math.round(h)); const maxPixels = 16777216; if (width * height > maxPixels) { const scale = Math.sqrt(maxPixels / (width * height)); width = Math.max(15, Math.round(width * scale)); height = Math.max(15, Math.round(height * scale)); } requestData.size = width + 'x' + height; } } else if (params.size) { requestData.size = params.size; } if (Array.isArray(params.images) && params.images.length > 0) { requestData.images = params.images; } if (maxImages > 1) { requestData.sequential_image_generation = 'auto'; requestData.max_images = maxImages; } else { requestData.sequential_image_generation = 'disabled'; } return requestData; }`,
  'ppio-seedream-4.5': `(params) => { const maxImages = params.maxImages || params.max_images || 1; const prompt = typeof params.prompt === 'string' ? params.prompt : ''; const finalPrompt = maxImages > 1 ? ('生成' + maxImages + '张图片。' + prompt) : prompt; const requestData = { prompt: finalPrompt, watermark: false }; if (params.resolution) { const resolution = params.resolution; if (resolution.width && resolution.height) { requestData.size = resolution.width + 'x' + resolution.height; } else { const quality = resolution.quality === '4K' ? '4K' : '2K'; const target = quality === '4K' ? 16777216 : 4194304; const ratioText = (resolution.aspectRatio === 'smart' || resolution.aspectRatio === 'auto' || !resolution.aspectRatio) ? '1:1' : String(resolution.aspectRatio); const pair = ratioText.split(':').map(Number); const ratio = pair[0] > 0 && pair[1] > 0 ? pair[0] / pair[1] : 1; let h = Math.sqrt(target / ratio); let w = h * ratio; let width = Math.max(1920, Math.round(w)); let height = Math.max(1920, Math.round(h)); const maxPixels = 16777216; if (width * height > maxPixels) { const scale = Math.sqrt(maxPixels / (width * height)); width = Math.max(1920, Math.round(width * scale)); height = Math.max(1920, Math.round(height * scale)); } requestData.size = width + 'x' + height; } } else if (params.size) { requestData.size = params.size; } if (Array.isArray(params.images) && params.images.length > 0) { requestData.image = params.images; } if (maxImages > 1) { requestData.sequential_image_generation = 'auto'; requestData.sequential_image_generation_options = { max_images: maxImages }; } else { requestData.sequential_image_generation = 'disabled'; } if (params.optimizePrompt === true) { requestData.optimize_prompt_options = { mode: 'standard' }; } return requestData; }`,
