...
- 模型存在产品级渠道切换时，UI 名称统一为“渠道”，选项统一为“普通 / 官方”；schema 使用 `sharedFieldText('apiChannel')` 与 `sharedOptionText('regular' | 'official')`。渠道参数必须严格排在所有其他参数之前，包括分辨率、比例和模式；这条规则优先于“模式优先”。音频“声道”不属于产品渠道。
