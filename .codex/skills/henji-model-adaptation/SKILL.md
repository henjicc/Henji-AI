...
- 模型存在产品级渠道切换时，渠道参数**必须同时满足两点**：显式声明 `role: 'channel'`，且字段名写 `sharedFieldText('apiChannel')`（显示为“渠道”）。二者是双向绑定，缺一边都会被 `modelParamConventionValidator` 在模型注册时拦下。生成面板只按 `role` 决定主选择器提前渲染，不再从参数名文案反推。渠道参数必须严格排在所有其他参数之前，包括分辨率、比例和模式；这条规则优先于“模式优先”。
