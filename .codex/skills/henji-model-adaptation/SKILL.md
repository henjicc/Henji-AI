...
- 模型存在产品级渠道切换时，UI 名称统一为“渠道”，并且**必须显式声明 `role: 'channel'`**——生成面板只按 `role` 决定主选择器提前渲染，不再从参数名文案反推；漏写会被 `modelParamConventionValidator` 在模型注册时直接拦下。恰好两档渠道时选项统一为“普通 / 官方”，schema 使用 `sharedFieldText('apiChannel')` 与 `sharedOptionText('regular' | 'official')`；渠道多于两档（例如 Grsai 的多接入点）时共享词表表达不了，改用自定义标签（字段名仍写“渠道”保持术语一致），选项由模型自定义。渠道参数必须严格排在所有其他参数之前，包括分辨率、比例和模式；这条规则优先于“模式优先”。模式 / 版本 / 变体这类主选择器同样要显式声明 `role: 'mode'`（不要求 order 为 1）。音频“声道”不属于产品渠道，不要声明 role。
