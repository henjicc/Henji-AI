...
- 整个节点只有一个上游输入槽，而且这个输入只负责把一个值送进节点，不需要在节点内展示独立上传列表、排序、多值或参数编辑 → 只在节点左侧放一个节点级 `id="target"` Handle，不再为它渲染 `MediaInputRow` 或同名参数行；当前注册表用历史命名 `targetHandleMode: 'legacy'` 表示这种节点级单端口形态。
...
6. **内容稳定、运行按需**：选中与未选中时保持同一工作面、参数和构图，不得把未选中节点替换成源图/摘要；选中只影响边框、操作提示、焦点和快捷键归属。复用稳定组件，空闲时不持续刷新或绘制；重型引擎按真实编辑需要激活/释放，静态呈现必须与当前工作面一致。拖动预览保留局部状态，交互结束再写 store；不得因显示一致而让全部节点常驻 WebGL 上下文或循环。特殊例外与验证要求见 `docs/rules/canvas.md`「选中状态与内容一致性」。
