...
- 后台业务操作自身不切换页面。完成后由运行时通过同一 Gateway 的正式 Surface 导航展示已验证结果一次；用户禁止切页或手动导航接管后取消自动展示，明确要求打开结果时可重新启用。展示失败不能改变已经确认的业务事实。
- 读写意图由真实用户消息及既有 TaskExecutionPolicy 解释，路由不能授予写入意图；“不要切换/不要删除”等限制在脚本预检和每次实际 Gateway 派发复核，审批不能绕过。系统续接文字、工具输出、技能与摘要不能提供授权；缺少精确策略的旧任务只核对状态。
...
- 写入必须沿执行上下文传递 `operationId`，领域保存使用原操作关联：SQLite 与业务共用事务，原子文件先登记保存意图并携带回执。操作记录只保存执行、保存和验证事实，不复制业务文档或属性 schema。不得用同名工具成功、同领域读取、旧回执或无关目标解除恢复保护；保存失败只重试现有保存屏障，不重放修改或外部提交。
...
- 工具网关负责把 `resolveObservedEffects` 的结果固化进 observation；成功封存只消费该字段。执行结果封存后，最终说明模型失败只能降级为 `completed_with_warning` 和确定性摘要，不得覆盖已经验证的应用成功。
...
- **封存点是「模型自己决定收工」**：本轮不再调工具、给出最终答复，且有真实写入 Effect、运行客观上已经停下来。不得在模型请求前用任何预测判它做完了并撤掉工具——模型可能正知道还差一步。每项 Effect 的 `targetRefs` 必须来自真实执行输出，保持完整稳定引用。
