...
> 本文件已迁移到统一技能的 `references/canvas/` 分支。本文件中的 `references/<file>.md` 均指向同目录下的 `references/canvas/<file>.md`。
...
2. **`nodeRegistry.ts`**：加一个 `CanvasNodeDefinition`，参考 `imageEditNodeDefinition`（约第 159 行起）。关键字段取值见 [references/canvas/node-registry-fields.md](references/canvas/node-registry-fields.md)，别凭空猜字段含义。
...
适用于"有独有交互的生成节点"，比如分镜生成节点（格子编辑器）。完整真实案例和踩坑点见 [references/canvas/special-node-pattern.md](references/canvas/special-node-pattern.md)，这里只给装配公式。
...
- [ ] `nodeRegistry.ts` 的 `CanvasNodeDefinition` 字段填全，对照 [references/canvas/node-registry-fields.md](references/canvas/node-registry-fields.md)
