...
describe('图片查看与同步对比', () => {
  ...
  it('两张不同分辨率的图在任意一侧缩放和平移时同步，重置会停止进行中的缩放', async () => {
    ...
    geometry(original, 400, 400);
    geometry(result, 0, 1600);
    fireEvent.wheel(original, { clientX: 500, clientY: 200, deltaY: -250 });
    act(() => vi.advanceTimersByTime(1000));
    expect(result.style.transform).toBe(original.style.transform);
    expect(result.style.transform).toContain('scale(1.414');
    // 指针相对右侧视口中心 -100，而非整个对比区中心 100。
    expect(result.style.transform).toContain('translate(29.289');
    fireEvent.mouseDown(result, { button: 0, clientX: 200, clientY: 200 });
    fireEvent.mouseMove(result, { clientX: 240, clientY: 240 });
    fireEvent.mouseUp(result);
    expect(result.style.transform).toBe(original.style.transform);
    fireEvent.wheel(result, { clientX: 200, clientY: 200, deltaY: -200 });
  ...
  it('分界线键盘调整不翻页，原图失效回退普通结果且换图后不残留错误', async () => {
    ...
    expect(screen.getByAltText('原图').parentElement?.style.clipPath).toBe('inset(0 0 0 52%)');
    expect(screen.getByAltText('放大后').parentElement?.style.clipPath).toBe('inset(0 48% 0 0)');
  ...
  it('重复点击当前模式交换图片，保持图片实例、视图和分界线，两种模式独立记忆顺序', async () => {
    ...
    geometry(original, 400, 400);
    geometry(result, 0, 1600);
    fireEvent.wheel(original, { clientX: 600, clientY: 200, deltaY: -250 });
    act(() => vi.advanceTimersByTime(1000));
    const zoomed = result.style.transform;
    fireEvent.click(side);
    expect(result.parentElement?.style.right).toBe('0px');
    ...
    expect(result.parentElement?.style.clipPath).toBe('inset(0 0 0 52%)');
    expect(result.style.transform).toBe(beforeSwap);
    expect(slider.getAttribute('aria-valuenow')).toBe('52');
    fireEvent.keyDown(slider, { key: 'ArrowRight' });
    expect(original.parentElement?.style.clipPath).toBe('inset(0 46% 0 0)');
    expect(result.parentElement?.style.clipPath).toBe('inset(0 0 0 54%)');
    fireEvent.click(overlay);
