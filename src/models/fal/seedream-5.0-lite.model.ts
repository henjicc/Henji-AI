/** Fal Seedream 5.0 Lite 图片生成与编辑模型 */
...
export const falSeedream50LiteModel = defineModel({
  meta: {
    ...
    provider: 'fal', type: 'image', i18nScope: 'models.defs.fal-ai-seedream-5.0-lite',
  ...
  params: [
    {
      ...
      options: [{ value: 'smart', label: sharedOptionText('smart') }, ...SEEDREAM_LITE_RATIOS.map((value) => ({ value, label: value }))]
    },
    {
    ...
    },
    {
      ...
      name: sharedFieldText('numberOfImages'), default: 1, min: 1, max: 6, step: 1
    },
    {
      id: 'falSeedream50LiteMaxImages', type: 'number', order: 4,
      name: { zh: '每轮最大图片数', en: 'Max Images Per Run' }, default: 1, min: 1, max: 6, step: 1
    }
  ],
  linkages: [],
  endpoints: {
  ...
  request: {
    builder: (params) => {
      ...
      let imageSize: DynamicValue = `auto_${resolution}`
      if (raw !== 'smart' && raw !== 'auto') {
