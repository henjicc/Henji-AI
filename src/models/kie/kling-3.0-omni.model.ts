/**
 * KIE Kling 3.0 Omni（Kling O3）多模态视频模型
 */
...
export const kieKling30OmniModel = defineModel({
  ...
  request: {
    builder: (params) => {
      ...
      if (mode === 'transformation') {
        if (videos[0]) input.video_urls = [videos[0]]
        if (images.length > 0) input.image_urls = images.slice(0, 7)
        if (videos.length > 0 && images.length === 0) input.aspect_ratio = 'auto'
  ...
  pricing: {
    ...
    calculator: (params) => {
      ...
      if (resolution === '1080p') return duration * (withAudio ? 0.115 : 0.09)
