...
export const apimartNanoBananaProModel = defineModel({
  meta: {
    ...
    tags: ['text-to-image', 'image-to-image', 'supports-image-editing', 'supports-multi-image', 'max-images-8', 'supports-4k', 'provider-apimart'],
    aliases: ['nano-banana-pro-apimart'],
    polling: { interval: 3000, maxAttempts: 200, expectedAttempts: 40 }
  },
  inputLimits: { images: { max: 8 }, videos: { max: 0 } },
  params: [
    {
  ...
  request: {
    builder: (params) => {
      ...
      const supported = ['1:1', '4:3', '3:4', '16:9', '9:16']
      ...
      const resolution = ['1K', '4K'].includes(String(params.apimartNanoBananaProResolution))
        ? String(params.apimartNanoBananaProResolution) : '2K'
      ...
      if (images.length > 0) body.image_urls = images.slice(0, 8)
      return body
    }
  },
  pricing: {
