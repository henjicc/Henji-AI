...
export const focusApplicationEntityCapability = defineCapability({
  ...
  acceptsRefs: ['generation.record', 'generation.result', 'asset', 'canvas.project', 'canvas.node', 'canvas.edge', 'settings.registry',
  ...
  inputSchema: z.object({ ref: applicationRefSchema, propertyIds: z.array(z.string().min(1)).max(256).optional(),
    presentation: z.enum(['surface', 'focus']).optional() }).strict(),
  outputSchema: revisionOutputSchema,
  aiInputSchema: {
    type: 'object',
    properties: {
      ...
      propertyIds: { type: 'array', items: { type: 'string' }, maxItems: 256 },
      presentation: { type: 'string', enum: ['surface', 'focus'], description: '展示所在页面，或进一步聚焦到对象。' },
    },
    required: ['ref'],
    additionalProperties: false,
  },
