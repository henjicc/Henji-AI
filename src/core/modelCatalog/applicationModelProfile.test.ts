...
describe('Henji-AI 显式模型选择', () => {
  it('组合 8 个 provider pack 并与兼容目录保持严格 109/109 parity', () => {
    expect(HENJI_GENERATION_PROVIDER_PACKS).toHaveLength(8)
    expect(HENJI_GENERATION_MODELS).toHaveLength(109)
  ...
  it('缺少任一 provider pack 时产品选择门禁明确变红', () => {
    ...
    expect(assertHenjiGenerationSelection(HENJI_GENERATION_PROVIDER_PACKS)).toHaveLength(109)
  })
  ...
  it('生产 client 使用 modular 目录、保留八供应商、取消与 LLM chat', async () => {
    ...
    try {
      ...
      expect(client.catalog.list()).toHaveLength(121)
...
describe('Henji-AI 统一能力发现', () => {
  it('只索引已选 109 generation 与真实 LLM 目录，不默认包含 Fal 工具', () => {
    const items = henjiModelCapabilityDiscovery.list()
    expect(items.filter((item) => item.sourceKind === 'generation-model')).toHaveLength(109)
