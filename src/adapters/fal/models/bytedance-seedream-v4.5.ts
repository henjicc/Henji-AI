...
export const bytedanceSeedreamV45Route: FalModelRoute = {
  matches: (modelId: string) => modelId === 'bytedance-seedream-v4.5',
  ...
  buildImageRequest: (params: any) => {
    ...
    // 注意：bytedance seedream v4.5 的 model ID 本身就包含完整路径
    ...
    if (images.length === 0) {
      // 文生图（异步模式，支持进度回调）
      const fullPath = 'fal-ai/bytedance/seedream/v4.5/text-to-image'
      return {
        submitPath: fullPath,
        modelId: fullPath,
        requestData: {
      ...
      // 图生图/编辑（异步模式，支持进度回调）
      const fullPath = 'fal-ai/bytedance/seedream/v4.5/edit'
      return {
        submitPath: fullPath,
        modelId: fullPath,
        requestData: {
