...
describe('多角度图片块', () => {
  ...
  it('多节点静态展示只保留六个面，不创建 GPU 上下文或启动刷新循环', () => {
    ...
    expect(container.querySelectorAll('path, canvas')).toHaveLength(0)
    expect(container.querySelectorAll('circle[data-multi-angle-direction-dot]')).toHaveLength(24 * 9)
    expect(container.querySelectorAll('[data-multi-angle-direction-label]')).toHaveLength(24 * 9)
