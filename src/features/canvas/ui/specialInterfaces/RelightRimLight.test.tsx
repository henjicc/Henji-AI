...
describe('轮廓光灯位交互', () => {
  it('拖动只预览，松手提交八方位且不改变主光；取消恢复原灯位', () => {
    ...
    const lampX = (): number => Number(document.querySelector('[data-relight-light="rim"] circle')!.getAttribute('cx'))
    expect(lampX()).toBeCloseTo(70)
    fireEvent.pointerMove(rim, { pointerId: 1, clientX: 142, clientY: 120 })
    expect(lampX()).toBeCloseTo(71)
    expect(rim.getAttribute('aria-valuetext')).toBe('右下')
    const targetX = Number(document.querySelector('[data-light-stop="右下"] circle')!.getAttribute('cx'))
