...
describe('打光方向可视化控件', () => {
  ...
  it('拖过右侧分区时只发送模型支持的右侧方向', () => {
    ...
    const lampX = (): number => Number(document.querySelector('[data-relight-light="main"] circle')!.getAttribute('cx'))
    fireEvent.pointerMove(control, { pointerId: 1, clientX: 154, clientY: 100 })
    expect(lampX()).toBeCloseTo(77)
    expect(document.querySelector('[data-light-stop="右侧"][data-snap-active="true"]')).not.toBeNull()
    fireEvent.pointerMove(control, { pointerId: 1, clientX: 156, clientY: 100 })
    expect(lampX()).toBeCloseTo(78)
    expect(onDirectionChange).not.toHaveBeenCalled()
    expect(document.querySelector('[data-light-stop="右侧"][data-snap-active="true"]')).not.toBeNull()
    const targetX = Number(document.querySelector('[data-light-stop="右侧"] circle')!.getAttribute('cx'))
    expect(lampX()).not.toBeCloseTo(targetX)
    fireEvent.pointerUp(control, { pointerId: 1, clientX: 156, clientY: 100 })
    expect(lampX()).toBeCloseTo(targetX)
