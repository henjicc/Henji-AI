...
export function createMultiLayerDocumentPersistenceConfirmation(input: {
  ...
  const currentRevisions = () => ({ canvas: requireCanvasProjectInstance(input.projectId).snapshot().updatedAt })
  ...
  const currentNode = () => {
    const node = requireCanvasProjectInstance(input.projectId).store.getState().nodes.find((candidate) => candidate.id === input.nodeId)
    if (!isEditableLayerStackResultNode(node)
      ...
      throw new MultiLayerDocumentNodeApplicationError('NODE_TARGET_CHANGED',
        '图片内容已保存，但原画布节点的文档关联已变化。请恢复原节点及其文档关联后重试同步。', true)
    }
    return node
  }
  ...
  return {
    ...
    async confirm(session: ImageEditSessionReferenceV3): Promise<ImageEditPersistenceProjectionV3> {
      ...
      return withCanvasProjectRuntime(input.projectId, async (runtime) => {
