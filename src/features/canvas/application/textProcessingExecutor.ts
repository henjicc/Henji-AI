...
export interface TextProcessingExecutionOptions {
  onMissingModel: () => void
...
export function createTextProcessingExecutor(
  nodeId: string,
  store: typeof useCanvasStore,
  readOptions: () => Promise<TextProcessingExecutionOptions>,
): CanvasRegisteredExecutor {
  const run = async (execution: CanvasNodeExecutionContext): Promise<CanvasNodeExecutionResult> => {
    const { choices, promptTemplates, setPromptInvalid, onMissingModel, t } = await readOptions()
    ...
    const runtimeCanvas = store.getState()
    ...
    const cancel = () => {
      finishFailure('文本处理已取消')
      void llmCancelTask(requestId).catch(error => logger.error('文本任务取消失败', error, {
        event: 'canvas.text_processing.cancel_failed', requestId, nodeId,
      }))
    }
    ...
    try {
      ...
      if (previewTimer !== null) clearTimeout(previewTimer)
      for (const displayNodeId of displayNodeIds) useCanvasTextStreamStore.getState().setPreview(displayNodeId, null, execution.runId)
      await execution.runtime?.persist()
    }
    if (failureHandled) throw new Error(failureMessage || '文本处理失败，已停止运行下游节点')
    return { status: 'completed', resultNodeIds: displayNodeIds }
  }

  const getInputSignatureExtras = async () => {
    const { choices, promptTemplates } = await readOptions()
    const latestData = store.getState().nodes.find((node) => node.id === nodeId)
  ...
  const preflightBeforeDependencies = async (execution: CanvasNodePreflightContext) => {
    execution.signal?.throwIfAborted()
    const { choices, onMissingModel, t } = await readOptions()
    const latestData = store.getState().nodes.find((node) => node.id === nodeId)
      ?.data as TextProcessingNodeData | undefined
    const choice = latestData
      ? resolveTextProcessingModel(choices, latestData.providerId, latestData.modelId)
