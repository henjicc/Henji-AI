...
/**
 * 2.2：把当前对象的实时状态记录进指定状态关键帧（`camera_stage.state_keyframe.capture_object_refs`）。
 *
 * 助手版不依赖"选中态"——直接指定目标 stateKeyframeId，只覆盖列出的对象、只落在这一张卡上。这几条
 * 用例主要守住一件事：不能重新打开 `modelingWrite.test.ts` 那个已修问题——写入不能溢出到
 * 其他状态关键帧，也不能溢出到这张卡上未列出的对象。
 */
...
describe('三维状态关键帧状态捕获（capture_object_refs）', () => {
  let cubeId: string
  let sphereId: string
  ...
  it('只把列出的对象的实时状态记录进目标卡，不动这张卡上未列出的对象', async () => {
    ...
    await executor.apply(stateKeyframeStep(second.id, [
      { propertyId: `${CAMERA_STAGE_ENTITY_TYPES.stateKeyframe}.capture_object_refs`, value: [cubeId] },
    ]))
  ...
  it('不触碰其他状态关键帧——不重新打开 modelingWrite 那个已修问题', async () => {
    const [first, second] = useCameraStageStore.getState().stateKeyframes
    moveLiveObject(cubeId, { x: 5, y: 0, z: 0 })
    ...
    const untouchedFirst = useCameraStageStore.getState().stateKeyframes.find((stateKeyframe) => stateKeyframe.id === first.id)!
  ...
  it('能一次捕获多个对象', async () => {
    ...
    await executor.apply(stateKeyframeStep(second.id, [
      { propertyId: `${CAMERA_STAGE_ENTITY_TYPES.stateKeyframe}.capture_object_refs`, value: [cubeId, sphereId] },
    ]))
  ...
  it('对象 id 不存在时拒绝写入，不改动任何状态关键帧', async () => {
    const second = useCameraStageStore.getState().stateKeyframes[1]
    const before = structuredClone(useCameraStageStore.getState().stateKeyframes)

    await expect(executor.apply(stateKeyframeStep(second.id, [
      { propertyId: `${CAMERA_STAGE_ENTITY_TYPES.stateKeyframe}.capture_object_refs`, value: ['不存在的对象'] },
    ]))).rejects.toThrow('STATE_KEYFRAME_CAPTURE_OBJECT_NOT_FOUND')
  ...
  it('接受带工程前缀的稳定引用', async () => {
    ...
    await executor.apply(stateKeyframeStep(second.id, [
      { propertyId: `${CAMERA_STAGE_ENTITY_TYPES.stateKeyframe}.capture_object_refs`, value: [`project-1:${cubeId}`] },
    ]))
