import { fieldWriterTable, type ApplicationRef, type ApplicationPropertyMutation, type JsonValue } from '@/core/application-control'
...
/*
 * 三维对象与摄像机 16 条可写属性（object 7 + camera 9）的统一定义——1.3 迁移。
 *
 * 写入目标是累积器 `CameraStageObjectDraft`（`update` / `transform` / `transformTouched` /
 * `current`），不是直接调 store 方法：变换三轴要先累积进同一个 transform 再一次性提交，
 * 逐轴提交会在时间轴上打出三个独立改动；`current` 供画幅与注视点互读。
 *
