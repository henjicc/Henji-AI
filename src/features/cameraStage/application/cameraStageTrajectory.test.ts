 ...
 * json 数组整体读写，落地到同一个 `store.setStateKeyframeSpatialPath`；起止端点是另一个 store 动作
 * （`setStateKeyframePathAnchor`，语义是改相邻状态关键帧里的对象位置快照，不是改 knots）。
 */
...
function trajectoryStep(stateKeyframeId: string, objectId: string, mutations: Array<{ propertyId: string; value: unknown }>): Extract<ApplicationPlannedStep, { kind: 'mutation' }> {
  return {
    kind: 'mutation',
    entityType: CAMERA_STAGE_ENTITY_TYPES.trajectory,
    target: { kind: CAMERA_STAGE_ENTITY_TYPES.trajectory, id: `project-1:${stateKeyframeId}:${objectId}`, revision },
...
describe('三维轨迹手工编辑', () => {
  let cameraId: string
  let stateKeyframeAId: string
  let stateKeyframeBId: string
  ...
  beforeEach(async () => {
    ...
    useCameraStageStore.getState().applyCameraPathPreset(stateKeyframeAId, cameraId, { kind: 'orbit', degrees: 180, direction: 'cw' })
  })

  function currentPath() {
    return useCameraStageStore.getState().stateKeyframes.find((stateKeyframe) => stateKeyframe.id === stateKeyframeAId)!.transition.perObject[cameraId]!.spatialPath!
  }
  ...
  it('整条重写 knots 会替换全部控制点', async () => {
    ...
    await executor.apply(trajectoryStep(stateKeyframeAId, cameraId, [
      {
  ...
  it('能挪起点位置，落在这张卡自己的对象快照上', async () => {
    ...
    const stateKeyframeA = useCameraStageStore.getState().stateKeyframes.find((stateKeyframe) => stateKeyframe.id === stateKeyframeAId)!
    expect(stateKeyframeA.objectStates[cameraId].transform.position).toEqual({ x: 9, y: 9, z: 9 })
  })
  ...
  it('能挪终点位置，落在下一张卡的对象快照上', async () => {
    ...
    const stateKeyframeB = useCameraStageStore.getState().stateKeyframes.find((stateKeyframe) => stateKeyframe.id === stateKeyframeBId)!
    expect(stateKeyframeB.objectStates[cameraId].transform.position).toEqual({ x: 8, y: 8, z: 8 })
  })

  it('目标状态关键帧没有轨迹时拒绝写入', async () => {
    await expect(executor.apply(trajectoryStep(stateKeyframeBId, cameraId, [
      { propertyId: `${CAMERA_STAGE_ENTITY_TYPES.trajectory}.start_out_tangent`, value: { x: 1, y: 0, z: 0 } },
