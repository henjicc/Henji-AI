...
export interface ShotMarqueeRect {
  left: number
  top: number
  width: number
  height: number
...
interface UseShotMarqueeSelectOptions {
 ...
 */
export function useShotMarqueeSelect({
  containerRef,
  shots,
  pxPerSecond,
  onCommit,
}: UseShotMarqueeSelectOptions): {
  rect: ShotMarqueeRect | null
  ...
  const [rect, setRect] = useState<ShotMarqueeRect | null>(null)
  ...
  const handlePointerMove = useCallback((event: React.PointerEvent<HTMLDivElement>): void => {
    ...
    const ids = shots
      .filter((shot) => {
        const x = timeToX(shot.time, pxPerSecond)
  ...
  }, [containerRef, pxPerSecond, shots])
