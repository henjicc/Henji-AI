...
/**
 * 属性行码表按钮（AE 式打点，可作用于整个分组或单条分量轨道）。三态：
...
const KeyframeStopwatch: React.FC<KeyframeStopwatchProps> = ({ objectId, groupPath, path, className }) => {
  ...
  const descriptor = isSingleTrack ? getAnimatablePropByPath(path) : undefined
  const childPaths = group?.children.map((child) => child.path) ?? (descriptor ? [descriptor.path] : [])
  ...
  const allKeyedHere =
    ...
    childPaths.every((path) => {
      const track = tracks.find((t) => t.objectId === objectId && t.propertyPath === path)
  ...
  return (
    <UiIconButton
      ...
      onClick={() => {
        if (isSingleTrack) toggleKeyframe(objectId, path)
        else if (typeof groupPath === 'string') toggleKeyframeGroup(objectId, groupPath)
      }}
    >
