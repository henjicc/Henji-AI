/**
 * 状态关键帧时间轴时间码格式化（纯函数，禁止 UI 依赖）。
 *
 * 重要记录 008（已定稿）：时间码支持纯秒 / 纯帧 / 秒:帧（SMPTE 风格 hh:mm:ss:ff）三种显示格式，
 * 交互方式为「按住 Ctrl 键点击时间码文本」循环切换；本文件只负责格式化与循环顺序，
 * 交互由 StateKeyframeTimecodeText.tsx 承载。
 */

export type StateKeyframeTimecodeMode = 'seconds' | 'frames' | 'secondsFrames'

const MODE_CYCLE: StateKeyframeTimecodeMode[] = ['seconds', 'frames', 'secondsFrames']

/** Ctrl+点击循环切换到下一个显示模式 */
export function nextStateKeyframeTimecodeMode(mode: StateKeyframeTimecodeMode): StateKeyframeTimecodeMode {
...
export function formatStateKeyframeTimecode(seconds: number, mode: StateKeyframeTimecodeMode, fps: number): string {
 ...
 */
export function formatCompactStateKeyframeTimecode(seconds: number, mode: StateKeyframeTimecodeMode, fps: number): string {
  if (mode !== 'secondsFrames') return formatStateKeyframeTimecode(seconds, mode, fps)
