...
export function createPlaybackSlice(
  ...
  return {
    play: () => {
      ...
      if (isAttached()) seekCameraStagePlaybackRuntime(time, true)
    ...
    pause: () => {
      ...
      const continuousTime = state.playback.playing && isAttached()
      ...
      if (isAttached()) pauseCameraStagePlaybackRuntime(snapped)
      set((current) => ({ playback: { ...current.playback, playing: false, currentTime: snapped } }))
    },

    stop: () => {
      applySampledObjectsSilently(0)
      if (isAttached()) pauseCameraStagePlaybackRuntime(0)
    ...
    seek: (time) => {
      ...
      if (isAttached()) seekCameraStagePlaybackRuntime(clamped, state.playback.playing)
