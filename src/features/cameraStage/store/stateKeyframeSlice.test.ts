import { beforeEach, describe, expect, it } from 'vitest'
import { createDefaultPlayback } from '../domain/animationTypes'
import { compileStateKeyframesToAnimation } from '../domain/stateKeyframeCompiler'
import { createCameraObject, createPrimitiveObject, pickDefaultColor } from '../domain/sceneDefaults'
import { createStateKeyframe } from '../domain/stateKeyframeTypes'
import type { StageStateKeyframe } from '../domain/stateKeyframeTypes'
import type { StageVec3 } from '../domain/sceneTypes'
import { applyAnimationAtTime } from './playbackSampling'
import {
  beginHistorySession,
  clearCameraStageHistory,
  endHistorySession,
  useCameraStageStore,
} from './cameraStageStore'

function resetSimpleStore(): void {
  const object = createPrimitiveObject('box', '方块01', pickDefaultColor(0))
  const stateKeyframe = createStateKeyframe([object], '片段 1')
  useCameraStageStore.setState({
    objects: [object],
    stateKeyframes: [stateKeyframe],
    selectedStateKeyframeId: stateKeyframe.id,
    selectedStateKeyframeIds: [],
    animation: compileStateKeyframesToAnimation([stateKeyframe], [object]),
    playback: createDefaultPlayback(),
  })
  clearCameraStageHistory()
}

function addStateKeyframeAt(time: number): void {
  useCameraStageStore.getState().seek(time)
  useCameraStageStore.getState().addStateKeyframe()
}

function expectAllStateKeyframesUseManualTarget(
  stateKeyframes: StageStateKeyframe[],
  cameraId: string,
  target: StageVec3,
): void {
  for (const stateKeyframe of stateKeyframes) {
    const lookAt = stateKeyframe.objectStates[cameraId]?.lookAt
    expect(lookAt?.mode).toBe('manual')
    if (lookAt?.mode === 'manual') expect(lookAt.target).toEqual(target)
  }
}

describe('状态关键帧模式 store 分片', () => {
  beforeEach(resetSimpleStore)

  it('应用运镜预设会生成可编辑路径并同步下一关键帧终点', () => {
    const camera = createCameraObject('摄像机01', pickDefaultColor(0), {
      position: { x: 0, y: 2, z: 5 },
      target: { x: 0, y: 0, z: 0 },
    })
    const stateKeyframeA = createStateKeyframe([camera], 'A', camera.id, 0)
    const stateKeyframeB = createStateKeyframe([camera], 'B', camera.id, 2)
    useCameraStageStore.setState({
      objects: [camera],
      stateKeyframes: [stateKeyframeA, stateKeyframeB],
      animation: compileStateKeyframesToAnimation([stateKeyframeA, stateKeyframeB], [camera]),
    })
    useCameraStageStore.getState().applyCameraPathPreset(
      stateKeyframeA.id,
      camera.id,
      { kind: 'orbit', degrees: 180, direction: 'cw' },
    )
    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes[0].transition.perObject[camera.id].spatialPath?.knots).toHaveLength(1)
    expect(state.stateKeyframes[1].objectStates[camera.id].transform.position.z).toBeCloseTo(-5, 5)
    expect(state.animation.tracks.some((track) => track.propertyPath === 'transform.position.z')).toBe(true)
  })

  it('修改轨迹后立即让场景对象对齐当前播放头采样位置', () => {
    const camera = createCameraObject('摄像机01', pickDefaultColor(0), {
      position: { x: 0, y: 2, z: 5 },
      target: { x: 0, y: 0, z: 0 },
    })
    const stateKeyframeA = createStateKeyframe([camera], 'A', camera.id, 0)
    const movedCamera = {
      ...camera,
      transform: {
        ...camera.transform,
        position: { x: 6, y: 3, z: 1 },
      },
    }
    const stateKeyframeB = createStateKeyframe([movedCamera], 'B', camera.id, 2)
    const stateKeyframes = [stateKeyframeA, stateKeyframeB]
    useCameraStageStore.setState({
      objects: [camera],
      stateKeyframes,
      animation: compileStateKeyframesToAnimation(stateKeyframes, [camera]),
      playback: { ...createDefaultPlayback(), currentTime: 1 },
    })

    useCameraStageStore.getState().applyCameraPathPreset(
      stateKeyframeA.id,
      camera.id,
      { kind: 'orbit', degrees: 90, direction: 'cw' },
    )

    const state = useCameraStageStore.getState()
    const sampled = applyAnimationAtTime([camera], state.animation, state.playback.currentTime)[0]
    expect(state.objects[0].transform.position).toEqual(sampled.transform.position)
  })

  it('连续拖动路径锚点合并为单条撤销记录并将预设标记为自定义', () => {
    const state = useCameraStageStore.getState()
    const objectId = state.objects[0].id
    addStateKeyframeAt(2)
    const stateKeyframeId = useCameraStageStore.getState().stateKeyframes[0].id
    useCameraStageStore.getState().setStateKeyframeSpatialPath(
      stateKeyframeId,
      objectId,
      {
        kind: 'bezier',
        source: { kind: 'preset', preset: { kind: 'truck', offset: 2 } },
        startOutTangent: { x: 1, y: 0, z: 0 },
        knots: [],
        endInTangent: { x: -1, y: 0, z: 0 },
      },
    )
    const originalEnd = structuredClone(useCameraStageStore.getState().stateKeyframes[1].objectStates[objectId].transform.position)
    clearCameraStageHistory()
    beginHistorySession()
    useCameraStageStore.getState().setStateKeyframePathAnchor(stateKeyframeId, objectId, 'end', { x: 2, y: 1, z: 0 })
    useCameraStageStore.getState().setStateKeyframePathAnchor(stateKeyframeId, objectId, 'end', { x: 3, y: 2, z: 0 })
    endHistorySession()
    expect(useCameraStageStore.getState().stateKeyframes[0].transition.perObject[objectId].spatialPath?.source.kind).toBe('custom')
    useCameraStageStore.temporal.getState().undo()
    expect(useCameraStageStore.getState().stateKeyframes[1].objectStates[objectId].transform.position).toEqual(originalEnd)
  })

  it('新建场景默认自带一台摄像机并进入摄像机视角', () => {
    useCameraStageStore.getState().newScene('新工程')
    const state = useCameraStageStore.getState()
    const camera = state.objects.find((item) => item.type === 'camera')
    expect(state.objects).toHaveLength(1)
    expect(camera).toBeDefined()
    expect(state.viewMode).toBe('camera')
    expect(state.activeCameraId).toBe(camera?.id)
    expect(state.selectedId).toBe(camera?.id)
    // 默认状态关键帧同步捕获了摄像机快照，且拍摄机位指向默认摄像机
    expect(state.stateKeyframes).toHaveLength(1)
    expect(state.stateKeyframes[0].cameraId).toBe(camera?.id)
    expect(camera && state.stateKeyframes[0].objectStates[camera.id]).toBeDefined()
  })

  it('编辑对象时原子写回选中卡并保留完整编译产物', () => {
    const initial = useCameraStageStore.getState()
    const objectId = initial.objects[0].id
    addStateKeyframeAt(2)
    useCameraStageStore.getState().updateTransform(objectId, { position: { x: 4, y: 0, z: 0 } })

    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes[1].objectStates[objectId].transform.position.x).toBe(4)
    expect(state.animation.tracks.some((track) => track.propertyPath === 'transform.position.x')).toBe(true)
    expect(state.animation.motionSchedule).toEqual([])

    useCameraStageStore.temporal.getState().undo()
    const undone = useCameraStageStore.getState()
    expect(undone.objects[0].transform.position.x).toBe(0)
    expect(undone.stateKeyframes[1].objectStates[objectId].transform.position.x).toBe(0)
    expect(undone.animation.tracks).toHaveLength(0)
  })

  it('播放头在过渡段编辑时自动插入当前帧关键帧', () => {
    const initial = useCameraStageStore.getState()
    const objectId = initial.objects[0].id
    addStateKeyframeAt(2)
    useCameraStageStore.getState().seek(1)
    useCameraStageStore.getState().updateTransform(objectId, { position: { x: 9, y: 0, z: 0 } })
    const afterTransitionEdit = useCameraStageStore.getState()
    expect(afterTransitionEdit.objects[0].transform.position.x).toBe(9)
    expect(afterTransitionEdit.stateKeyframes).toHaveLength(3)
    const inserted = afterTransitionEdit.stateKeyframes.find((stateKeyframe) => Math.abs(stateKeyframe.time - 1) < 1e-6)
    expect(inserted?.objectStates[objectId].transform.position.x).toBe(9)
    expect(afterTransitionEdit.selectedStateKeyframeId).toBe(inserted?.id)
  })

  it('手动定位只能停在整帧，播放内部回写仍保留连续时间', () => {
    const store = useCameraStageStore.getState()
    store.seek(1.019)
    expect(useCameraStageStore.getState().playback.currentTime).toBeCloseTo(31 / 30, 10)

    useCameraStageStore.getState().setPlaybackTime(1.019)
    expect(useCameraStageStore.getState().playback.currentTime).toBeCloseTo(1.019, 10)
  })

  it('暂停播放时把播放头和场景吸附到最近整帧', () => {
    useCameraStageStore.getState().setPlaybackTime(1.019)
    useCameraStageStore.getState().pause()
    const state = useCameraStageStore.getState()
    expect(state.playback.playing).toBe(false)
    expect(state.playback.currentTime).toBeCloseTo(31 / 30, 10)
  })

  it('过渡段自动插帧时播放头原子对齐吸附帧，避免重编译后二次插值跳变', () => {
    const initial = useCameraStageStore.getState()
    const objectId = initial.objects[0].id
    addStateKeyframeAt(2)
    useCameraStageStore.getState().seek(1.019)

    useCameraStageStore.getState().updateTransform(objectId, { position: { x: 9, y: 0, z: 0 } })

    const state = useCameraStageStore.getState()
    const snappedTime = 31 / 30
    const inserted = state.stateKeyframes.find((stateKeyframe) => Math.abs(stateKeyframe.time - snappedTime) < 1e-6)
    expect(inserted?.objectStates[objectId].transform.position.x).toBe(9)
    expect(state.playback.currentTime).toBeCloseTo(snappedTime, 10)
  })

  it('播放态编辑不自动记录状态关键帧', () => {
    const before = useCameraStageStore.getState()
    const objectId = before.objects[0].id
    useCameraStageStore.setState({ playback: { ...before.playback, playing: true } })
    before.updateTransform(objectId, { position: { x: 2, y: 0, z: 0 } })
    expect(useCameraStageStore.getState().stateKeyframes[0].objectStates[objectId].transform.position.x).toBe(0)
  })

  it('拖动关键帧按帧吸附并自动重算相邻过渡时长', () => {
    addStateKeyframeAt(2)
    const before = useCameraStageStore.getState()
    const second = before.stateKeyframes[1]
    before.moveStateKeyframeTime(second.id, 1.019)

    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes[1].time).toBeCloseTo(31 / 30, 10)
    expect(state.stateKeyframes[0].transitionDuration).toBeCloseTo(31 / 30, 10)
    expect(state.animation.duration).toBeCloseTo(31 / 30, 10)
  })

  it('过渡区连续编辑只插入一个状态点并持续更新它', () => {
    const objectId = useCameraStageStore.getState().objects[0].id
    addStateKeyframeAt(2)
    useCameraStageStore.getState().seek(1)
    useCameraStageStore.getState().updateTransform(objectId, { position: { x: 6, y: 0, z: 0 } })
    useCameraStageStore.getState().updateTransform(objectId, { position: { x: 7, y: 0, z: 0 } })

    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes).toHaveLength(3)
    const inserted = state.stateKeyframes.find((stateKeyframe) => Math.abs(stateKeyframe.time - 1) < 1e-6)
    expect(inserted?.objectStates[objectId].transform.position.x).toBe(7)
    expect(state.selectedStateKeyframeId).toBe(inserted?.id)
  })

  it('交互开始时先用当前采样画面冻结关键帧，首个移动增量不再改变动画拓扑', () => {
    const objectId = useCameraStageStore.getState().objects[0].id
    addStateKeyframeAt(2)
    useCameraStageStore.getState().seek(1)

    useCameraStageStore.getState().prepareStateKeyframeEdit(objectId)
    const prepared = useCameraStageStore.getState()
    expect(prepared.stateKeyframes).toHaveLength(3)
    const insertedId = prepared.selectedStateKeyframeId

    useCameraStageStore.getState().updateTransform(objectId, { position: { x: 7, y: 0, z: 0 } })
    const edited = useCameraStageStore.getState()
    expect(edited.stateKeyframes).toHaveLength(3)
    expect(edited.selectedStateKeyframeId).toBe(insertedId)
    expect(edited.stateKeyframes.find((stateKeyframe) => stateKeyframe.id === insertedId)?.objectStates[objectId].transform.position.x).toBe(7)
  })

  it('摄像机视图一次原子更新同时记录位置、XYZ 旋转和注视点', () => {
    const camera = createCameraObject('摄像机01', pickDefaultColor(0))
    const first = createStateKeyframe([camera], '关键帧 1', camera.id, 0)
    const second = createStateKeyframe([camera], '关键帧 2', camera.id, 2)
    const stateKeyframes = [first, second]
    useCameraStageStore.setState({
      objects: [camera],
      stateKeyframes,
      selectedStateKeyframeId: second.id,
      animation: compileStateKeyframesToAnimation(stateKeyframes, [camera]),
      playback: { ...createDefaultPlayback(), currentTime: 1 },
      activeCameraId: camera.id,
    })

    useCameraStageStore.getState().updateCameraView(camera.id, {
      position: { x: 1, y: 2, z: 3 },
      rotation: { x: 10, y: 20, z: 30 },
      lookAtTarget: { x: 4, y: 5, z: 6 },
    })

    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes).toHaveLength(3)
    const inserted = state.stateKeyframes.find((stateKeyframe) => Math.abs(stateKeyframe.time - 1) < 1e-6)
    const snapshot = inserted?.objectStates[camera.id]
    expect(snapshot?.transform.position).toEqual({ x: 1, y: 2, z: 3 })
    expect(snapshot?.transform.rotation).toEqual({ x: 10, y: 20, z: 30 })
    expect(snapshot?.lookAt).toEqual({ mode: 'manual', target: { x: 4, y: 5, z: 6 } })
    expectAllStateKeyframesUseManualTarget(state.stateKeyframes, camera.id, { x: 4, y: 5, z: 6 })
  })

  it('修改注视目标会同步全部状态关键帧但不会自动添加关键帧', () => {
    const camera = createCameraObject('摄像机01', pickDefaultColor(0))
    const first = createStateKeyframe([camera], '关键帧 1', camera.id, 0)
    const second = createStateKeyframe([camera], '关键帧 2', camera.id, 2)
    const stateKeyframes = [first, second]
    useCameraStageStore.setState({
      objects: [camera],
      stateKeyframes,
      selectedStateKeyframeId: first.id,
      animation: compileStateKeyframesToAnimation(stateKeyframes, [camera]),
      playback: { ...createDefaultPlayback(), currentTime: 1 },
    })

    useCameraStageStore.getState().updateObject(camera.id, {
      lookAt: { mode: 'manual', target: { x: 3, y: 4, z: 5 } },
    })

    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes).toHaveLength(2)
    expect(state.playback.currentTime).toBe(1)
    expectAllStateKeyframesUseManualTarget(state.stateKeyframes, camera.id, { x: 3, y: 4, z: 5 })
  })

  it('仅更新摄像机注视点时不会自动添加关键帧', () => {
    const camera = createCameraObject('摄像机01', pickDefaultColor(0))
    const first = createStateKeyframe([camera], '关键帧 1', camera.id, 0)
    const second = createStateKeyframe([camera], '关键帧 2', camera.id, 2)
    const stateKeyframes = [first, second]
    useCameraStageStore.setState({
      objects: [camera],
      stateKeyframes,
      selectedStateKeyframeId: first.id,
      animation: compileStateKeyframesToAnimation(stateKeyframes, [camera]),
      playback: { ...createDefaultPlayback(), currentTime: 1 },
    })

    useCameraStageStore.getState().updateCameraView(camera.id, {
      lookAtTarget: { x: -2, y: 1, z: 6 },
    })

    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes).toHaveLength(2)
    expectAllStateKeyframesUseManualTarget(state.stateKeyframes, camera.id, { x: -2, y: 1, z: 6 })
  })

  it('批量删除关键帧：单条撤销记录、清空框选、重算过渡与选中', () => {
    addStateKeyframeAt(1)
    addStateKeyframeAt(2)
    addStateKeyframeAt(3)
    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes).toHaveLength(4)
    const removeIds = [state.stateKeyframes[1].id, state.stateKeyframes[2].id]
    useCameraStageStore.setState({ selectedStateKeyframeIds: removeIds })

    state.removeStateKeyframes(removeIds)
    const removed = useCameraStageStore.getState()
    expect(removed.stateKeyframes).toHaveLength(2)
    expect(removed.selectedStateKeyframeIds).toEqual([])
    expect(removed.stateKeyframes[0].transitionDuration).toBeCloseTo(3, 10)
    expect(removed.selectedStateKeyframeId).toBe(removed.stateKeyframes[1].id)

    useCameraStageStore.temporal.getState().undo()
    expect(useCameraStageStore.getState().stateKeyframes).toHaveLength(4)
  })

  it('零轨道但状态关键帧存在时间跨度时仍可播放', () => {
    addStateKeyframeAt(2)
    const compiled = useCameraStageStore.getState()
    expect(compiled.animation.tracks).toHaveLength(0)
    expect(compiled.animation.duration).toBeGreaterThan(0)
    compiled.play()
    expect(useCameraStageStore.getState().playback.playing).toBe(true)

  })

  it('新增与删除对象同步所有卡片且清理过渡详情', () => {
    const state = useCameraStageStore.getState()
    addStateKeyframeAt(2)
    state.addCamera()
    const added = useCameraStageStore.getState()
    const cameraId = added.objects.find((object) => object.type === 'camera')?.id
    expect(cameraId).toBeDefined()
    expect(added.stateKeyframes.every((stateKeyframe) => cameraId !== undefined && stateKeyframe.objectStates[cameraId] !== undefined)).toBe(true)
    if (!cameraId) return
    added.removeObject(cameraId)
    const removed = useCameraStageStore.getState()
    expect(removed.stateKeyframes.every((stateKeyframe) => stateKeyframe.objectStates[cameraId] === undefined)).toBe(true)
    expect(removed.animation.tracks.every((track) => track.objectId !== cameraId)).toBe(true)
  })

  it('选择状态关键帧会应用快照并定位到卡片起点', () => {
    const first = useCameraStageStore.getState()
    const objectId = first.objects[0].id
    addStateKeyframeAt(2)
    const secondId = useCameraStageStore.getState().selectedStateKeyframeId
    if (!secondId) throw new Error('新增状态关键帧后应存在选中项')
    useCameraStageStore.getState().updateTransform(objectId, { position: { x: 3, y: 0, z: 0 } })
    useCameraStageStore.getState().selectStateKeyframe(first.stateKeyframes[0].id)
    expect(useCameraStageStore.getState().objects[0].transform.position.x).toBe(0)
    useCameraStageStore.getState().selectStateKeyframe(secondId)
    const selected = useCameraStageStore.getState()
    expect(selected.objects[0].transform.position.x).toBe(3)
    expect(selected.playback.currentTime).toBe(first.stateKeyframes[0].hold + first.stateKeyframes[0].transitionDuration)
  })

  it('建卡默认沿用当前 activeCameraId 作为拍摄机位', () => {
    useCameraStageStore.getState().addCamera()
    const cameraId = useCameraStageStore.getState().activeCameraId
    expect(cameraId).toBeTruthy()
    addStateKeyframeAt(2)
    const stateKeyframes = useCameraStageStore.getState().stateKeyframes
    expect(stateKeyframes[stateKeyframes.length - 1].cameraId).toBe(cameraId)
  })

  it('updateStateKeyframeCamera 更新机位并触发重编译（机位不同的相邻卡强制硬切）', () => {
    useCameraStageStore.getState().addCamera()
    const cameraA = useCameraStageStore.getState().activeCameraId
    if (!cameraA) throw new Error('测试需要摄像机 A')
    useCameraStageStore.getState().updateStateKeyframeCamera(useCameraStageStore.getState().stateKeyframes[0].id, cameraA)
    useCameraStageStore.getState().addCamera()
    const cameraB = useCameraStageStore.getState().activeCameraId
    if (!cameraB) throw new Error('测试需要摄像机 B')
    addStateKeyframeAt(2)
    const stateKeyframes1 = useCameraStageStore.getState().stateKeyframes
    useCameraStageStore.getState().updateStateKeyframeCamera(stateKeyframes1[1].id, cameraB)

    const state = useCameraStageStore.getState()
    expect(state.stateKeyframes[1].cameraId).toBe(cameraB)
    // 机位不同 → 区间保留，画面在后关键帧时刻执行阶跃硬切。
    expect(state.animation.duration).toBeCloseTo(2, 5)
  })

  it('新建摄像机在已有摄像机时继承首摄像机画幅，且非首摄像机画幅补丁被钳制忽略', () => {
    useCameraStageStore.getState().addCamera()
    const cameraA = useCameraStageStore.getState().objects.find((item) => item.type === 'camera')
    if (!cameraA) throw new Error('测试需要摄像机 A')
    useCameraStageStore.getState().updateObject(cameraA.id, { aspectRatio: { preset: 'custom', ratio: 2.5 } })

    useCameraStageStore.getState().addCamera()
    const cameraB = useCameraStageStore.getState().objects.find(
      (item) => item.type === 'camera' && item.id !== cameraA.id,
    )
    if (!cameraB) throw new Error('测试需要摄像机 B')
    expect(cameraB.type === 'camera' ? cameraB.aspectRatio : null).toEqual({ preset: 'custom', ratio: 2.5 })

    // 非首摄像机改画幅被钳制忽略
    useCameraStageStore.getState().updateObject(cameraB.id, { aspectRatio: { preset: '1:1', ratio: 1 } })
    const afterIgnored = useCameraStageStore.getState().objects.find((item) => item.id === cameraB.id)
    expect(afterIgnored?.type === 'camera' ? afterIgnored.aspectRatio : null).toEqual({ preset: 'custom', ratio: 2.5 })

    // 首摄像机再次改画幅，联动同步非首摄像机
    useCameraStageStore.getState().updateObject(cameraA.id, { aspectRatio: { preset: '16:9', ratio: 16 / 9 } })
    const after = useCameraStageStore.getState().objects
    for (const item of after) {
      if (item.type === 'camera') expect(item.aspectRatio).toEqual({ preset: '16:9', ratio: 16 / 9 })
    }
  })
})
