 ...
 *
 * 机制是两个函数叠出来的：新对象由 syncAddedObjectToStateKeyframes 以**默认状态**写进所有关键帧卡；
 * 之后的 updateObject 在状态关键帧模式下只捕获进当前选中的那一张卡（captureObjectsIntoStateKeyframe 对其他
 * 卡原样返回）。于是球体在 1 张卡上是"新位置 + 白色"、在其余卡上是"默认位置 + 默认颜色"，
 * 播放时插值——实测截图里球体是淡黄色而不是白色，那正是插值中间态。
 *
 * 人手动拖物体时希望自动打点（那确实是在编辑动画），所以 updateObject 行为不动；
 * 助手与批量写入走 updateObjectAcrossStateKeyframes 的建模语义。
 */
...
function objectStateColors(objectId: string): string[] {
  return useCameraStageStore.getState().stateKeyframes.map((stateKeyframe) => stateKeyframe.objectStates[objectId]?.color ?? '')
}
...
describe('建模语义写入不产生意料之外的动画', () => {
  ...
  it('updateObjectAcrossStateKeyframes 把改动同步进所有关键帧卡', () => {
    ...
    useCameraStageStore.getState().addStateKeyframe()
    expect(useCameraStageStore.getState().stateKeyframes.length).toBeGreaterThanOrEqual(2)

    useCameraStageStore.getState().updateObjectAcrossStateKeyframes(objectId, { color: WHITE })
