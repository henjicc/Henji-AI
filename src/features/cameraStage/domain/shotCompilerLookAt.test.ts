...
function shotsFor(objects: StageObject[]): StageShot[] {
  // 用真实工厂建卡，避免手搓结构漏字段（transition.perObject 之类）导致测试假红。
  return [0, 1].map((index) => {
    const shot = createShot(objects, `关键帧 ${index + 1}`, 'cam-1', index)
    // 第二张卡把摄像机挪个位置，否则两卡完全相同、编译器不产生任何轨道。
    if (index === 0) return shot
    const camera = shot.objectStates['cam-1']
    return {
      ...shot,
      objectStates: {
        ...shot.objectStates,
...
function cameraRotationTrack(objects: StageObject[]): unknown[] {
  const animation = compileShotsToAnimation(shotsFor(objects), objects)
