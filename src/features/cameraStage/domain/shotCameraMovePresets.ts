 ...
 * （time ∈ [segStart, segEnd] + 三维位置），供 shotCompiler.ts 的
 ...
 * 的原始机位——本文件只负责几何计算，B 卡快照被忽略/覆盖的决策落在 shotCompiler.ts。
 */
...
import type { StageCameraMove } from './shotTypes'
 ...
 * 供 shotCompiler.ts 的 compileCameraPositionGroup 调用，写入 x/y/z 三条 scalar 轨道。
 */
