 ...
 *
 * 由状态关键帧序列派生"某一时刻应由哪台摄像机渲染"的阶跃函数（重要记录 005）。复用
 * `buildStateKeyframeTimeline`（stateKeyframeCompiler.ts）算出的静止/过渡时间点：每枚状态关键帧的静止段 + 其后过渡段
 * 都由该卡自己的机位拍摄，下一次切换发生在下一卡静止段起点——这与布点层"机位不同强制硬切"
 * 的切换点天然重合，播放/scrub/导出三处只要都查询本表即可保持一致（3.2/3.3 消费）。
 */

import { buildStateKeyframeTimeline } from './stateKeyframeCompiler'
...
export function buildRenderCameraSchedule(
  ...
  if (stateKeyframes.length === 0) return []
  ...
  return stateKeyframes.map((stateKeyframe, index) => ({
    ...
    cameraId: stateKeyframe.cameraId ?? fallbackCameraId,
  }))
}

/**
 * 按时间查询渲染机位（边界时间归属"后一段"，与 stateKeyframeClipGeometry 的 findClipAtTime 语义一致）；
