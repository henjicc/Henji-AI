...
class SettingsReflectionProvider implements ApplicationEntityProvider {
  ...
  async listEntities(): Promise<{
    ...
    return { refs: [SETTINGS_REF], nextCursor: null, revisions: { settings: getSettingsRegistryRevision() } }
  }

  async readEntity(ref: ApplicationRef, request: { propertyIds?: string[] }): Promise<{
  ...
  async getPropertyAvailability(ref: ApplicationRef, propertyIds: string[]): Promise<Array<{
    propertyId: string
    readable: boolean
    writable: boolean
    reasons: string[]
...
export function createSettingsReflectionRegistration(): ApplicationEntityRegistration {
  return {
    entity: {
      ...
      schemaRef: {
        catalogVersion: APPLICATION_CAPABILITY_CATALOG_VERSION,
        kind: 'entity', id: SETTINGS_ENTITY_TYPE, version: 1, digest: ENTITY_SCHEMA_DIGEST,
      },
    },
