...
describe('视口共享像素源的边界预算与缩略图优先级', () => {
  it('薄图mip10跨画笔边界只补读必要的mip0块，并预留真实解码大小', () => {
    ...
    const candidate = planImageEditorViewportTilesV3({ resourceRef: RESOURCE, documentSize: document.geometry,
    ...
    expect(estimateImageEditorViewportWorkingRegionPixelsV3(prepared, candidate, false, sizes)).toBe(512)
  })
  ...
  it('小原图全局分析仍按文档输出尺寸预留，不能只预算16像素原图', () => {
    ...
    expect(estimateImageEditorViewportWorkingRegionPixelsV3(prepared,
      { mip: 0, tiles: [], estimatedBytes: 0 }, true, new Map([[RESOURCE, { width: 16, height: 16 }]]))).toBe(64 * 64)
  })
