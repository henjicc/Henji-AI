...
describe('ImageEditorPreviewV3 managed frame ownership', () => {
  ...
  it('GPU 已接管时 pointermove 立即更新会话并由GPU帧合并且不发布 PreviewOverride', async () => {
    ...
    await waitFor(() => expect(changes).toHaveBeenCalledTimes(1))
    // React 尚未把新文档同步给渲染会话。此时清除瞬态会让旧场景闪回原位。
    expect(renderSession.clearTransientLayerTransform).not.toHaveBeenCalled()
    expect(renderSession.requestFrame).toHaveBeenCalledTimes(3)
    expect(liveSession.bus.getSnapshot().previewOverrides).toEqual({})
    expect(liveSession.bus.getSnapshot().document.revision).toBe(1)
    expect(liveSession.bus.getSnapshot().document.layers[0].transform).toEqual([1, 0, 0, 1, 40, 25])
    fireEvent.pointerDown(surface, { pointerId: 72, isPrimary: true, button: 0, clientX: 50, clientY: 35 })
    fireEvent.pointerMove(surface, { pointerId: 72, clientX: 70, clientY: 55, ctrlKey: true })
    fireEvent.pointerCancel(surface, { pointerId: 72 })
    // 取消才需要主动清除；不能为了保留提交画面而破坏取消语义。
