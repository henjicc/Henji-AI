...
@fragment fn fs_main(@builtin(position) position: vec4f) -> @location(0) vec4f { let m = params.inverseLinear; let point = vec2f(m.x * position.x + m.z * position.y + params.inverseTranslation.x, m.y * position.x + m.w * position.y + params.inverseTranslation.y) - vec2f(0.5); return sampleColor(point) * (params.options.x * sampleMask(position.xy - vec2f(0.5))); }
