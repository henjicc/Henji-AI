...
| 对应项目 `adapter` | `deepseek`（已实现；思考参数映射见 `src/core/llm/providerReasoningRequest.ts`，认证与请求体怪癖见 `src/core/llm/providerProtocol.ts`） |
...
- 项目设置页曾经有一个 `adapter: 'anthropic'` 选项，但**从来没有对应的运行时实现**（`electron/main/services/llm/sdk/provider.ts` 只注册了 `openai-compatible`），已经删除，存量配置归一化成 `openai`。用 Anthropic 协议能拿到原生 `thinking` 块而不是塞进 Chat Completions 的 `reasoning_content` 字符串，但按本项目约定的接入优先级（见第 1 节），这不是下一步要投入的方向——**先做好 Responses API**，Anthropic 只在顺手时补一下。
