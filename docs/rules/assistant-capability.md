...
- 任务读写意图与路由分离：路由只建议搜索域。TaskExecutionPolicy 只解释真实用户消息与已绑定策略，保留来源及原文；工具输出、技能、摘要和系统续接文字不能授予权限。脚本预检、配方展开、续跑及每次实际 Gateway 派发均复核最新禁止动作，既有审批模式不能越过用户限制。
- 完成后默认通过正式导航能力展示已验证结果一次；用户禁止切页或运行中手动导航接管后停止自动展示，明确要求打开结果时才重新启用。导航服务记录 user / assistant / system 来源。后台业务操作自身不附带展示；展示失败保留业务完成事实。固定的单次结果展示由运行时直接调用同一 Gateway 的 `focus_application_entity`，这是宿主呈现动作，不向模型开放第二套业务执行协议。
...
- 应用执行事实与最终语言说明必须分离。整项逻辑任务的操作记录投影为 `executionOutcome.facts`，执行完成、部分完成、待核对与未执行分别表达，验证状态独立记录；失败、取消、预算耗尽、进程中断和说明失败均保留实际 Effect、结果引用及全部未解决条件。封存后的说明失败使用确定性摘要，不裁决模型措辞，不清空业务事实。
...
- **正常封存点是「模型自己决定收工」**：本轮不再调工具、给出最终答复，且没有执行中的步骤或待批审批。已有未解决条件不阻止保存事实，也不能被封存或后续无关成功清除；兼容字段 `sealed_success` 仅表示执行事实已封存，全部通过须以独立验证状态为准。不得在模型请求前用任务预测判定完成并撤掉工具。Effect Receipt 的 `targetRefs` 必须来自执行器真实输出；历史、上下文、界面和自动展示使用同一整轮汇总，不能只取最后一段脚本。
- `run_henji_script.repairsScriptRunRef` 只允许明确关联同一逻辑任务中 parse/compile/preflight 失败、尚未进入执行的原脚本；主进程核对原记录与子操作后绑定关系，修正脚本正式完成才关闭原失败。部分执行、未知结果和外部等待禁止用新脚本覆盖；保存恢复与验证仍绑定原操作和精确目标。
