...
| **L-B 剧本 harness** | `createScriptedModelStepExecutor` 驱动真运行时，唯一替身是 LLM | 十几秒、零成本 | 如果模型这么做，运行时对不对 |
...
- **（第 16 条）改域清单、能力目录或发现投影时，静态不变量必须全域穷举，不许手挑场景。** 手挑守的是「我想到的那几个域没问题」，而问题是注册数据的函数：某个域多注册两条能力、某条 schema 深一层就可能越线，没被挑中的域没有任何东西盯着它。三条现成的：域非空与可路由（`capability-domain-coverage.test.ts`）、投影体积与深度（`capability-discovery-size.test.ts`）、评测用例点名的工具真实存在（`regression-cases.test.ts`）。**门禁必须与生产用同一把尺子**——阈值、档位一律按工具名从生产函数解析，写死常量会造出假红假绿，实测两处都发生过。
- **（第 17 条）改拒绝路径、实体读写落地或脚本执行链路时，跑 `npm run test:assistant-harness`。** 这层用剧本驱动真运行时（真注册表、真 Gateway、真解释器、真领域执行器、真 zustand），唯一替身是 LLM。替身只允许出现在进程边界和外部付费/像素边界上；schema、权限、revision、动态 availability、执行器、真相源一旦出现替身，这层立刻变成漂亮的假绿。判断方法：把替身撤掉后，被测行为的**判断逻辑**有没有任何一条搬进了替身里。harness 自身必须有防空转自检（注册工具数、前端能力数下限）与保真锚点（写域用例断言对应 scope 的 revision 真的推进了）——实测最初版本忘了 `retainHostContextTracking()`，所有 revision 停在 0，整层对乐观并发失明还一路绿灯。
