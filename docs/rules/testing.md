...
- 每条写结果场景还必须对账 Effect Receipt：正式状态源中的每项持久差异都有 receipt，每项 receipt 都能在最终状态中找到对应变化；`evidence.data.applicationEffect` 等文本标记不得推动结算。
...
- 跨领域结果传递必须同时覆盖：自然语言任务图保留上游 Facet、下游 Facet 依赖上游、稳定结果引用的桥梁能力进入租约、桥梁调用正式领域写入并从真相源读回。领域名词的同义表达（如“海报”未附带“图片”）至少保留一条原话回归。
...
- 真实助手矩阵统一走 `npm run assistant:live:suite`；每个场景检查一次 `run_henji_script`、强类型 Effect、正式验证和成功封存。非外部任务主模型最多 4 回合；生成场景必须带 `--await-generation` 并验证断点续跑后的正式画布状态。
