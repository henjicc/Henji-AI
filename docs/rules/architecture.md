...
**领域归属与执行线程分开决定。** 移除当前界面后仍然成立、需要后台执行或被其他界面复用的逻辑，放正式领域服务或独立核心模块，不因组件调用方便而复制业务实现。

- 界面层：展示、交互、轻量视图状态与即时反馈，不承载可复用业务真相。
- 主进程：协调文件/数据库/网络 I/O、系统权限、任务生命周期与 IPC，不直接运行阻塞事件循环的重计算。
- Worker / utility process：执行 CPU 密集或需要隔离的解码、计算与批处理；线程选择不改变领域契约和唯一状态源。
- GPU：适合并行的像素、合成与效果计算按真实规模实测选用，保留与设备能力匹配的回退，不默认所有计算都上 GPU。

其他接入说明中的“主进程负责”指任务协调归属；具体计算位置以本节为准，不表示在主进程主线程执行。

## 复杂功能开工门槛

新增复杂编辑、计算、跨域或长任务前，先在当前任务说明中回答以下问题；复用既有计划或交接，不要求新建文档。普通叶子修复不机械套用。

- **真实规模**：典型与上限输入、并发、像素/帧数、内存和响应预算是什么？用真实规模样本验证，不拿微型夹具代替。
- **技术选型**：现有能力能否复用？CPU/GPU/Worker/主进程的取舍有什么官方依据或实测？不因界面实现方便决定计算架构，也不默认所有问题都上 GPU。
- **唯一状态源**：业务真相、视图和瞬态状态分别归谁？UI、助手、后台任务是否委托同一领域入口？
- **异步落盘**：何时只是内存变化，何时才是保存确认？关闭、切换和退出等待哪个屏障？失败如何保留修改？
- **取消与恢复**：旧任务、旧资源、设备丢失和部分失败如何隔离？恢复不能重放已经完成的业务操作。
- **验证边界**：哪些结果可由精确测试证明，哪些必须在真实容器或设备验证？先确定可判定的验收结果，再扩展实现。
...
- 正式领域服务是持久业务逻辑唯一入口；UI、Pi 和 MCP 适配器只能委托同一服务，不得各自维护第二份 schema、校验、状态机或写入逻辑。
- `ApplicationCapabilityDefinition` 是应用语义能力元数据唯一来源；禁止手写第二份 Agent 工具描述、旧 `HostCommand`/`HostQuery` 执行表或跨阶段兼容执行入口。
- AI 输入必须是封闭 schema，禁止任意 Store Patch、任意 JavaScript/TypeScript 执行和原始路径参数。多项实体修改使用现有通用事务，算法操作使用正式领域能力，不再引入脚本执行链。
- 公开实体、属性、语义操作、Surface、模型/媒体模态或长任务发生变化时，必须更新真实注册源并通过 `npm run check:assistant-capabilities`，不得只修改助手提示词。

- 跨进程领域声明放在 `core/application-control/domains/`，只依赖纯契约；反射与执行器归各领域，渲染层装配只做组合和依赖注入。
- `electron/main/services/application-runtime/` 先于 Pi／MCP 启动，协调调用者身份、账本、费用、保存恢复与宿主桥。关闭 MCP 不得终止 Pi、业务实例或已提交任务。
- 工程／文档实例持有业务状态、撤销和保存，页面只订阅或附着。普通内容修改显式绑定目标，禁止用当前页面决定后台任务归属。

## 文件体积与职责

- 不设统一行数门槛；文件长短只是审查信号，不是质量结论或验收条件。先判断读者能否理解其职责、变更影响和验证方式。
- 出现以下情况时评估拆分：混入彼此独立的变更原因；跨越 UI、领域、I/O 或线程边界；某部分难以独立测试；修改局部功能需要理解大量无关状态或引入无关依赖。即使文件很短，也不能因此忽略这些问题。
- 拆分应形成名称明确、接口稳定、可以独立理解或验证的单元，并实际降低依赖与状态耦合。若只是按行切块、增加透传包装或让多个文件相互传递大量共享可变状态，应先理清职责，而不是继续拆文件。
- 内聚的 schema、声明式配置、着色器、测试用例矩阵等可以较长；生成文件应修改生成源，不手工拆分产物。这不豁免跨层边界或业务逻辑审查。
- 局部缺陷修复只处理相关职责，不因文件较长扩大成无关重构；新增功能前先确认是否应进入已有模块或形成独立职责。禁止用压行、删解释性注释或隐藏复杂度代替结构改进。
...
- 禁止新增裸 `any`（存量可留，增量为零）；未知边界使用 `unknown` 并校验或收窄类型，注释不能豁免此要求。
- 新增/修改的导出函数补齐显式返回类型
