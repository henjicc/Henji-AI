...
- 只要模型存在产品级渠道切换，参数名称统一显示为“渠道”，并且必须在 schema 里显式声明 `role: 'channel'`。生成面板（`parameterOrder.ts`）只认这个 `role` 来决定「主选择器提前渲染」，不再按参数名文案猜测；漏写不会报错、只会让渠道掉回普通排序静默排到分辨率后面，因此 `modelParamConventionValidator` 会在模型注册时拦下漏写。
- 渠道恰好两档时，选项统一为“普通 / 官方”，schema 使用 `sharedFieldText('apiChannel')` 与 `sharedOptionText('regular' | 'official')`；渠道多于两档时共享词表无法表达，改用自定义标签（字段名仍写“渠道”，保持跨模型术语一致），选项由模型自定义，此时校验器不再要求“普通 / 官方”两档。
- 渠道参数的 `order` 必须严格小于所有其他参数，始终排在分辨率、比例、模式等参数之前。这条规则优先于“模式优先”。模式 / 版本 / 变体等主选择器则声明 `role: 'mode'`，同样会被提到比例分辨率面板之前，但不要求 `order` 为 1。音频的“声道”不是产品渠道，不适用本条，也不要声明 `role`。
