...
## 当前方向：外部能力开放（2026-09-12）

旧助手完善计划暂停；本项目默认隐藏侧栏、快捷键和诊断唤起，源码与历史保留。应用独立执行入口在 `src/features/application-control/applicationCapabilityService.ts`，使用宿主授权、原注册表与领域服务。MCP 已接入本地读取、设置／模型可见性／素材／画布受控写入及独立 SQLite 操作记录；证据见 `docs/task/外部智能体能力开放/任务/第二阶段-可靠写入与后台任务/2.1-交接记录.md`。未知操作保持阻断，不能自动重放；后续写域尚未完成。下文历史助手验收不作为 MCP 完成证明。
