...
旧助手完善计划暂停；本项目默认隐藏侧栏、快捷键和诊断唤起，源码与历史保留。应用独立执行入口在 `src/features/application-control/applicationCapabilityService.ts`，使用宿主授权、原注册表与领域服务。旧根桥仍保留协议兼容，新的 MCP 连接在能力开放计划 1.2 接入；下文历史助手验收不作为 MCP 完成证明。
