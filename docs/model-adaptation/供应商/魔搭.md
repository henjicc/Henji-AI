...
该常量在仓库里有**三份副本**，改动必须同时覆盖，否则静默分叉：

| 位置 | 作用 |
|---|---|
| `src/models/modelscope/utils.ts` | 源码常量 |
| `scripts/generate-model-manifest.cjs` 的 `KNOWN_ENDPOINT_CONSTANTS` | 跨文件 import 解析不到，manifest 生成时取这份 |
| `electron/main/services/ai-runtime/js-runtime.ts` 的 `JS_PRELUDE` | builderJs 在 VM 里实际执行的那份 |

`src/models/modelscope/utils.test.ts` 有三处一致性的回归用例。
...
请求体里图片来源有三个键，builder 必须都认：生成页提交是 `uploadedFilePaths`，画布节点是 `images`，旧工程可能直接存 `image_url`。JS_PRELUDE 里的副本曾漏掉 `uploadedFilePaths`，导致生成页上传的图被静默丢弃。
...
- 支持 → 返回 `Providers` 数组（含档位与魔粒消耗）
- 不支持 → 返回 `Providers: null, TotalCount: 0`

另有 `GET /api/v1/inference/<ns>/<name>/api_inference_example` 返回官方示例代码与 `InferenceType`。

> **自定义模型入口必须做这个校验。** API-Inference 只覆盖魔搭挑选的一个子集（依据点赞/下载热度），官方明确说明「随着新模型的推出，比较早的模型可能逐渐从 API-Inference 下架」。已实测不支持的热门模型包括 `black-forest-labs/FLUX.1-dev`、`FLUX.1-Kontext-dev`、`Tencent-Hunyuan/HunyuanImage-3.0`、`stabilityai/stable-diffusion-3.5-large`、`HiDream-ai/HiDream-O1-Image`。当前项目的自定义入口不做校验，用户填任意 ID 都能提交但必然失败。
...
- 计价不能用货币表达，应按魔粒档位展示。
- 自定义模型入口需要在提交前调 `list_model_providers` 校验可用性。
