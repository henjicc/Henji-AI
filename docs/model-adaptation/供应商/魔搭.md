...
本项目 `src/models/modelscope/utils.ts` 里的 `MODELSCOPE_CREATE_TASK_ENDPOINT` 是 `/api/v1/jobs/createTask`，**这是 KIE 的路由格式**。魔搭官方文档与每一个模型页自动生成的示例代码统一都是 `POST /v1/images/generations`，无一例外。

该差异**尚未用真实 token 实测确认**（探测请求被网络层拦截，拿不到 HTTP 状态码）。接入或排障时优先核对这一项——如果官方路由才是对的，当前全部 7 个魔搭模型都是不可用状态。
...
> 项目当前在 `utils.ts` 里对所有模型统一用 `calculateResolutionWithBounds(..., 64, 2048)`，**没有按模型分档**。FLUX 超过 1024 会被拒，Z-Image-Turbo 低于 512 不合法。
...
> 项目当前 7 个魔搭模型全部硬编码 `¥0.12/次`，与魔粒计费模型完全不符，需要重新设计这一供应商的计价展示方式。
