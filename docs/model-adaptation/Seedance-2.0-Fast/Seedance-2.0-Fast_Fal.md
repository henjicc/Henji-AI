...
| 模态 | 视频 |
| 供应商 | fal.ai（聚合平台） |
| 平台模型 ID | `bytedance/seedance-2.0/fast/text-to-video`、`bytedance/seedance-2.0/fast/image-to-video`、`bytedance/seedance-2.0/fast/reference-to-video` |
| 接口形态 | 队列异步（推荐）或同步直连 |
| 文档可见性 | 公开，无需登录 |
| 价格可见性 | 公开，无需登录 |
...
| 能力 | endpoint id |
|---|---|
| 文生视频 | `bytedance/seedance-2.0/fast/text-to-video` |
| 图生视频（首帧，可选尾帧） | `bytedance/seedance-2.0/fast/image-to-video` |
| 参考生视频（图 / 视频 / 音频多模态） | `bytedance/seedance-2.0/fast/reference-to-video` |
...
|---|---|
| 720p（按秒） | **$0.2419 / 秒** |
| token 价 | **$0.0112 / 1000 tokens** |

token 数计算：`(输出视频高 × 输出视频宽 × 时长 × 24) / 1024`。

`reference-to-video` 有视频输入时：token 数按 `(高 × 宽 × (输入视频时长 + 输出视频时长) × 24) / 1024` 计算，且**价格 × 0.6**——有视频输入 + 720p 时约 **$0.14515 / 秒**。
...
| 信息 | 链接 | 是否需登录 |
|---|---|---|
| 文生视频 schema + 价格 | https://fal.ai/models/bytedance/seedance-2.0/fast/text-to-video/llms.txt | 否 |
| 图生视频 schema + 价格 | https://fal.ai/models/bytedance/seedance-2.0/fast/image-to-video/llms.txt | 否 |
| 参考生视频 schema + 价格 | https://fal.ai/models/bytedance/seedance-2.0/fast/reference-to-video/llms.txt | 否 |
