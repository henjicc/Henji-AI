...
- **提交**：`POST /api/v1/jobs/createTask`，体为 `{ model: "bytedance/seedance-2", callBackUrl?, input }`
- **查询**：`GET /api/v1/jobs/recordInfo?taskId=...`
- **终态**：`state` ∈ `waiting` / `queuing` / `generating` / `success` / `fail`
- **结果**：`JSON.parse(resultJson)` → `{ resultUrls: [...] }`；**开启 `return_last_frame` 时结构为 `{ resultUrls: [], firstFrameUrl: [], lastFrameUrl: [] }`**（KIE 通用任务详情文档明确说明）
...
| `input.reference_video_urls` | array | 可选 | — | 参考视频，**最多 3 个**。格式 mp4/mov；分辨率 480p/720p；单个时长 [2, 15] s 且总时长 ≤ 15 s；宽高比 [0.4, 2.5]；边长 [300, 6000] px；总像素 [409600 (640×640), 927408 (834×1112)]；单个 ≤ 50 MB；帧率 [24, 60] |
| `input.reference_audio_urls` | array | 可选 | — | 参考音频，**最多 3 段**。格式 wav/mp3；单个时长 [2, 15] s 且总时长 ≤ 15 s；单个 ≤ 15 MB |
| `input.return_last_frame` | boolean | 可选 | `false` | 是否返回视频最后一帧图片 |
| `input.generate_audio` | boolean | 可选 | **`true`** | 是否生成与画面同步的音频 |
| `input.resolution` | string | 可选 | `720p` | `480p` / `720p` / `1080p` / `4k` |
...
|---|---|---|---|---|
| 480p no video input | 19 /秒 | **$0.095/秒** | $0.1406 | 32.4% |
| 480p with video input | 11.5 /秒 | **$0.057/秒** | $0.0844 | 32.5% |
| 720p no video input | 41 /秒 | **$0.205/秒** | $0.3024 | 32.2% |
| 720p with video input | 25 /秒 | **$0.125/秒** | $0.1814 | 31.1% |
| 1080p no video input | 102 /秒 | **$0.51/秒** | $0.6804 | 25.0% |
| 1080p with video input | 62 /秒 | **$0.31/秒** | $0.4082 | 24.1% |
| 4K no video input | 208 /秒 | **$1.04/秒** | $1.56 | 33.3% |
| 4K with video input | 128 /秒 | **$0.64/秒** | $0.93 | 31.2% |
...
- `return_last_frame` 会改变 `resultJson` 的结构，解析逻辑必须分支处理。
- `resolution` 档位是 480p / 720p / 1080p / 4k。
...
| 信息 | 链接 | 是否需登录 |
|---|---|---|
| Seedance 2.0 文档 | https://docs.kie.ai/cn/market/bytedance/seedance-2 | 否 |
| 获取任务详情（含 return_last_frame 的结果结构） | https://docs.kie.ai/cn/market/common/get-task-detail | 否 |
