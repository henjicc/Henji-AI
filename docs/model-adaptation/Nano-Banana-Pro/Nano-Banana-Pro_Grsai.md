...
| 项目当前状态 | 已接入，[src/models/grsai/nano-banana-pro.model.ts](../../../src/models/grsai/nano-banana-pro.model.ts)，`canonicalModelId: nano-banana-pro` |
...
- 五个渠道共享同一套请求/响应结构，做成模型内的「渠道」下拉；当前项目的渠道文案约定（`sharedOptionText('regular' | 'official')`）只覆盖两档，这里有 5 个渠道，代码没有套用那套共享词表，用了自定义标签「标准 / VT（备用线路）/ CL·1K / VIP / VIP·4K」。
