...
> 与 Nano Banana 2 类似，Pro 也拆成 5 个渠道，价差可达 **10 倍**（`nano-banana-pro` ¥0.09 起 vs `nano-banana-pro-4k-vip` 最高 ¥1.8）。特别之处：`nano-banana-pro` 与 `nano-banana-pro-vt` 的积分消耗、价格区间、支持分辨率**完全相同**，大概率是两条互为备份的后端线路，而不是能力差异——这点与 Nano Banana 2 的 `-lite`/`-fast` 重复现象类似，都需要接入前实测澄清。
...
结构、状态枚举、结果链接有效期等公共部分见 [Grsai 基础文档](../供应商/Grsai.md) 第 3、7 节，五个渠道共用同一套响应格式，不在本文件重复。
...
- 五个渠道共享同一套请求/响应结构，适合做成模型内的「渠道」下拉；当前项目的渠道文案约定（`sharedOptionText('regular' | 'official')`）只覆盖两档，这里有 5 个渠道，需要先跟用户确认渠道选项的文案与数量如何呈现，再写 schema。
- `nano-banana-pro` 与 `nano-banana-pro-vt` 规格完全一致，接入前需要实测确认二者是否真的等价（例如只是负载均衡/线路冗余），避免在 UI 上重复暴露两个无差异的选项。
- `cl` 渠道的分辨率支持范围历史上被官方收窄过一次，接入时不要硬编码「支持所有分辨率」，要以当前 dashboard 页面的实时标签为准。
- Pro 系列是否支持极端比例（`1:4`/`4:1`/`1:8`/`8:1`）文档未明确排除，需要实测。
