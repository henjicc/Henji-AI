...
> Henji 产品中基础图片生成合并为一个“Grok Imagine Image 2.0”入口，通过顶层“接口渠道”切换；图层与选区编辑仍属于依赖结果卡片的独立连续工作流，不塞入普通生成参数。
...
- **两条渠道能力互补**：基础图片生成可以合成一个产品入口，但参数显隐、输入限制、请求和计价必须随“接口渠道”严格分支；EXT 只能文生图但另有图层/选区编辑，官方渠道能图生图和多图参考但没有 segment。
