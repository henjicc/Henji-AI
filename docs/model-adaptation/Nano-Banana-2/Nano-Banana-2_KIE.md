...
| 模态 | 图片 |
| 供应商 | KIE.ai（聚合平台） |
| 平台模型 ID | `nano-banana-2`、`nano-banana-2-lite` |
...
| 模型 | 说明 |
|---|---|
| `nano-banana-2` | 文生图 + 图生图合一（有无 `image_input` 都走同一个 model） |
| `nano-banana-2-lite` | 轻量版，文生图 + 图生图合一 |

KIE **没有把文生图/图生图拆成两个 model**，这与它家 Seedream / Qwen 的做法不同。
...
### 3.2 `nano-banana-2-lite`

| 字段 | 类型 | 必填 | 默认 | 取值与说明 |
|---|---|---|---|---|
| `input.prompt` | string | 必填 | — | 最多 20000 字符 |
| `input.aspect_ratio` | string | 必填 | `auto` | 同上 15 个取值 |
| `input.image_urls` | array | 可选 | `[]` | **最多 10 张**（主模型是 14 张）。字段名是 **`image_urls`**（与主模型的 `image_input` 不同）。纯文生图可传空数组或不传。支持 `image/jpeg`、`image/png`、`image/webp`；单张 ≤ 30 MB |

Lite **没有 `resolution`、没有 `output_format`**。
...
定价页未单独列出 `nano-banana-2-lite` 条目。

## 6. 适配要点

- 本项目默认**绝对不显示**：`seed`、负面提示词。KIE 本接口两个字段都没有。
- `output_format` 默认不显示、不请求；注意取值是 `jpg`（不是 `jpeg`），且默认 `jpg` 与其他供应商的 `png` 不同。
- **主模型与 Lite 的图片字段名不同**（`image_input` vs `image_urls`），数量上限也不同（14 vs 10），非常容易写错。
- 比例集合含 4 个极端比例（1:4 / 4:1 / 1:8 / 8:1），与 APIMart 主模型一致、与 APIMart Lite 不一致。
