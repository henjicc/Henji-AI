...
- 当前阶段：第四阶段 · 智能助手侧边栏
- 状态：4.1 两轮性能反馈修复与 4.1～4.3 自动化验证完成，待真实 Electron 手动复验
...
2. 左停靠拖右边缘、右停靠拖左边缘，确认拖动期间面板以合成层平滑预览、工作区不连续重排，松手后最终宽度与工作区避让一次性对齐。
3. 悬浮态分别拖右边缘、下边缘和右下角，确认宽、高、双向预览流畅，松手后内容按最终尺寸清晰重排且边界限制正确。
...
- 4.1 几何与合成层预览锚点已由单测覆盖，代码路径已消除逐帧根级样式失效和工作区布局；鼠标跟手性、缩放期间内容视觉和不同 DPI 下的实际感受仍需用户在真实 Electron 窗口确认。
