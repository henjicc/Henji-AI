...
### D-024 语义理解、工具权限与批准方式严格分层

- 本决定替代 D-023 的“明确媒体表达优先走本地关键词规则”：除任务 ID、工作区等稳定协议标识构成的快捷命令外，自然语言生成、诊断、画布和长期偏好均由 router 模型按完整语义分类；不得通过继续扩张关键词表弥补模型判断。
- router 只决定 intent、complexity 和 reason；path/toolDomains 继续由本地权威映射，是否允许执行只由工具 schema、risk、data class、宿主 revision 和批准方式决定，路由来源不再被当作授权依据。
- 批准方式固定为 `ask / assistant_decides / full_access`。每次询问保留逐次审批；助手判断自动执行 R0/R1 和只读非破坏 R2；充分访问自动执行 R0～R2。R3 在三种方式下均需确认，R4 始终禁止。
- 展示层只提供可核验的“执行思路/计划摘要”，不暴露不可验证的隐藏思维链；工具轨迹默认单行折叠，最终答复优先给结论与可执行方案。

### D-025 Provider 网络重试必须证明尚未产生副作用

- KIE 的 POST/GET 统一保留 Undici/Node 底层错误码，用户可见错误使用脱敏后的稳定网络码，不记录密钥、授权头或完整请求体。
...
- 能力接入以现有 ModelRegistry、GenerationService、项目/画布应用动作和领域 service/store 为事实源；Agent adapter 不复制业务编排，不读取 DOM，不模拟鼠标。
- “全面接入”表示所有已有且具备稳定语义入口的能力均可按需发现；仍只有瞬时 UI 状态、缺少稳定对象 ID 或没有可撤销/补偿边界的操作，先补领域入口或明确保留为手动能力，不伪造空工具。
- 全量 schema、画布 JSON 和媒体内容不常驻上下文；目录返回摘要，详情按稳定 ID 查询，大媒体只传 ref。
- 跨工作区工作流仅收录可证明顺序、审批点和失败补偿的高频流程；模型选择流程并填语义参数，执行顺序由代码持有。

### D-029 第七阶段工具注册必须与宿主目录一一对应

- 3D 工程/对象、图片编辑和分镜能力虽然已有 HostCommand/HostQuery，但只有进入 utility 侧权威 AgentToolRegistry 后才算可发现能力；禁止只在目录字符串中声明而没有可执行工具。
- 主进程不重复注册工作流工具；utility 组合 frontend proxy 工具与确定性 workflow 工具，跨进程子步骤统一经 gateway RPC 回到 main 的宿主边界。

### D-030 大领域查询只返回受限摘要，结构化编辑入口共享契约

- 3D/分镜项目详情禁止把 `sceneJson/nodesJson/edgesJson` 直接送入 Agent；返回稳定 ID、计数、必要元数据和最多 32 项受限详情，完整数据仍由领域服务和 UI 持有。
- 图片编辑操作使用共享 `imageEditContracts` discriminated union 校验标记类型、几何、样式、裁剪边界；preview 不覆盖原图，commit 只生成新素材并按风险审批。

### D-031 素材跨工作区引用必须传递真实稳定 ID

- 日期：2026-07-24
- 状态：已实现，待真机验证
- 背景：`asset_edit_to_canvas` 如果只创建空上传节点，工作流虽然结束但画布没有真实素材引用，后续生成和编辑无法复用产物。
- 最终决定：素材编辑提交步骤返回的 `assetId` 必须通过 `add_asset_to_canvas` 传给画布宿主动作；画布节点数据只保存稳定素材引用，不复制大媒体内容。
- 选择原因：保持跨工作区流程的引用完整性、渐进式上下文和领域边界；失败时可按步骤补偿，不以空节点伪造成功。
- 影响任务：7.3、7.4、8.1、8.2。

### D-032 手动验收前冻结代码范围，不提前伪造生产化结论

- 日期：2026-07-24
- 状态：已确认
- 最终决定：1～7 阶段实现代码、自动化测试、静态检查和构建入口完成后，统一交给用户执行真实 Electron/Provider/鼠标/日志/长稳验收；第八阶段 8.1/8.2 在获得真实数据后再启动。
- 选择原因：真实 Provider 费用、随机性、窗口交互和长稳不能由 mock 或静态检查替代；保持任务文件中的“待手动验收”不等于缺少业务代码。
- 影响任务：4.1～8.2。
