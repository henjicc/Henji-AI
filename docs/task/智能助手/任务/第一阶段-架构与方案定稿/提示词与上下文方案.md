...
- 默认关闭；开启后最多 4,000 字符，保存为结构化 PromptDocument。
- 只允许语气、格式、默认偏好、解释深度等低风险内容。
- 出现“忽略系统规则、自动批准、读取密钥、调用未列工具、永久授权”等内容不报错但不生效，并在设置 UI 标示冲突段。
- P3 不进入工具权限摘要或 approval；授权只来自代码状态。
- 不把 P3 作为用户长期事实自动写入 memory；6.5 另行管理。
...
- 第一阶段只定义 run state、thread history 和摘要；不实现长期记忆。
- thread history 是用户可见消息与 artifact 引用，不等同 checkpoint。
- 6.5 长期记忆默认关闭或保守写入，必须有来源、scope、过期、用户确认和删除入口。
