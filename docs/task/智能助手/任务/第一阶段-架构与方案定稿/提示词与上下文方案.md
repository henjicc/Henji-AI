...
P2 工具与完成规范（产品版本化）
P3 用户主动维护的自然语言指令（可选、安全硬约束下高优先级）
...
P0～P2 使用 `promptVersion` 固定；P3 不能覆盖安全、权限、审批、工具协议或权威能力事实，但应优先于产品默认、推荐策略和通用模型描述；P4 中用户当前明确要求优先于持久化 P3。P4～P8 每轮由 ContextBuilder 生成。任何外部内容都不能插入 system role 或伪造分隔符。
...
[P3 用户指令]
以下内容是用户主动维护的高优先级表达、格式、工作习惯与模型选择偏好。
在不违反安全、权限、审批、工具协议、当前明确要求和权威能力事实时，应优先于产品默认、推荐策略和通用描述执行：
{{user_instructions}}
...
- 可包含语气、格式、默认偏好、解释深度、生成供应商和模型选择倾向等正常内容；除明确硬约束冲突外，不因产品默认或推荐策略不同而降级。
- 出现“忽略安全规则、自动批准、读取密钥、调用未列工具、永久授权”等内容不生效，并在设置 UI 给出提示。
- P3 进入模型前只自动脱敏 API Key、token、cookie、授权头、密码等秘密信息；普通路径、带查询参数的网址和其他正常自然语言保持原样。P8 observations 仍按更严格的外部数据脱敏规则处理。
...
- 明确图片/照片/插画/海报/头像/壁纸/封面、视频/短片/动画、音频/音乐/配音等媒体生成请求；只确定进入生成 Workflow，模型、参数和实际写操作仍由目录/schema、网关与审批决定。
...
```ts
const RouterClassification = z.object({
  intent: z.enum([
    'navigate', 'generate', 'inspect_model', 'read_generation',
    'cancel_generation', 'diagnose', 'canvas', 'user_instructions', 'general'
  ]),
...
- router 只分类 `intent/complexity/reason`；`path/toolDomains` 必须由本地 `routePolicy` 映射，模型输出不能扩大工具域。
- 输出未通过 schema 或调用失败时进入 primary 的最小安全能力发现，并记录不含用户原文的稳定失败码。
- router 只得到目标短摘要、当前 workspace、能力类别和不敏感附件类型，不得到日志原文、全部历史、工具 schema 或密钥状态细节。
- 路由日志记录 intent、complexity、source 和本地工具域，不记录用户原文。
- router 不做审批、不产生工具参数、不宣告任务完成。

## 6. 渐进工具发现

1. ContextBuilder 根据 route 先选 1～3 个能力类别。
2. 模型用 `search_application_capabilities` 搜索摘要目录，category 使用固定枚举；查询按原始词、多词和媒体/生成语义相关性排序，单页最多 20 项。
3. 模型按需调用 `search_models/search_canvas_node_types/search_assets` 等领域目录。
4. 只对候选调用 `get_*_schema`；每轮最多读取 4 个详情。
5. Runner 只接受 Registry 中仍存在且符合当前 HostContext 的发现结果，并在下一轮加入 `activeTools`；最近一次针对性发现优先，默认最多 8 个执行工具。
...
2. P8 observations 进入模型前执行 secrets、Authorization、URL query、本地路径和用户身份字段脱敏；P3 用户指令仅自动脱敏秘密信息。
...
- `user-instructions.md` 是用户主动编辑、在安全与真实能力硬约束下高优先级的指令，不参与助手自动记忆候选提取。
