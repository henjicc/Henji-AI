...
- 验证日期：2026-07-23
- 验证范围：用户指令契约、Markdown 持久化/旧偏好迁移、IPC/PAL、Agent 工具/路由、低信任上下文注入、设置页和 Electron 构建。

### 已执行检查

| 检查 | 结果 |
|---|---|
| 用户指令与上下文定向测试 | 通过；2 个文件、13 个用例 |
...
- 用户指令以明确不可信边界注入，不进入 system role；凭据形态在进入模型前脱敏。
- 模型能力硬约束、当前请求优先级、中央描述排序、最多 8 个 active tools、历史压缩和大 observation offload 保持有效。

### 待用户手动验证

- 在“助手用户指令”输入多行自然语言，保存、重新读取、打开外部 Markdown、清空，确认 UI 与文件同源。
- 若本地已有旧 `model-preferences.json`，首次打开时确认内容被转成可读自然语言，旧文件仍保留。
- 对助手说“以后图片优先 PPIO，回答简洁”，批准 R2 更新；下一次 run 确认指令生效，当前明确要求仍能覆盖。
