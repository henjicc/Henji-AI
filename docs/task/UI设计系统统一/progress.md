...
### 收尾 — 选项集合静息态收敛（用户发起，不在原计划内）

用户对着三张截图（模型选择网格、提示词优化配置菜单、任务卡图标按钮行）提出：
这些地方当初是把它们当"按钮"设计的，所以给了边框背景；但鼠标没停留、也不是选中项的时候，
是不是不该有这层框？
...
| 步 | 提交 | 内容 |
|---|---|---|
| 1 | `067087a` | 新增 `UiOptionButton` 的 `menu` 变体；先只改用户截图的三处 |
| 2 | `a5253f2` | 按判据推广到其余调用点；规则写入 skill；补齐第二三阶段手动验证清单 |

第二步覆盖 22 个调用点 / 15 个文件，另有 12 个调用点**刻意保留**边框（判据与清单见 D-009）。

同时发现并补齐一个记录缺口：`test-report.md` 此前只覆盖第一阶段，
第二、三阶段的手动验证项散在各任务文件里没有汇总。现已补为 A~N 共 14 组。
