...
### 收尾续二 — 固定调色板收敛（用户："其他地方也统一按这个风格"）

| 项 | 数量 |
|---|---|
| `zinc-*` 硬编码 | 292 → **0**（66 个文件） |
| 死的 `dark:` 双分支 | 15 → 0 |
| 新增语义色档位 | `text-soft` / `text-faint`（由主题派生，非新增用户设置） |
| 被暴露出的存量表面违规 | 9 处，已全部修掉 |
| 顺带修好的静默失效 | 上传区拖拽高亮（从未生效） |
| 新增门禁 | ESLint 拦截 `*-zinc-*`，已反向验证 |

关键认识：这轮的驱动力不是美观，是**主题预设失效**——用户切换主题时硬编码颜色会原地不动。
和此前「节点选中描边不跟随强调色」是同一类缺陷，只是面更大。

skill 与 `CLAUDE.md` 同步新增两条长期规则：「同属性叠类 = 静默失效」「颜色必须跟随主题」。
