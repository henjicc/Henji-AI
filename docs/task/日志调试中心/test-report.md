...
| `npm run gen:model-manifest` | 通过（61 个模型，本任务未改模型定义，仅确认脚本链路未被破坏） |
| `node -e "JSON.parse(...)"` 校验 `ui.json` 两份文件语法 | 通过 |
| 新增/修改文件裸 `any` 排查（逐一 review 本次全部改动文件） | 无命中，未新增裸 `any` |
| 新增文件行数检查（`query.ts` 233 行、`useLogHistoryQuery.ts` 171 行；修改后 `ipc/logging.ts` 140 行、`LogFilterToolbar.tsx` 222 行、`LogsPanel.tsx` 168 行、`LogEventList.tsx` 105 行） | 全部 ≤400 行，未触及"400~500 可接受"档 |

**主控复核后重新执行（2026-07-10）**：发现原 `beforeTimestamp` 分页在同一毫秒存在多条事件时会漏项，已改为 GUI 使用 `beforeLine`/`nextBeforeLine` 文件行号游标，并增强结构非法 JSON 行的跳过逻辑。修正后实际执行以下命令，均退出码 0：
...
未执行 `npm run electron:build` / `npm run electron:smoke`：任务文件"执行步骤"第 6 条只要求跑 `tsc electron` / `eslint electron` / `npm run lint` 三项，已全部覆盖；仓库现有 `out/` 构建产物早于本次改动（7 月 4 日），跑冒烟测试意义不大，且现有 `electron-phase4-smoke.cjs` 冒烟脚本本身不覆盖新增的历史日志查询 IPC（`logging:listDates`/`logging:query`），跑一次昂贵的 `electron:build` 换不来这部分覆盖率；本任务风险面（新增只读查询 IPC + preload 白名单 + 纯渲染层 UI 扩展）已被上述静态检查完整覆盖，交互与运行时行为验证与用户即将进行的 `electron:dev` 手动验证重合。

### 验收标准逐项对照

| 验收标准 | 状态 | 说明 |
|---|---|---|
| 重启应用后能在历史模式看到重启前的日志事件 | **待人工验证** | 见步骤 AD |
| 日期列表与实际存在的日志文件一致 | **待人工验证** | 见步骤 AE |
| 历史模式下过滤（级别/来源/domain/requestId/关键词）由主进程执行，大文件不整体传给渲染层（分页生效） | **待人工验证** | 见步骤 AF |
| 历史事件的详情、链路、复制功能可用 | **待人工验证** | 见步骤 AG |
| 文件中混入损坏行不导致查询失败 | **待人工验证** | 见步骤 AH |
| 类型/静态检查通过；任务总览已同步更新 | 已通过 | 见上表；任务总览同步见 `00-任务总览.md` |

以下步骤涉及启动真实 Electron 应用、重启应用、切换历史模式、手动构造测试文件，按项目约定交给用户执行，我没有自己上手操作。本次改动涉及主进程与 preload（新增 IPC 通道），**必须先重启** `npm run electron:dev`。

---

### 步骤 AD：验证"重启应用后仍能在历史模式看到之前的日志"

1. 重启 `npm run electron:dev`。
2. 在应用内正常触发几次操作（比如一次 LLM 对话、一次 AI 生成任务），确认日志窗口"实时"模式下能看到这些事件（复用 2.1/2.2 已验证过的能力）。
3. **完全退出应用**（不是刷新页面/关闭日志窗口，是整个应用退出，比如关闭主窗口触发 `window-all-closed` 后进程退出，或任务管理器确认进程已结束）。
4. 重新执行 `npm run electron:dev` 启动应用。
5. 打开日志窗口，切换到"历史"模式（工具栏最左侧模式切换按钮）。
6. 确认日期下拉框自动选中今天的日期，列表里能看到重启前触发的那几条日志事件（内容、时间戳与重启前一致）。

### 步骤 AE：验证日期列表与实际日志文件一致

1. 承接步骤 AD，打开系统文件管理器，进入 `%LOCALAPPDATA%\com.henji.ai\Henji-AI\logs\`，确认当前实际存在的 `henji-YYYY-MM-DD.log` 文件列表。
2. 对比日志窗口历史模式的日期下拉框选项，确认两者完全一致（数量、具体日期都对得上；由于保留策略是 1 天+总大小上限，正常情况下应该只有 1-2 个日期可选，见重要记录 005）。
3. 如果条件允许，可以手动在日志目录里临时复制一份现有文件并重命名为一个更早的日期（比如 `henji-2026-07-08.log`），重新打开日志窗口历史模式（或点击"实时/历史"来回切换一次触发重新拉取），确认新日期出现在下拉框里；测试完记得删除这个临时文件，避免污染真实日志目录。

### 步骤 AF：验证历史模式过滤下沉到主进程 + 分页生效

1. 承接步骤 AD/AE，切到历史模式，选中一个有较多日志的日期。
2. **过滤下沉验证**：分别测试来源（前端/后端）、级别（选一个具体级别比如 INFO）、domain（下拉框选一个选项）、关键词（输入一个已知会命中的词）四种过滤，确认列表结果符合预期收窄；这几种过滤组合起来测试（比如"后端"+"INFO"+关键词同时生效）。
3. **"只看错误"验证**：勾选"只看错误"，确认列表只剩 error 级别事件；再叠加一个具体级别下拉（比如选 WARN），确认列表变空（"只看错误"与 levelFilter 是"与"关系，不会静默改变 levelFilter 语义，见 `decisions.md`）。
4. **分页验证**：如果当天日志超过 200 条（可以先在实时模式下反复触发生成/对话攒够数量，也可以直接观察"加载更早的日志"按钮是否出现），点击"加载更早的日志"，确认能持续加载更旧的事件，直到没有更多为止（按钮消失）；加载过程中按钮应短暂显示"加载中..."。
   - 额外验证同毫秒边界：若能构造超过 200 条的同毫秒事件，连续翻页后确认每条只出现一次且没有遗漏；这是 `beforeLine` 行号游标覆盖的边界场景。
5. 打开浏览器开发者工具的 Network 面板（这里指日志窗口自己的 devtools，`Ctrl+Shift+I`），确认没有看到任何"一次性把整个日志文件内容传给渲染层"的异常大响应（IPC 调用本身不会出现在 Network 面板，可以改为观察内存占用或简单信任代码走查：`query.ts` 逐行流式处理 + 滚动缓冲区，不整文件进内存）。

### 步骤 AG：验证历史事件的详情/链路/复制功能

1. 承接步骤 AF，在历史模式列表中点击任意一条事件，确认右侧详情面板正常显示（JSON 折叠树可展开/折叠，行为与实时模式一致）。
2. 找一条带 `requestId` 的历史事件，点击"查看完整链路"，确认弹出的链路时间线里能看到该 requestId 下的全部关联事件（不局限于当前过滤条件），按时间升序排列。
3. 在链路弹层里点击"复制 Markdown"/"复制 JSON"，粘贴到文本编辑器，确认格式正确、内容与链路一致（与 2.2 步骤 AC 验证方式相同）。
4. 回到详情面板，点击"复制 Markdown"/"复制 JSON"，确认单条历史事件也能正常复制。
5. 也可以直接在工具栏 requestId 输入框粘贴一个 requestId 手动查询链路（不依赖点击列表行），确认效果一致。

### 步骤 AH：验证损坏行容错

1. 关闭应用。
2. 用文本编辑器打开当天的 `henji-YYYY-MM-DD.log`，在文件中间随便找一行，手动破坏它的 JSON 格式（比如删掉一个引号，或者插入一行乱码文本如 `this is not json`），保存文件。**建议先备份原文件，测试完恢复**，避免影响后续正常使用。
3. 重新启动 `npm run electron:dev`（如果只是要测试历史查询，其实不需要重启应用，因为查询是按需读文件；但为了确保测试环境干净，建议重启一次）。
4. 打开日志窗口，切到历史模式，选中被破坏的那个日期。
5. 确认：
   - 应用没有崩溃、没有报错弹窗；
   - 列表正常显示该文件里其余合法的日志行（顺序、内容不受影响）；
   - 被破坏的那一行不出现在列表里（被跳过），但也没有导致查询整体失败或列表为空。
6. 测试完成后，用备份恢复原始文件内容，避免真实日志文件长期带着人为损坏的行。

### 步骤 AI：历史模式与实时模式切换的基本可用性回归

1. 在日志窗口里来回切换"实时"/"历史"模式几次，确认：
   - 切到"历史"模式后暂停/清空按钮消失，换成日期下拉框；
   - 切回"实时"模式后日期下拉框消失，暂停/清空按钮恢复，且实时模式的数据（内存缓冲）不受历史模式操作影响（比如历史模式翻了几页，切回实时模式应该看到的还是正常的实时事件流，不应该被历史数据污染）；
   - 过滤条件（来源/级别/domain/关键词/只看错误）的选中状态在切换模式时保持不变（两种模式共用同一套过滤 UI 状态）。
2. 确认历史模式下选中日期后，再切换过滤条件（比如换一个级别），列表会重新查询并刷新（不是停留在旧结果不动）。
