...
### 决策：只规范增量 domain，不批量重命名存量

- 现有 domain 同时存在路径式、模块式与短横线形式，历史 JSONL 已使用这些值作为查询条件。
- 选择：新主进程代码用 `main.<服务>`，前端按代码层级前缀命名；`ai-runtime`、`llm-runtime` 与既有路径式 domain 继续保留。
- 理由：批量重命名收益只在视觉一致性，代价是历史查询分裂且改动面大；增量约束已经足以保证后续接入不需要逐案适配。

### 决策：覆盖补点选择主进程项目包与更新器，数据库只记录缺口

- 盘点显示项目包、更新器与数据库缺少主进程结构化事件；生成与 LLM 已在主进程完整记录，其他前端链路已有至少一个日志点。
- 选择：项目包补 start/completed/failed，更新器在统一状态入口补 info/debug/error；不为数据库逐条 SQL 或初始化补埋点。
- 理由：前两项直接覆盖用户触发的关键长链路，且可在日志窗口即时观察；数据库高频 SQL 日志噪声和敏感面更大，初始化成功日志的边际排查收益较低，留给后续有具体故障时按规范增量补齐。
