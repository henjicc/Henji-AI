...
- 第六、七阶段及全部 23 项代码任务已经完成；当前整体状态为待用户实机验证，没有剩余实现任务或代码阻塞。
- 最终自动门禁全部通过：生产评测 347 项、全仓 846 项、双侧 TypeScript/lint、能力/颜色/Surface/图标/i18n、完整 Electron build 和 CDP smoke。
- 最终手测唯一入口为 `docs/task/内部应用接口与智能助手控制升级/manual-test.md`；已删除逐阶段重复项，只保留 13 个真实 Provider、鼠标、WebGL、隐私、恢复兼容与短时稳定性检查。
- 因本轮修改了 Electron 主进程、preload 和 IPC，执行手测前必须停止旧进程并重新运行 `npm run electron:dev`。
- 用户通过手测后，将 7.2、第七阶段和总览状态从待验证改为已完成，并在本计划及两套交叉计划记录实机结果。
...
- 待完成：仅用户实机验证；7.1、7.2 的代码与自动化均已完成。
- 阻塞：无。

## 任务 7.2 开始接力

- 复用 `test:assistant-eval`、`test:assistant-production`、全量 Vitest、能力门禁与 Electron build/smoke；避免重复跑互相包含的专项。
- 重要记录 012 阈值：一次批量发现、工具调用 ≤12、模型步骤 ≤12、输入 Token ≤25 万、相同无进展指纹最多一次有新证据恢复。
- 最终人工清单必须从跨阶段全局视角去重，只留下自动化无法证明且用户确实需要操作的项目。
