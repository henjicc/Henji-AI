...
- 不建立第三套 evaluator；在现有 regression/minimal evaluator/production 套件中补充固定阈值和关键安全兼容断言。
- 自动化可以确定的 schema、范围、遮罩、状态机、兼容读取和调用计数全部自动化；真实鼠标、3D 画面质量、真实 API key/provider 只保留为最小人工验收。
- 计划代码完成与自动检查通过后标记实现完成；用户人工结果作为交付后的最终实机验收，不伪造已执行结果。
...
## 2026-08-02 · 最终评测与人工验收收口

- 代表性三维组合请求固定为 1 次批量发现、最多 12 次工具、最多 12 轮模型步骤、累计输入最多 25 万 Token；同一无进展指纹只允许 1 次带新证据的恢复尝试。
- 最终验收继续复用既有 evaluator、`test:assistant-production` 和全仓门禁，不新增第三套框架；评测声明的工具名必须能从真实内置注册表解析。
- 语义压缩后的上下文重建必须读取实时会话，而不是进入本轮时的数组副本；二次预算裁剪优先保留已生成的会话语义摘要。
- 人工验收以风险为单位去重：自动覆盖的 22 Surface 和 schema 不再要求逐项点击，只保留真实 Provider、鼠标、WebGL、隐私边界、恢复兼容和短时稳定性。
