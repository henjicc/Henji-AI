...
- 已通过 44/44 Vitest、manifest、颜色/i18n、前后端 lint/tsc、Electron build、原生控件/any/行数/diff 扫描。
- 阻断：`electron:smoke` 干净 build 前后均报资源 403；断言在 `scripts/electron-phase4-smoke.cjs:214`，消息不含 URL。
- 下一步先定位请求 URL/来源，只在证实预期请求后最小处理；随后重跑全套、补 v10 夹具，再由用户执行真实 Electron/MP4 清单。
- 当前不得提交“完成：整体回归与验收”，不得把总任务标为已完成。
