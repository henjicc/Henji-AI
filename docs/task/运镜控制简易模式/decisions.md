...
- **不宽泛忽略 403**：没有请求 URL/来源证据前，保持 Electron smoke 失败，禁止按状态码或文案全局白名单。
- **区分自动与手动完成**：自动链路未全绿时不标代码完成；即使自动通过，也只能在用户操作前标“代码完成、手动验收待用户”。
- **406 行暂不拆分**：`shotCompiler.ts` 位于允许的 400~500 区间，本阶段不做纯行数重构；以后继续修改时优先拆分。
