...
| 命令 | 结果 |
|---|---|
| `npm run test` | 5 个测试文件，40/40 通过（含同模式空卡初始化回归） |
| `npm run lint` | 通过，无错误 |
| `npx tsc --noEmit` | 通过，无错误 |
| `npx tsc -p tsconfig.electron.json --noEmit` | 通过，无错误 |
| `npm run check:colors` | 通过，无硬编码颜色 |
| 原生控件检查 | 业务组件无命中 |
| `git diff --check` | 通过（仅换行符提示） |

### 用户手动验证（真实 Electron，待执行）

1. 在工程列表点“新建工程”，确认默认选中“简易模式 · 推荐”；创建后时间轴显示镜头卡且已有“片段 1”，顶栏显示“简易”。
2. 选中摄像机、角色等对象并检查属性面板，确认无码表按钮；确认“转为专业工程”不可点击。
3. 返回列表，新建“专业模式”工程，确认顶栏显示“专业”、时间轴和属性码表保持原专业功能。
4. 打开一个 v10 及以下旧工程，确认仍按专业模式展示，不会变成简易模式。
5. 保存简易工程，退出并重新启动 `npm run electron:dev` 后重开，确认模式与“片段 1”保持。

按项目约定，本阶段未由 agent 执行鼠标/UI 操作。

## 3.3 整体回归与验收

| 检查 | 结果 |
|---|---|
| manifest / colors / model-i18n | 通过（61 个模型） |
| Vitest | 6 文件，44/44 通过 |
| 前后端 lint / TypeScript | 通过 |
| `npm run electron:build` | 通过 |
| `npm run electron:smoke` | **失败**：资源请求 403，干净 build 前后均复现 |
| 原生控件 / `any` 增量 / diff check | 无业务命中 / 无新增 / 通过 |
| 新增文件体积 | `shotCompiler.ts` 406 行，其余 ≤303 行；无 >500 行新增文件 |

用户手动验证：按 3.3 任务文件的 10 步清单执行基准三卡场景、播放/scrub、细节层、撤销重做、v10 兼容、效果器三路径、单向烘焙、重开持久化、专业模式回归及 720p MP4 抽帧核对。本阶段未自行操作鼠标/UI；因 smoke 阻塞，上述清单尚未标记通过，v10 夹具尚未完成。
...
| 命令 | 结果 |
|---|---|
| `npm run test` | 6 个测试文件，44/44 通过（新增烘焙回归 1 个） |
...
新增覆盖：最终 animation 与 motionSchedule 保留；摄像机 effectors 保留；shots/selectedShotId 清空；editorMode 切为 pro；专业模式拒绝回到简易。

### 用户手动验证（真实 Electron，待执行）

1. 建三张简易镜头卡，包含环绕运镜、角色走跑和已启用摄像机效果器；播放并记住效果。
2. 点击“转为专业工程”，先点取消，确认模式、镜头卡、播放效果和撤销历史均无变化。
3. 再次打开并确认转换，确认文案明确不可逆；完成后顶栏为“专业”，专业时间轴出现轨道与关键帧，镜头卡消失。
4. 播放并与转换前对照，确认运镜、角色动作时间表和效果器一致；编辑一个专业关键帧确认可用。
5. 执行撤销，确认不能回到简易；保存并重启 `npm run electron:dev` 后重开，确认仍是专业工程且效果器配置保留。
...
| 命令 | 结果 |
|---|---|
| `npm run test` | 6 个测试文件，43/43 通过（新增效果器 3 个用例） |
| `npm run lint` | 通过，无错误或警告 |
| `npx tsc --noEmit` | 通过 |
| `npx tsc -p tsconfig.electron.json --noEmit` | 通过 |
| `npm run check:colors` | 通过，无硬编码颜色 |
...
新增覆盖：同一 time 采样严格相等；关闭/零强度返回零偏移；频率改变采样周期且仍保持确定性。

### 用户手动验证（真实 Electron，待执行）

1. 在简易工程和专业工程分别选中摄像机，开启“手持晃动”，播放静态镜头与有位置动画的镜头，确认画面持续轻微晃动。
2. 播放到明显构图后暂停，记录时间并前后 scrub 再回到该时间，确认构图偏移与播放时一致。
3. 开启“呼吸推拉”，确认画面沿视线低频推进/拉远；调高强度、频率后变化即时生效。
4. 导出 MP4，与实时播放对比相同时刻/整体观感；确认无随机跳变且逐帧 seek 稳定。
5. 关闭两种效果器，确认画面恢复静止；保存、重开工程，确认基础摄像机位置未发生漂移。
