...
## 第三阶段-多机位硬切（自动验收完成，等待用户手测，2026-07-11）

| 任务 | 状态 | 一句话摘要 |
|---|---|---|
| 3.1 镜头卡机位字段与渲染机位时间表 | 已完成 | `cameraId`、跨机位强制硬切、渲染机位时间表与首摄像机画幅锁定已由提交 `24c728e` 交付。 |
| 3.2 播放与视口机位切换 | 已完成 | `useRenderCameraId` 派生预览机位；机位徽标、跨机位硬切呈现和画幅遮罩联动已由提交 `17c3f43` 交付。 |
...
| 3.4 整体回归与验收 | 自动验收完成，用户手测待执行 | 全套自动检查与治理复查通过；`electron:smoke` 仅复现既有资源 403，真实 Electron 手测尚未执行。 |

第三阶段实现与自动验收已收口；最终人工验收仅待用户按 3.4 总清单在 Electron 窗口执行。未完成手测前，整体任务保持“等待用户验收”，不可标记为完成。
...
- 用户须在真实 Electron 窗口（`npm run electron:dev`）执行 3.4 的单机位、多机位、兼容三条手测流；自动化不能覆盖鼠标/拖拽/悬浮和 MP4 逐帧视觉一致性。
- `npm run electron:smoke` 仍有既有资源 403 阻塞（`scripts/electron-phase4-smoke.cjs:214`）；本次复现一致，未发现本任务新增自动化失败。
