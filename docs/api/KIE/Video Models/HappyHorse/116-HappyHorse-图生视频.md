...
paths:
  /api/v1/jobs/createTask:
    post:
              ...
              properties:
                model:
                  ...
                  description: |-
                    ...
                    - 该接口必须使用 `happyhorse/image-to-video` 模型
                  enum:
                ...
                input:
                  ...
                  properties:
                    prompt:
                      type: string
                      description: 可选的文本提示词，用于补充/约束由首帧图驱动的视频内容。
                      maxLength: 5000
                      examples:
                        - 一只猫在草地上奔跑
                    image_urls:
                      ...
                      description: >-
                        输入首帧图片 URL（必须且只能 1
                        张）。图像限制：格式：JPEG、JPG、PNG、WEBP。分辨率：宽和高不小于300像素。宽高比：1:2.5～2.5:1。文件大小：不超过10MB。
                      maxItems: 1
                    resolution:
                      type: string
                      description: 输出视频分辨率。
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1184766/apis/api-34251258-run
components:
  schemas:
    ApiResponse:
      type: object
