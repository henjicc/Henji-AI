...
paths:
  /api/v1/jobs/createTask:
    post:
              ...
              properties:
                model:
                  ...
                  description: |-
                    ...
                    - 该接口必须使用 `wan/2-2-animate-replace` 模型
                  examples:
                    - wan/2-2-animate-replace
                callBackUrl:
                  type: string
                ...
                input:
                  ...
                  properties:
                    video_url:
                      description: >-
                        输入视频的 URL。（需为上传后的文件
                        URL，而非文件内容；支持的格式：video/mp4、video/quicktime、video/x-matroska；最大文件大小：10.0MB）
                      type: string
                      examples:
                        - >-
                    ...
                    image_url:
                      description: >-
                        输入图片的 URL。若输入图片与所选宽高比不匹配，将自动缩放并居中裁剪。（需为上传后的文件
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1184766/apis/api-28506696-run
components:
  schemas:
    ApiResponse:
      type: object
