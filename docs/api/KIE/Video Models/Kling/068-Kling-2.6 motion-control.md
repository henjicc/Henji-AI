    ...
    post:
      ...
      description: >
        ...
        <Step title="上传运动视频">
          ...
          **要求：**
          - **文件类型**：MP4、QuickTime 或 Matroska 格式
          - **持续时间**：每个视频 3-30 秒
          - **最大文件大小**：每个文件 100MB
        ...
        - 支持的图像格式：JPEG、PNG、JPG（最大：10MB）

        - 支持的视频格式：MP4、QuickTime、Matroska（最大：100MB，3-30 秒）
              ...
              properties:
                model:
                  ...
                  description: |-
                    ...
                    - 此端点必须为 `kling-2.6/motion-control`
                  examples:
                    - kling-2.6/motion-control
                callBackUrl:
                  type: string
                ...
                input:
                  ...
                  properties:
                    prompt:
                      description: 所需输出的文本描述。最大长度为 2500 个字符。（最大长度：2500 个字符）
                      type: string
                      maxLength: 2500
                      examples:
                        - 卡通角色在跳舞。
                    input_urls:
                      description: >-
                        包含单个图像 URL 的数组。照片必须清楚显示对象的头部、肩膀和躯干。（上传后的文件
                        URL，不是文件内容；接受的类型：image/jpeg, image/png,
                        image/jpg；最大大小：10.0MB,尺寸需要大于 300px, 宽高比 1:2.5 to 2.5:1.）
                      type: array
                      items:
                        type: string
                        format: uri
                    ...
                    video_urls:
                      description: >-
                        包含单个视频 URL 的数组。持续时间必须在 3 到 30
                        秒之间，视频必须清楚显示对象的头部、肩膀和躯干。（上传后的文件
                        URL，不是文件内容；接受的类型：video/mp4, video/quicktime,
                        video/x-matroska；最大大小：100.0MB）
                      type: array
                      items:
                        type: string
                        format: uri
                    ...
                    character_orientation:
                      description: >-
                        生成视频中角色的方向。'image': 与图片中的人相同（最大 10 秒视频）。'video':
                        与视频中的人相同（最大 30 秒视频）。
                      type: string
                      enum:
                        - image
                      ...
                      examples:
                        - video
                    mode:
                      description: 输出分辨率模式。使用 '720p' 表示 720p 或 '1080p' 表示 1080p。
                      type: string
                      enum:
                        - 720p
                        - 1080p
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1184766/apis/api-28506668-run
components:
  schemas:
    ApiResponse:
      type: object
