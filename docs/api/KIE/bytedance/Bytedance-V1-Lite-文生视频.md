...
基于字节跳动先进 AI 模型，通过文本描述生成高质量视频。

## 接口说明

**端点：** `POST /api/v1/jobs/createTask`

**功能：** 使用 `bytedance/v1-lite-text-to-video` 模型基于文本生成视频。
  ...
  --data '
{
  ...
  "input": {
    "prompt": "广角镜头：黎明时分，一艘宁静的帆船在港湾中轻轻摇曳，周围环绕着印象派风格的柔粉、橙黄与象牙白色调。镜头缓缓扫过画面，捕捉水面细腻的倒影，以及光线渐亮时船帆的精致纹理。",
    "aspect_ratio": "16:9",
    "resolution": "720p",
    "duration": "5",
    "camera_fixed": false,
...
提交任务后，可通过统一的查询接口查看任务进度并获取结果。建议查阅 [Get Task Details](/cn/market/common/get-task-detail)。

生产环境中，建议使用 `callBackUrl` 参数接收生成完成的自动通知，而非轮询状态接口。

## 相关资源

*   [Market Overview](/cn/market/quickstart) - 浏览所有可用模型
*   [Common API](/cn/common-api/get-account-credits) - 查看账户积分与使用情况
...
**类型：** `enum<string>`
**默认值：** `bytedance/v1-lite-text-to-video`
**必需：** `是`

用于生成任务的模型名称。必填字段。
*   该接口必须使用 `bytedance/v1-lite-text-to-video` 模型。
...
**示例：**
```json
"广角镜头：黎明时分，一艘宁静的帆船在港湾中轻轻摇曳，周围环绕着印象派风格的柔粉、橙黄与象牙白色调。镜头缓缓扫过画面，捕捉水面细腻的倒影，以及光线渐亮时船帆的精致纹理。"
```
...
Playground 环境下安全校验始终启用。仅可通过 API 将该参数设为 `false` 以关闭安全校验。
