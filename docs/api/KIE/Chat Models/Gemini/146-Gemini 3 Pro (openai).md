  ...
  /gemini-3-pro/v1/chat/completions:
    post:
      ...
      description: >-
        ...
        **`response_format` 和函数调用是互斥的**：不能在同一个请求中同时使用 `response_format`
        和函数调用。请根据您的需求选择其中一种。
        ...
        <Accordion title="JSON Schema 支持">
        ...
        - **`null`**：要允许 null 值，请在类型数组中添加 `"null"`（例如，`{"type": ["string",
        "null"]}`）
        ...
        **对于 `string` 值：**
        ...
        - `format`：指定字符串语法（例如，`date-time`、`date`、`time`）
        ...
        **对于 `array` 值：**
        ...
        - **强类型**：尽可能使用特定类型（`integer`、`string`、`enum`）
              ...
              properties:
                ...
                tools:
                  ...
                  description: >-
                    ...
                    1. **Google 搜索**：`{"type": "function", "function": {"name":
                    "googleSearch"}}` - 通过 Google 搜索启用实时信息检索。

                    2. **函数调用**：定义自己的函数，包含 name、description 和
                    parameters。可以在数组中定义多个函数。函数使用 JSON（具体来说是 OpenAPI
                    架构格式的选定子集）来定义。


                    **重要**：Google 搜索和函数调用是互斥的 -
                    不能在同一个请求中同时使用两者。使用函数调用时，可以包含多个函数定义。函数调用和 `response_format`
                    也是互斥的 - 不能在同一个请求中同时使用两者。
                  items:
                ...
                response_format:
                  description: >-
                    可选的 JSON Schema
                    对象，定义响应的结构。提供此参数时，模型将生成符合此架构的响应。**`response_format`
                    和函数调用是互斥的** - 不能在同一个请求中同时使用两者。
      ...
      responses:
        '200':
          description: 请求成功。当提供 `response_format` 时，响应将符合指定的 JSON 架构。否则，返回标准的聊天完成格式。
          content:
            application/json:
              schema:
                type: object
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1184766/apis/api-28506707-run
components:
  schemas:
    ResponseFormat:
      type: object
      properties:
        type:
          type: string
          description: 仅 json_schema 值
        json_schema:
          type: object
          properties:
            name:
              type: string
              description: 仅structured_output 值
            strict:
              type: boolean
            ...
            schema:
              ...
              properties:
                ...
                properties:
                  ...
                  x-apidog-orders:
                    - response
                  description: 对象将属性名称映射到其 JSON Schema 定义
                  x-apidog-ignore-properties: []
                required:
                  type: array
                  items:
                    type: string
                  description: 必需属性名称数组
                title:
                  type: string
                  description: 模式简述
                description:
          ...
          x-apidog-orders:
            ...
            - strict
            - schema
          description: 用于定义响应结构的 JSON Schema 对象。提供此对象后，模型将生成符合此模式的响应。
          x-apidog-ignore-properties: []
      required:
        - type
        - json_schema
