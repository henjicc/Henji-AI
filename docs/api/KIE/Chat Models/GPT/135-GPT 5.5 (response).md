  ...
  /codex/v1/responses:
    post:
      ...
      description: |-
        ...
        > GPT-5.5 是 OpenAI 开发的用于自动化编程、知识工作、科学研究以及复杂多步骤任务执行的高级推理模型。
        ...
        <CardGroup cols={2}>
          ...
          <Card title="统一端点" icon="code">
            使用统一的 <code>/codex/v1/responses</code> 端点，<code>model</code> 参数设置为 <code>gpt-5-5</code>。
          </Card>
        </CardGroup>
          ...
          application/json:
            schema:
              ...
              properties:
                model:
                  type: string
                  description: 目标模型名称。可选值：`gpt-5-5`。
                  enum:
                    - gpt-5-5
                  x-apidog-enum:
                    - value: gpt-5-5
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1184766/apis/api-34227770-run
components:
  schemas:
    ToolFunction:
      type: object
