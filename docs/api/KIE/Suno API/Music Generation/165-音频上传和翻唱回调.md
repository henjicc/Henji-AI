...
当您向 Suno API 提交音频上传和翻唱任务时，可以通过 `callBackUrl` 参数设置回调地址。任务完成后，系统会自动将结果推送到您指定的地址。
      ...
      tracks.forEach((track, index) => {
        ...
        if (audio_url) {
          downloadFile(audio_url, `suno_cover_${taskId}_${id}.mp3`)
            .then(() => console.log(`翻唱音频下载成功: ${id}`))
            .catch(err => console.error(`翻唱音频下载失败 ${id}:`, err));
        }
        ...
        if (image_url) {
          downloadFile(image_url, `suno_cover_img_${taskId}_${id}.jpeg`)
            .then(() => console.log(`翻唱封面下载成功: ${id}`))
            .catch(err => console.error(`翻唱封面下载失败 ${id}:`, err));
        }
        ...
        for i, track in enumerate(tracks):
            ...
            if audio_url:
                try:
                    ...
                    print(f"翻唱音频下载成功: {track_id}")
                except Exception as e:
                    print(f"翻唱音频下载失败 {track_id}: {e}")
            ...
            if image_url:
                try:
                    ...
                    print(f"翻唱封面下载成功: {track_id}")
                except Exception as e:
                    print(f"翻唱封面下载失败 {track_id}: {e}")
        ...
        foreach ($tracks as $i => $track) {
            ...
            if (!empty($audioUrl)) {
                try {
                    ...
                    error_log("翻唱音频下载成功: $trackId");
                } catch (Exception $e) {
                    error_log("翻唱音频下载失败 $trackId: " . $e->getMessage());
                }
            }
            ...
            if (!empty($imageUrl)) {
                try {
                    ...
                    error_log("翻唱封面下载成功: $trackId");
                } catch (Exception $e) {
                    error_log("翻唱封面下载失败 $trackId: " . $e->getMessage());
                }
            }
