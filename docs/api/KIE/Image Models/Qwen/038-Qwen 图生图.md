...
paths:
  /api/v1/jobs/createTask:
    post:
              ...
              properties:
                model:
                  ...
                  description: |-
                    ...
                    - 该端点必须使用 `qwen/image-to-image` 模型
                  examples:
                    - qwen/image-to-image
                callBackUrl:
                  type: string
                ...
                input:
                  ...
                  properties:
                    prompt:
                      description: 用于图像生成的文本描述（最大长度：5000 字符）
                      type: string
                      maxLength: 5000
                      examples:
                        - ''
                    image_url:
                      description: >-
                        用于引导图像生成的参考图 URL（为上传后的文件
                        URL，而非文件内容；支持类型：image/jpeg、image/png、image/webp；最大大小：10.0MB）
                      type: string
                      examples:
                        - ''
                    strength:
                      description: 去噪强度。1.0 = 完全重构图像；0.0 = 完全保留原图（最小值：0，最大值：1，步长：0.01）
                      type: number
                      minimum: 0
                      maximum: 1
                      default: 0.8
                    ...
                    acceleration:
                      description: >-
                        图像生成的加速级别。可选值：'none'、'regular'、'high'。加速级别越高，生成速度越快。'regular'
                        模式平衡速度与质量。'high' 模式推荐用于不含文字的图像生成
                      type: string
                      enum:
                        - none
                        - regular
                      ...
                      examples:
                        - none
                    negative_prompt:
                      description: 图像生成的反向文本描述（最大长度：500 字符）
                      type: string
                      maxLength: 500
                      examples:
                        - 模糊、丑陋
                    seed:
                      description: 对于同一版本的模型，相同的种子值和文本描述将生成完全相同的图像
                      type: integer
                    num_inference_steps:
                      description: 推理步数（最小值：2，最大值：250，步长：1）
                      type: number
                      minimum: 2
                      maximum: 250
                      default: 30
                      examples:
                        - 30
                    guidance_scale:
                      description: 无分类器引导系数（CFG），用于控制生成图像与文本描述的贴合程度（最小值：0，最大值：20，步长：0.1）
                      type: number
                      minimum: 0
                      maximum: 20
                      default: 2.5
                    ...
                    enable_safety_checker:
                      description: >-
                        在控制台中安全检测功能始终启用。仅可通过 API 将该参数设为 false
                        来关闭此功能。（布尔值（true/false））
                      type: boolean
                      examples:
                        - true
                    nsfw_checker:
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1184766/apis/api-28506654-run
components:
  schemas:
    ApiResponse:
      type: object
