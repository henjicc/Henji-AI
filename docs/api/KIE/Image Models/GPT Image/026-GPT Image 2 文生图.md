...
paths:
  /api/v1/jobs/createTask:
    post:
      ...
      operationId: gpt-image-2-text-to-image-cn
      tags:
          ...
          application/json:
            schema:
              ...
              properties:
                model:
                  type: string
                  description: 用于生成任务的模型名称。该字段为必填项。此接口必须使用 `gpt-image-2-text-to-image` 模型。
                  enum:
                ...
                input:
                  type: object
                  description: 文生图任务的输入参数。
                  required:
                    - prompt
                  properties:
                    prompt:
                      type: string
                      description: 文本提示词。必填 , 最多 20000 个字符。
                      minLength: 1
                      maxLength: 20000
                      examples:
                        - 一张高质量的电影感城市夜景海报，霓虹灯在雨夜街道上形成倒影。
                    aspect_ratio:
                      type: string
                      description: |-
                        生成图片的比例，默认auto
                        注意: 2K 和 4K 不支持以下比例：5:4、4:5、3:1、1:3、9:21。
                      enum:
                        - auto
                        - '1:1'
                        - '3:2'
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/1184766/apis/api-33846828-run
components:
  schemas:
    ApiResponse:
      type: object
