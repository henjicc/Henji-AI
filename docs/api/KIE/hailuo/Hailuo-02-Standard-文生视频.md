# POST Hailuo Standard 文生视频

使用 `hailuo/02-text-to-video-standard` 模型将文本生成视频。
...
*   **类型**: `enum<string>`
*   **默认值**: `hailuo/02-text-to-video-standard`
*   **是否必需**: 是
...
视频时长（单位：秒）。1080p 分辨率不支持生成 10 秒时长的视频（注：本模型未开放 1080p 分辨率，此处为通用约束说明）。
