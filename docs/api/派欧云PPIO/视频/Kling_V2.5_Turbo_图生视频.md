...
**请求方法**: POST
**请求地址**: `https://api.ppinfra.com/v3/async/kling-2.5-turbo-i2v`
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| `image` | string | 是 | - | 视频的首帧图片。支持的图片格式包括 `.jpg`、`.jpeg`、`.png`；图片文件大小不得超过 10MB，分辨率不小于 300*300 像素。 |
| `prompt` | string | 是 | - | 生成视频的正向提示词文本；不可超过 2500 个字符。 |
| `duration` | string | 否 | `"5"` | 生成视频的时长（单位：秒）。可选值：`5`、`10` |
