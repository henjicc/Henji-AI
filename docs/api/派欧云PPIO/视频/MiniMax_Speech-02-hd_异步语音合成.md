...
提交长文本语音合成请求后，返回的 URL 有效期为自 URL 返回开始的 24 个小时，请注意及时下载。
...
**请求地址**：`https://api.ppinfra.com/v3/async/minimax-speech-02-hd`
...
```bash
curl --request POST \
  --url https://api.ppinfra.com/v3/async/minimax-speech-02-hd \
...
| `audio_setting.bitrate` | number | 否 | 128000 | 比特率。可选值：`32000`, `64000`, `128000`, `256000`。仅对 `mp3` 格式生效。 |
...
| `pronunciation_dict.tone` | list | 否 | - | 替换特殊标注的文字、符号及对应注音。格式示例：`["燕少飞/(yan4)(shao3)(fei1)", "达菲/(da2)(fei1)", "omg/oh my god"]`。声调用数字表示：1(阴平), 2(阳平), 3(上声), 4(去声), 5(轻声)。 |
...
| `voice_modify` | object | 否 | - | 声音效果器设置，支持 `mp3`, `wav`, `flac` 格式。 |
...
| 参数名 | 类型 | 是否必需 | 说明 |
| :--- | :--- | :--- | :--- |
| `task_id` | string | 是 | 异步任务的 ID。需使用此 `task_id` 请求 [查询任务结果 API](/docs/models/reference-get-async-task-result) 以获取生成结果。 |
...
```bash
curl \
-X POST https://api.ppinfra.com/v3/async/minimax-speech-02-hd \
...
使用上一步获取的 `task_id` 调用 [查询任务结果 API](/docs/models/reference-get-async-task-result) 获取音频。
...
```
音频文件可从响应中的 `audios[0].audio_url` 获取下载链接，该链接有效期为 24 小时。