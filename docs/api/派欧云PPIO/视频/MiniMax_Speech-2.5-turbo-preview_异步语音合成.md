...
提交长文本语音合成请求后，返回的 URL 有效期为自 URL 返回开始的 24 个小时，请注意下载信息的时间。

适用于整本书籍等长文本的语音生成，任务排队耗时可能会较长。短句生成、语音聊天、在线社交等场景，建议使用 [同步调用语音合成](/docs/models/reference-minimax-speech-2.5-turbo)。

## 请求

**端点**
```
POST https://api.ppinfra.com/v3/async/minimax-speech-2.5-turbo-preview
```

**cURL 示例**
```bash
curl --request POST \
  --url https://api.ppinfra.com/v3/async/minimax-speech-2.5-turbo-preview \
...
| `language_boost` | string | 否 | `null` | 增强对指定小语种/方言的识别能力。 |
...
| `text_normalization` | bool | 否 | `false` | - | 支持英语文本规范化，可提升数字阅读场景的性能，但会略微增加延迟。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `tone` | list | 否 | 替换需要特殊标注的文字、符号及对应的注音。格式示例：`["燕少飞/(yan4)(shao3)(fei1)","达菲/(da2)(fei1)"，"omg/oh my god"]`。声调用数字代替：一声（阴平）为 1，二声（阳平）为 2，三声（上声）为 3，四声（去声）为 4，轻声为 5。 |
...
如果不明确小语种类型，则可以选择 `"auto"`，模型将自主判断小语种类型。
...
| `timbre` | integer | 否 | - | [-100, 100] | 音色调整（磁性/清脆），数值接近 -100，声音更浑厚；数值接近 100，声音更清脆。 |
