...
```
curl --request POST \
  --url https://api.ppio.com/v3/async/seedance-v1.5-pro-i2v \
...
string
必填
图生视频的首帧图像。可以是图像 URL 或 Base64 编码的图像。URL 必须可访问。Base64 格式：`data:image/<图像格式>;base64,<Base64编码>`。支持的格式：jpeg、png、webp、bmp、tiff、gif。宽高比：(0.4, 2.5)，尺寸：(300, 6000) 像素，大小：< 30 MB。
