...
```
curl --request POST \
  --url https://api.ppio.com/v3/async/wan2.6-v2v \
...
基于 AI 的参考生视频视频服务，支持通过文本提示生成高质量视频内容，提供专业的视频生成能力：开箱即用的 REST 推理 API，最佳性能，无冷启动，价格实惠。
...
array
必填
按照参考视频顺序，将参考视频指代为characterx。如两个参考视频时，提示词写为“character1在路边唱歌，character2在旁边跳舞”。单参考时也需要写character1带上1数组长度：1 - 3
...
integer
默认值:5
生成视频的时长，单位为秒。可选值为5、10，默认值为5。duration直接影响费用，费用=单价（基于分辨率）×时长（秒），请在调用前确认模型价格。可选值：`5`, `10`
