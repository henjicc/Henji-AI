...
> 来源: https://ppio.com/docs/models/reference-minimax-speech-2.6-turbo
...
提交长文本语音合成请求后，返回的 url 的有效期为自 url 返回开始的 24 个小时，请注意下载信息的时间。
...
*   **类型**: `string`
*   **必需**: 是
*   **说明**: 待合成的文本，长度限制小于 10000 字符，段落切换用换行符替代。支持自定义文本与文本之间的语音时间间隔，以实现自定义文本语音停顿时间的效果。在字间增加 `<#x#>`，x 单位为秒，支持 0.01-99.99，最多两位小数。需要注意的是文本间隔时间需设置在两个可以语音发音的文本之间，且不能设置多个连续的时间间隔。
...
*   **默认值**: `"1.0"`
*   **范围**: `[0.5, 2]`
*   **说明**: 生成声音的语速，可选，取值越大，语速越快。

#### vol
*   **类型**: `float`
*   **默认值**: `"1.0"`
*   **范围**: `(0, 10]`
*   **说明**: 生成声音的音量，可选，取值越大，音量越高。

#### pitch
*   **类型**: `int`
*   **默认值**: `"0"`
*   **范围**: `[-12, 12]`
*   **说明**: 生成声音的语调，可选，（0 为原音色输出，取值需为整数）。
...
*   **说明**: 生成声音的采样率。可选，默认为 32000。

#### bitrate
*   **类型**: `int`
*   **默认值**: `"128000"`
*   **范围**: `[32000, 64000, 128000, 256000]`
*   **说明**: 生成声音的比特率。可选，默认值为 128000。该参数仅对 mp3 格式的音频生效。

#### format
*   **类型**: `string`
*   **默认值**: `"mp3"`
*   **范围**: `[mp3, pcm, flac, wav]`
*   **说明**: 生成的音频格式。默认 mp3。wav 仅在非流式输出下支持。

#### channel
*   **类型**: `int`
*   **默认值**: `"1"`
*   **说明**: 生成音频的声道数。可选：`1`：单声道，`2`：双声道
...
*   **类型**: `int`
*   **范围**: `[1, 100]`
*   **说明**: 权重，须与 voice_id 同步填写。最多支持 4 种音色混合，取值为整数，单一音色取值占比越高，合成音色越像。
...
*   **类型**: `integer`
*   **范围**: `[-100, 100]`
*   **说明**: 音高调整（低沉/明亮），数值接近 -100，声音更低沉；接近 100，声音更明亮

#### intensity
*   **类型**: `integer`
*   **范围**: `[-100, 100]`
*   **说明**: 强度调整（力量感/柔和），数值接近 -100，声音更刚劲；接近 100，声音更轻柔

#### timbre
*   **类型**: `integer`
*   **范围**: `[-100, 100]`
*   **说明**: 音色调整（磁性/清脆），数值接近 -100，声音更浑厚；数值接近 100，声音更清脆
...
以下是如何使用 Minimax Speech-2.6-turbo 同步请求 API 的示例。
...
```bash
curl \
-X POST https://api.ppinfra.com/v3/minimax-speech-2.6-turbo \
...
**音频文件:**
[](https://static-ppinfra-com-1312767721.cos.ap-shanghai.myqcloud.com/docs/assets/minimax-speech-2.6-turbo-result.mp3)
...
[MiniMax Speech-2.6-hd 异步语音合成](/docs/models/reference-minimax-speech-2.6-hd-async) [MiniMax Speech-2.6-turbo 异步语音合成](/docs/models/reference-minimax-speech-2.6-turbo-async)