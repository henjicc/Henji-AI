...
适用于短句生成、语音聊天、在线社交等场景，耗时短但文本长度限制小于 10000 字符。长文本建议使用 [异步调用语音合成](/docs/models/reference-minimax-speech-02-turbo-async)。
...
*   **必需**: 是
    ...
    *   **类型**: float
    *   **默认值**: `"1.0"`
    *   **描述**: 生成声音的语速，可选，取值越大，语速越快。范围 `[0.5, 2]`。

    **vol**
    *   **类型**: float
    *   **默认值**: `"1.0"`
    *   **描述**: 生成声音的音量，可选，取值越大，音量越高。范围 `(0, 10]`。

    **pitch**
    *   **类型**: int
    *   **默认值**: `"0"`
    *   **描述**: 生成声音的语调，可选，0 为原音色输出，取值需为整数。范围 `[-12, 12]`。
...
*   **类型**: object
    ...
    *   **类型**: int
    *   **默认值**: `"32000"`
    *   **描述**: 生成声音的采样率。可选，默认为 32000。范围 `[8000, 16000, 22050, 24000, 32000, 44100]`。

    **bitrate**
    *   **类型**: int
    *   **默认值**: `"128000"`
    *   **描述**: 生成声音的比特率。可选，默认值为 128000。该参数仅对 mp3 格式的音频生效。范围 `[32000, 64000, 128000, 256000]`。

    **format**
    *   **类型**: string
    *   **默认值**: `"mp3"`
    *   **描述**: 生成的音频格式。默认 mp3，范围 `[mp3, pcm, flac, wav]`。wav 仅在非流式输出下支持。
...
*   **描述**: 与 `voice_id` 二选一必填。
    ...
    **weight**
    *   **类型**: int
    *   **描述**: 权重，须与 voice_id 同步填写。最多支持 4 种音色混合，取值为整数，单一音色取值占比越高，合成音色越像。范围 `[1, 100]`。
...
*   **描述**: 声音效果器设置，该参数支持的音频格式：
    ...
    **pitch**
    *   **类型**: integer
    *   **描述**: 音高调整（低沉/明亮），范围 `[-100, 100]`，数值接近 -100，声音更低沉；接近 100，声音更明亮。

    **intensity**
    *   **类型**: integer
    *   **描述**: 强度调整（力量感/柔和），范围 `[-100, 100]`，数值接近 -100，声音更刚劲；接近 100，声音更轻柔。

    **timbre**
    *   **类型**: integer
    *   **描述**: 音色调整（磁性/清脆），范围 `[-100, 100]`，数值接近 -100，声音更浑厚；数值接近 100，声音更清脆。
...
以下是如何使用 Minimax Speech-02-turbo 同步请求 API 的示例。
...
**音频文件:**
[](https://static-ppinfra-com-1312767721.cos.ap-shanghai.myqcloud.com/docs/assets/minimax-speech-02-turbo-result.mp3)
