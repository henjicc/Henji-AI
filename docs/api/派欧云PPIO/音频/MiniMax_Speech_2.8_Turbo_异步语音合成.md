---
title: "MiniMax Speech 2.8 Turbo 异步语音合成 - PPIO 派欧云文档中心"
url: "https://ppio.com/docs/models/reference-minimax-speech-2.8-turbo-async"
captured_at: "2026-03-22T16:15:02.017Z"
---
# MiniMax Speech 2.8 Turbo 异步语音合成

## 接口信息
- **方法**: POST
- **路径**: `/v3/async/minimax-speech-2.8-turbo`
...
## 说明
这是一个异步 API，只会返回异步任务的 `task_id`。您应该使用该 `task_id` 请求 **查询任务结果 API** 来检索生成结果。
...
### text
- **类型**: string
- **说明**: 待合成音频的文本，限制最长 5 万字符。和 `text_file_id` 二选一必填。
...
### text_file_id
- **类型**: integer
- **说明**: 待合成音频的文本文件 id，单个文件长度限制小于 10 万字符，支持的文件格式：txt、zip。和 `text` 二选一必填，传入后自动校验格式。
...
- **属性**:
  ...
  - **bitrate**: integer，默认值: `128000`，生成音频的比特率。可选范围 `[32000, 64000, 128000, 256000]`，仅对 mp3 格式生效。
...
- **类型**: object，必填
- **属性**:
  - **vol**: number，默认值: `1`，合成音频的音量，取值范围 `(0, 10]`。
  - **pitch**: integer，默认值: `0`，合成音频的语调，取值范围 `[-12, 12]`，0 为原音色输出。
  - **speed**: number，默认值: `1`，合成音频的语速，取值范围 `[0.5, 2]`。
  ...
  - **voice_id**: string，必填，合成音频的音色编号。若需要设置混合音色，请设置 `timber_weights` 参数，本参数设置为空值。支持系统音色、复刻音色以及文生音色三种类型。以下是部分最新的系统音色（ID）：
...
### 其他参数
- **aigc_watermark**: boolean，默认值: `false`，控制在合成音频的末尾添加音频节奏标识，仅对非流式合成生效。
...
- **pronunciation_dict**: object，定义需要特殊标注的文字或符号对应的注音或发音替换规则。在中文文本中，声调用数字表示：一声为 1，二声为 2，三声为 3，四声为 4，轻声为 5。示例如下：`["燕少飞/(yan4)(shao3)(fei1)", "omg/oh my god"]`。
...
| task_id | string | 异步任务的 `task_id`。您应该使用该 `task_id` 请求 **查询任务结果 API** 以获取生成结果。 |
| base_resp | object | 包含状态详情和状态码。 |
